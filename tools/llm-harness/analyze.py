...
def find_reports(reports_dir: Path, run_id: str | None, latest: bool) -> list[Path]:
    ...
    all_json = sorted(
        [p for p in reports_dir.glob("*.json") if "analysis" not in p.name and "SYNTHETIC" not in p.name],
        key=lambda p: p.name,
    )
    if not all_json:
...
def analyse_model(report: dict[str, Any]) -> dict[str, Any]:
    ...
    total = len(outcomes)
    resumed = sum(1 for o in outcomes if o.get("message", "").startswith("resumed"))
    scored = total - resumed
    pass_count = sum(1 for o in outcomes if o.get("status") == "PASS")
    partial_count = sum(1 for o in outcomes if o.get("status") == "PARTIAL_PASS")
    fail_count = sum(1 for o in outcomes if o.get("status") == "FAIL")
    skip_count = sum(1 for o in outcomes if o.get("status") == "SKIP")
    ...
    error_taxonomy = [
        {"attempt": att, "counts": round_cats[att]}
        for att in sorted(round_cats)
    ]

    # --- token/cost efficiency ---
    successful_outcomes = [o for o in outcomes if o.get("status") in ("PASS", "PARTIAL_PASS")]
    ...
    return {
        ...
        "pass_rate": {
            ...
            "any_clean": round((pass_count + partial_count) / scored, 4) if scored else None,
        },
...
def generate_recommendations(analyses: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for a in analyses:
        ...
        p1 = next((c["cumulative_pass_rate"] for c in convergence if c["attempt"] == 1), 0)
        p3 = next((c["cumulative_pass_rate"] for c in convergence if c["attempt"] == 3), 0)
        ...
        round1_cats = next((e["counts"] for e in error_taxonomy if e["attempt"] == 1), {})
        ...
        if total1 > 0 and syn1 / total1 > 0.7:
            recs.append({
                ...
                "recommendation": (
                    ...
                    "surface syntax. The grammar-in-context primer (arm2) showed ~95%+ "
                    "pass@1 — consistently include the full grammar in the system prompt "
                    "for all production runs. Consider building a syntax-only task set "
                    "to isolate the learning curve."
                ),
            })

        # Type errors signal partial learning
        round3_cats = next((e["counts"] for e in error_taxonomy if e["attempt"] == 3), {})
    ...
    for r in comparison.get("models_ranked_by_quality", []):
        lines.append(
            "| {m} | {s} | {sv} | {tp} | {c} | {lat}s | {tps} | {cps} |".format(
                ...
                lat=f"{r['mean_latency_s']:.1f}" if r["mean_latency_s"] else "n/a",
                tps=str(r["tokens_per_success"]) if r["tokens_per_success"] else "n/a",
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("files", nargs="*", type=Path, help="report JSON files to analyse")
    parser.add_argument("--run-id", help="analyse all reports matching this run id prefix")
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-write", action="store_true", help="print markdown to stdout, do not write files"
    )
