; tree-sitter highlight queries for Mycelium — GENERATED by tools/grammar/generate.py.
; Capture names are the ratified RFC-0026 §3.2 (Accepted) standard tree-sitter captures.

["Float" "backbone" "colony" "consume" "cyst" "default" "derive" "else" "fn" "for" "forage" "fuse" "graft" "grow" "hypha" "if" "impl" "in" "lambda" "let" "lower" "match" "matured" "mesh" "nodule" "object" "paradigm" "phylum" "policy" "pub" "reclaim" "spore" "swap" "thaw" "then" "tier" "to" "trait" "type" "use" "via" "wild" "with" "xloc"] @keyword
["Binary" "Bytes" "Dense" "Seq" "Sparse" "Substrate" "Ternary" "VSA"] @type
["BF16" "F16" "F32" "F64"] @type.builtin
["Declared" "Empirical" "Exact" "Proven"] @attribute
