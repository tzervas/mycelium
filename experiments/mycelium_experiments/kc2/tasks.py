...
TASKS: tuple[Task, ...] = (
    ...
    Task(
        ...
        prompt=(
            ...
            "with a `// @matured: true` header line **following the `// nodule: bench` structured-"
            "header marker** (Nodule-Header spec §3; maturation is a scope attribute, RFC-0017, not a "
            "`fn` modifier). In the baseline DSL there is no equivalent marker — just define `main`."
        ),
        expect_main="Binary{8}",
        expect_baseline=("bin", 8),
