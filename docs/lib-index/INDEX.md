...
| `compiler.semcore::checkty_unit_prelude` | fn | `lib/compiler/semcore.myc:4215` | `fn checkty_unit_prelude() => DataInfo` | checkty_unit_prelude: checkty.rs::unit_prelude (DN-137 Alt D / M-1102) -- the second builtin prelude entry, `type Unit = Unit;` -- a payload-free, single-constructor data type (the arity-0 M-826 product-family member). Seeded the SAME way as `checkty_prelude`'s `Bool` (this mirror carries no `home` field at all -- see `unqualify_data_info`'s doc comment -- so the seed is just `DI(name, params, ctors)`, byte-identical in shape to `Bool`'s). | Empirical/Declared |
| `compiler.semcore::nodule_uses_fuse` | fn | `lib/compiler/semcore.myc:4221` | `fn nodule_uses_fuse(items: Vec[Item]) => Bool` | `impl Fuse[...] for ...`? (Never based on whether `fuse(a, b)` is CALLED -- the repr-type fast path in `check_fuse` never touches the trait registry at all; restated from the oracle's own comment.) | Empirical/Declared |
| `compiler.semcore::fuse_redeclare_err` | fn | `lib/compiler/semcore.myc:4237` | `fn fuse_redeclare_err() => Bytes` | fuse_redeclare_err: checkty.rs 1372-1378 / 1381-1387 -- the never-silent refusal for either branch that finds `Fuse` already present in `traits` (a user nodule trying to redeclare the built-in). Both oracle call sites use the SAME message text (FLAG-semcore-32: error TEXT is not differential- compared -- only Ok/Err CLASSIFICATION is; this file's established convention throughout STEP 4/5). | Empirical/Declared |
| `compiler.semcore::seed_fuse_trait` | fn | `lib/compiler/semcore.myc:4244` | `fn seed_fuse_trait(traits: Vec[TraitInfo], items: Vec[Item]) => Result[Vec[TraitInfo], Bytes]` | (already present, on EITHER branch) -- never a silent shadow of the built-in (G2/VR-5), mirroring how redeclaring `Bool` would collide in `types`. | Empirical/Declared |
| `compiler.semcore::fns_contains` | fn | `lib/compiler/semcore.myc:4258` | `fn fns_contains(fns: Vec[Pair[Bytes, FnDecl]], name: Bytes) => Bool` | fns_contains: the `fns.contains_key`-equivalent membership scan over the FLAG-semcore-4 assoc-list fn registry `Vec[Pair[Bytes, FnDecl]]`. | Empirical/Declared |
| `compiler.semcore::register_fns` | fn | `lib/compiler/semcore.myc:4269` | `fn register_fns(items: Vec[Item], acc: Vec[Pair[Bytes, FnDecl]]) => Result[Vec[Pair[Bytes, FnDecl]], Bytes]` | register_fns: checkty.rs 1392-1400 -- Pass 2, the fn-signature registration loop. Non-`Item::Fn` items are skipped (the `let Item::Fn(fd) = item else { continue }` mirror); each fn's OWN type-parameter names (`fnsig_tyvar_names`, the `sig.param_names()` mirror) must have no duplicate, then the fn NAME must be free in the registry so far -- the first refusal wins, `Ok` with the built registry otherwise. | Empirical/Declared |
| `compiler.semcore::NoduleRegs` | type | `lib/compiler/semcore.myc:4286` | `type NoduleRegs = NR(Vec[DataInfo], Vec[Pair[Bytes, FnDecl]], Vec[TraitInfo])` | NoduleRegs: mirrors checkty.rs::NoduleRegs (1336-1340) field-for-field (types, fns, traits). New ctor `NR` (FLAG-semcore-2 scheme; checked against every other family in this file, no collision). | Empirical/Declared |
| `compiler.semcore::NoduleRegs::NR` | ctor | `lib/compiler/semcore.myc:4286` | `NR(Vec[DataInfo], Vec[Pair[Bytes, FnDecl]], Vec[TraitInfo])` | — | Empirical/Declared |
| `compiler.semcore::nr_types` | fn | `lib/compiler/semcore.myc:4288` | `fn nr_types(r: NoduleRegs) => Vec[DataInfo]` | — | Empirical/Declared |
| `compiler.semcore::nr_fns` | fn | `lib/compiler/semcore.myc:4291` | `fn nr_fns(r: NoduleRegs) => Vec[Pair[Bytes, FnDecl]]` | — | Empirical/Declared |
| `compiler.semcore::nr_traits` | fn | `lib/compiler/semcore.myc:4294` | `fn nr_traits(r: NoduleRegs) => Vec[TraitInfo]` | — | Empirical/Declared |
| `compiler.semcore::register_nodule_decls` | fn | `lib/compiler/semcore.myc:4300` | `fn register_nodule_decls(nod: Nodule) => Result[NoduleRegs, Bytes]` | register_nodule_decls: checkty.rs 1348-1401, top level. Runs the four passes in the oracle's EXACT order -- types (seeded with the Bool and Unit (DN-137) preludes), traits, the conditional Fuse seed, then fns -- and returns Ok(NoduleRegs) or the FIRST refusal. | Empirical/Declared |
| `compiler.semcore::closure_field_ty` | fn | `lib/compiler/semcore.myc:4361` | `fn closure_field_ty(t: Ty) => Ty` | — | Empirical/Declared |
| `compiler.semcore::closure_param_ref` | fn | `lib/compiler/semcore.myc:4367` | `fn closure_param_ref(t: Ty) => TypeRef` | — | Empirical/Declared |
| `compiler.semcore::mangle_ty_in_ty` | fn | `lib/compiler/semcore.myc:4374` | `fn mangle_ty_in_ty(t: Ty) => Ty` | — | Empirical/Declared |
| `compiler.semcore::ty_to_source_base` | fn | `lib/compiler/semcore.myc:4392` | `fn ty_to_source_base(t: Ty) => BaseType` | — | Empirical/Declared |
| `compiler.semcore::ty_to_source_ref` | fn | `lib/compiler/semcore.myc:4407` | `fn ty_to_source_ref(t: Ty) => TypeRef` | — | Empirical/Declared |
| `compiler.semcore::ty_to_source_ref_list` | fn | `lib/compiler/semcore.myc:4412` | `fn ty_to_source_ref_list(ts: Vec[Ty]) => Vec[TypeRef]` | ty_to_source_ref_list: map a constructor's applied type args through ty_to_source_ref (the direct recursive walk this nodule uses throughout — no first-class-fn `map` primitive, FLAG-semcore-5's KISS/YAGNI precedent for avoiding a generic higher-order helper in a bounded increment). | Empirical/Declared |
| `compiler.semcore::ty_to_ref_base` | fn | `lib/compiler/semcore.myc:4418` | `fn ty_to_ref_base(t: Ty) => BaseType` | ty_to_ref_base: the MANGLED-NULLARY round-trip (an applied data type's arguments are baked into its mangled name, so it becomes a nullary `Named`) — used for a rewritten `FnDecl`/`Param`/ `Ascribe`'s emitted surface type. | Empirical/Declared |
| `compiler.semcore::ty_to_ref` | fn | `lib/compiler/semcore.myc:4433` | `fn ty_to_ref(t: Ty) => TypeRef` | — | Empirical/Declared |
| `compiler.semcore::ty_to_ref_tagged` | fn | `lib/compiler/semcore.myc:4437` | `fn ty_to_ref_tagged(t: Ty, guarantee: Option[Strength]) => TypeRef` | ty_to_ref_tagged: like ty_to_ref, but attaches the CALLER-supplied `guarantee` (the source declaration's own `@ g` — never derived from `t`, never merged across instantiations). | Empirical/Declared |
| `compiler.semcore::WorkItem` | type | `lib/compiler/semcore.myc:4442` | `type WorkItem = WiFn(Bytes, Vec[Ty], Vec[Width], Vec[Pair[Binary{32}, Bytes]], Vec[Pair[Binary{32}, Bytes]]) \| WiData(Bytes, Vec[Ty]) \| WiMethod(Bytes, Bytes, Ty)` | — | Empirical/Declared |
| `compiler.semcore::WorkItem::WiFn` | ctor | `lib/compiler/semcore.myc:4443` | `WiFn(Bytes, Vec[Ty], Vec[Width], Vec[Pair[Binary{32}, Bytes]], Vec[Pair[Binary{32}, Bytes]])` | — | Empirical/Declared |
| `compiler.semcore::WorkItem::WiData` | ctor | `lib/compiler/semcore.myc:4444` | `WiData(Bytes, Vec[Ty])` | — | Empirical/Declared |
| `compiler.semcore::WorkItem::WiMethod` | ctor | `lib/compiler/semcore.myc:4445` | `WiMethod(Bytes, Bytes, Ty)` | — | Empirical/Declared |
| `compiler.semcore::item_key` | fn | `lib/compiler/semcore.myc:4448` | `fn item_key(item: WorkItem) => Bytes` | item_key: the canonical dedup key of a work item — a kind-tagged mangled string. | Empirical/Declared |
| `compiler.semcore::LVal` | type | `lib/compiler/semcore.myc:4511` | `type LVal = LData(Bytes, Bytes, Vec[LVal]) \| LReprBytes(Bytes) \| LOpaque` | FLAG-semcore-36 (`LVal` -- the eval.rs::L1Value mirror, TRIMMED to what `try_match`'s ported fragment needs). `Data { ty, ctor, fields }` mirrors verbatim (`LData`, FLAG-semcore-2 `L`-prefix scheme extended: no collision against any family declared above). `Repr`/`Substrate`/`Fn` collapse into ONE nullary `LOpaque` (FLAG-semcore-35 explains why this is lossless for the ported arms). `binds` (Rust's `&mut Vec<(String, L1Value)>`, mutated by `push` -- APPEND order) is restated as the FLAG-semcore-6 value-threaded collapse: threaded through and RETURNED, and appended (not prepended, unlike the FLAG-semcore-13 assoc-list convention) via `bind_append` below, because `binds`' ORDER is part of the observable differential (`Vec` equality is order-sensitive) -- unlike `unify`'s substitution, whose FIRST-MATCH lookup semantics make prepend-order invisible. DN-105 / M-1035 (ENB-12): the `Bytes` repr value is LIFTED out of the `LOpaque` collapse into a dedicated `LReprBytes(Bytes)` carrier, so `lval_try_match`'s `PLit` arm can compare a `Bytes` scrutinee against a TEXT (`Str`) byte-string-literal pattern by byte content -- a REAL eval-match mirror of eval.rs's `try_match` `Pattern::Lit` `Repr`-case for the string-literal (#72) target (DN-105 §5). Every OTHER repr (`Binary`/`Ternary`/`Float`) plus `Substrate`/`Fn` stays collapsed into `LOpaque` (their `PLit` eval-match stays deferred -- FLAG-semcore-35), and the `0x..`-hex byte-string \*value synthesis\* stays deferred too (FLAG-semcore-25 -- `lit_value`'s `LBytes` arm), so a `0x..` pattern arm's eval propagates that deferral as an explicit `Err`, never a silent mismatch (G2). The Bytes carrier is thus a MINIMAL, targeted lift, not a full `L1Value` port. | Empirical/Declared |
| `compiler.semcore::LVal::LData` | ctor | `lib/compiler/semcore.myc:4511` | `LData(Bytes, Bytes, Vec[LVal])` | — | Empirical/Declared |
| `compiler.semcore::LVal::LOpaque` | ctor | `lib/compiler/semcore.myc:4511` | `LOpaque` | — | Empirical/Declared |
| `compiler.semcore::LVal::LReprBytes` | ctor | `lib/compiler/semcore.myc:4511` | `LReprBytes(Bytes)` | — | Empirical/Declared |
| `compiler.semcore::bind_append` | fn | `lib/compiler/semcore.myc:4517` | `fn bind_append(binds: Vec[Pair[Bytes, LVal]], n: Bytes, v: LVal) => Vec[Pair[Bytes, LVal]]` | bind_append: append one `(name, val)` binding to the END of `binds` -- mirrors Rust's `binds.push((n.clone(), val.clone()))` exactly (order-preserving; bounded by one pattern's own binder count, never source-input-driven, so no depth budget is warranted -- the same class of bounded structural recursion as `pat_append`/`ty_append` above). | Empirical/Declared |
| `compiler.semcore::ctorinfo_any_named` | fn | `lib/compiler/semcore.myc:4530` | `fn ctorinfo_any_named(ctors: Vec[CtorInfo], n: Bytes) => Bool` | lval_ident_is_ctor_of / ctorinfo_any_named: mirrors try_match's `Ident` guard `self.env.types.get(ty).is_some_and(\|d\| d.ctors.iter().any(\|c\| c.name == \*n))` over the ALREADY -ported `types_lookup`/`di_ctors`/`ci_name` registry accessors (increment 8, M-1013 STEP 3) -- no new registry machinery needed, only the `.any(name == n)` scan. | Empirical/Declared |
| `compiler.semcore::lval_ident_is_ctor_of` | fn | `lib/compiler/semcore.myc:4536` | `fn lval_ident_is_ctor_of(types: Vec[DataInfo], ty: Bytes, n: Bytes) => Bool` | — | Empirical/Declared |
| `compiler.semcore::lval_try_match` | fn | `lib/compiler/semcore.myc:4547` | `fn lval_try_match(types: Vec[DataInfo], pat: Pattern, val: LVal, binds: Vec[Pair[Bytes, LVal]]) => Result[Pair[Bool, Vec[Pair[Bytes, LVal]]], Bytes]` | lval_try_match: the ported fragment (mirrors eval.rs::Evaluator::try_match's `Wildcard`/`Ident`/ `Ctor`/`Tuple`/`Or` arms exactly; `Lit` refuses per FLAG-semcore-35). `site` is DROPPED (the FLAG-semcore-13 precedent: the differential asserts `Ok`/`Err` + the resulting `(bool, binds)`, never the message bytes) -- both the oracle's `L1Error` and this fn's `Bytes` message collapse to `Err(())` on the marshalling harness's `decode_result`. | Empirical/Declared |
| `compiler.semcore::lval_match_zip` | fn | `lib/compiler/semcore.myc:4604` | `fn lval_match_zip(types: Vec[DataInfo], subs: Vec[Pattern], fields: Vec[LVal], binds: Vec[Pair[Bytes, LVal]]) => Result[Pair[Bool, Vec[Pair[Bytes, LVal]]], Bytes]` | lval_match_zip: the `Ctor` arm's `for (sub, fv) in subs.iter().zip(fields.iter())` loop, restated as a fold that THREADS `binds` and short-circuits `Ok(false)` on the first sub-pattern mismatch -- mirrors Rust's `zip` early-stop-at-the-shorter-list semantics exactly (a length mismatch between `subs`/`fields` never occurs in a checked program; where it would, `zip` silently stops rather than erroring, and so does this fold, for byte-for-byte behavioral parity). | Empirical/Declared |
| `compiler.semcore::paradigm_name` | fn | `lib/compiler/semcore.myc:4639` | `fn paradigm_name(t: Ty) => Option[Bytes]` | paradigm_name: mirrors checkty.rs::paradigm_name (checkty.rs 7175-7197) — the swap-paradigm name of a representation type, for the never-silent cross-paradigm framing (RFC-0001 §4.1 / M-892). The four swap-paradigms (Binary/Ternary/Dense/VSA) name themselves; every other `Ty` is `None` — `Seq`/`Bytes`/`Float` are first-class reprs but not v0 swap-paradigms (RFC-0032 D3/D4, ADR-040 §5), and `Data`/`Substrate`/`Var`/`Fn` are not representation types at all (RFC-0024 §3). All 11 arms are enumerated EXPLICITLY (no `_` wildcard), exactly as the Rust does: a future `Ty` variant must force a paradigm decision here, never be absorbed into a silent `None` (G2 — never-silent). | Empirical/Declared |
| `compiler.semcore::cons_list_scan` | fn | `lib/compiler/semcore.myc:4665` | `fn cons_list_scan(ctors: Vec[CtorInfo], name: Bytes, nparams: Binary{32}, nil: Option[Bytes], cons: Option[Bytes]) => Pair[Option[Bytes], Option[Bytes]]` | cons_list_scan: checkty.rs::cons_list_ctors's `for c in &di.ctors { match c.fields.len() ... }` mutable-accumulator loop, restated as a threaded fold over the ctor list carrying the `(nil_name?, cons_name?)` pair (the FLAG-semcore-6 value-threaded collapse of the two `mut` locals). The field-count dispatch mirrors the Rust exactly: 0 fields -> record `nil`; exactly 2 fields whose SECOND is the recursive `Data(name, ..)` self-reference at the type's own arity -> record `cons`; every other shape leaves the accumulator untouched. The 1-field and 3+-field shapes fall to Rust's OUTER `match c.fields.len() { _ => {} }` (the `Nil`/`Cons(_, _)` inner arms below); a 2-field ctor whose 2nd field is NOT the `Data(..)` self-reference falls to Rust's inner `if let Ty::Data(..) = &c.fields[1] { .. } else {}` (the `_` arm on `f1` below). `&c.fields[1]` (Rust indexing) is expressed structurally as the exactly-two-elements shape (`Cons(_, Cons(f1, Nil))`), destructured one level at a time per this nodule's single-level-pattern convention. | Empirical/Declared |
| `compiler.semcore::cons_list_ctors` | fn | `lib/compiler/semcore.myc:4696` | `fn cons_list_ctors(types: Vec[DataInfo], expected: Ty) => Option[Pair[Bytes, Bytes]]` | cons_list_ctors: mirrors checkty.rs::cons_list_ctors (checkty.rs 3592-3624) — recognizes whether `expected` is a 2-constructor linked-list type (one nullary "nil" ctor; one binary "cons" ctor whose SECOND field is the recursive `Data(name, ..)` self-reference at the type's own arity) and returns `(nil_name, cons_name)`; every non-conforming shape yields `None` (never a silent reinterpret — the `Seq{T,N}`/no-context paths stay untouched). Pure structural registry inspection: no Env, no evaluator, no kernel `Value`. `BTreeMap<String, DataInfo>` is the already-established FLAG-semcore-4 `Vec[DataInfo]` + `types_lookup` (no new deviation). The Rust `di.ctors.len() != 2` early-return is expressed by matching the ctor list's shape to exactly two elements before scanning (one level at a time per the single-level-pattern convention). | Empirical/Declared |
| `compiler.semcore::strength_or` | fn | `lib/compiler/semcore.myc:4741` | `fn strength_or(a: Option[Strength], b: Option[Strength]) => Option[Strength]` | strength_or: `Option[Strength]::or` — keep the left guarantee if present, else take the right. The exact semantics of `tr.guarantee.or(concrete.guarantee)`: an occurrence's own `@ g` wins over the concrete type's, and a bare occurrence inherits the concrete's guarantee. | Empirical/Declared |
| `compiler.semcore::subst_typeref_list` | fn | `lib/compiler/semcore.myc:4744` | `fn subst_typeref_list(trs: Vec[TypeRef], param: Bytes, concrete: TypeRef) => Vec[TypeRef]` | — | Empirical/Declared |
| `compiler.semcore::subst_type_param_in_typeref` | fn | `lib/compiler/semcore.myc:4751` | `fn subst_type_param_in_typeref(tr: TypeRef, param: Bytes, concrete: TypeRef) => TypeRef` | — | Empirical/Declared |
...
| `compiler.totality::Expr::Wrapping` | ctor | `lib/compiler/totality.myc:254` | `Wrapping(Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::Consume` | ctor | `lib/compiler/totality.myc:255` | `Consume(Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::Try` | ctor | `lib/compiler/totality.myc:258` | `Try(Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::Colony` | ctor | `lib/compiler/totality.myc:261` | `Colony(Vec[Hypha])` | — | Empirical/Declared |
| `compiler.totality::Expr::Lambda` | ctor | `lib/compiler/totality.myc:262` | `Lambda(Vec[Param], Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::App` | ctor | `lib/compiler/totality.myc:263` | `App(Expr, Vec[Expr])` | — | Empirical/Declared |
| `compiler.totality::Expr::Fuse` | ctor | `lib/compiler/totality.myc:264` | `Fuse(Expr, Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::Reclaim` | ctor | `lib/compiler/totality.myc:265` | `Reclaim(Expr, Expr)` | — | Empirical/Declared |
| `compiler.totality::Expr::Lit` | ctor | `lib/compiler/totality.myc:267` | `Lit(Literal)` | — | Empirical/Declared |
| `compiler.totality::Expr::Ascribe` | ctor | `lib/compiler/totality.myc:268` | `Ascribe(Expr, TypeRef)` | — | Empirical/Declared |
| `compiler.totality::Expr::TupleLit` | ctor | `lib/compiler/totality.myc:269` | `TupleLit(Vec[Expr])` | — | Empirical/Declared |
| `compiler.totality::Totality` | type | `lib/compiler/totality.myc:272` | `type Totality = Total \| Partial` | — | Empirical/Declared |
| `compiler.totality::Totality::Partial` | ctor | `lib/compiler/totality.myc:272` | `Partial` | — | Empirical/Declared |
| `compiler.totality::Totality::Total` | ctor | `lib/compiler/totality.myc:272` | `Total` | — | Empirical/Declared |
| `compiler.totality::WalkDepthExceeded` | type | `lib/compiler/totality.myc:275` | `type WalkDepthExceeded = WDE(Binary{32})` | — | Empirical/Declared |
| `compiler.totality::WalkDepthExceeded::WDE` | ctor | `lib/compiler/totality.myc:275` | `WDE(Binary{32})` | — | Empirical/Declared |
| `compiler.totality::max_walk_depth` | fn | `lib/compiler/totality.myc:279` | `fn max_walk_depth() => Binary{32}` | max_walk_depth: mirrors totality.rs::MAX_WALK_DEPTH (4096) exactly — the same numeric ceiling parse.myc's own `max_expr_depth` uses (FLAG-parse-7's sibling budget, a DIFFERENT traversal). | Empirical/Declared |
| `compiler.totality::max_assignments` | fn | `lib/compiler/totality.myc:284` | `fn max_assignments() => Binary{32}` | max_assignments: mirrors totality.rs::MAX_ASSIGNMENTS (4096) — a DIFFERENT budget (a bound on the mutual-descent position-assignment SEARCH SPACE, not on AST nesting) that happens to share the same numeric value; kept as its own named constant for honesty (never conflated in-file). | Empirical/Declared |
| `compiler.totality::charge_depth` | fn | `lib/compiler/totality.myc:289` | `fn charge_depth(depth: Binary{32}) => Result[Binary{32}, WalkDepthExceeded]` | charge_depth: mirrors the `let depth = depth + 1; if depth > MAX_WALK_DEPTH { return Err(...) }` prologue shared by totality.rs's `walk_expr_at`/`descend_walk`/`pattern_binders` (M-674's FLAG-totality-3 threaded-budget discipline). | Empirical/Declared |
| `compiler.totality::alist_get` | fn | `lib/compiler/totality.myc:299` | `fn alist_get[V](xs: Vec[Pair[Bytes, V]], k: Bytes) => Option[V]` | alist_get: linear-scan lookup by Bytes key (the BTreeMap::get / BTreeMap::contains_key mirror). | Empirical/Declared |
| `compiler.totality::contains_bytes` | fn | `lib/compiler/totality.myc:310` | `fn contains_bytes(xs: Vec[Bytes], x: Bytes) => Bool` | contains_bytes / add_bytes_if_absent: the parse.myc convention (redeclared, self-contained) — the BTreeSet<String>::contains / ::insert mirror over a plain Vec[Bytes]. | Empirical/Declared |
| `compiler.totality::add_bytes_if_absent` | fn | `lib/compiler/totality.myc:319` | `fn add_bytes_if_absent(xs: Vec[Bytes], x: Bytes) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::remove_bytes_acc` | fn | `lib/compiler/totality.myc:327` | `fn remove_bytes_acc(xs: Vec[Bytes], x: Bytes, acc: Vec[Bytes]) => Vec[Bytes]` | remove_bytes: drop every occurrence of `x` from `xs` (the BTreeSet::remove mirror; direct-tail accumulator + rev_acc, bounded by list LENGTH — idiom #2). | Empirical/Declared |
| `compiler.totality::remove_bytes` | fn | `lib/compiler/totality.myc:336` | `fn remove_bytes(xs: Vec[Bytes], x: Bytes) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::remove_all_bytes` | fn | `lib/compiler/totality.myc:339` | `fn remove_all_bytes(xs: Vec[Bytes], names: Vec[Bytes]) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::add_all_bytes` | fn | `lib/compiler/totality.myc:345` | `fn add_all_bytes(xs: Vec[Bytes], names: Vec[Bytes]) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::append_bytes_acc` | fn | `lib/compiler/totality.myc:353` | `fn append_bytes_acc(xs: Vec[Bytes], ys: Vec[Bytes], acc: Vec[Bytes]) => Vec[Bytes]` | append_bytes: `xs ++ ys` (order-irrelevant for the reachability worklist it feeds — direct-tail accumulator + rev_acc, bounded by `xs`'s length — idiom #2). | Empirical/Declared |
| `compiler.totality::append_bytes` | fn | `lib/compiler/totality.myc:359` | `fn append_bytes(xs: Vec[Bytes], ys: Vec[Bytes]) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::rev_acc` | fn | `lib/compiler/totality.myc:364` | `fn rev_acc[A](xs: Vec[A], acc: Vec[A]) => Vec[A]` | rev_acc: the direct-tail list reversal underpinning every accumulator+reverse builder above (the parse.myc M-985 convention, redeclared per self-containment). | Empirical/Declared |
| `compiler.totality::vec_param_len` | fn | `lib/compiler/totality.myc:369` | `fn vec_param_len(ps: Vec[Param]) => Binary{32}` | — | Empirical/Declared |
| `compiler.totality::vec_param_get` | fn | `lib/compiler/totality.myc:372` | `fn vec_param_get(xs: Vec[Param], i: Binary{32}) => Option[Param]` | — | Empirical/Declared |
| `compiler.totality::vec_expr_get` | fn | `lib/compiler/totality.myc:381` | `fn vec_expr_get(xs: Vec[Expr], i: Binary{32}) => Option[Expr]` | — | Empirical/Declared |
| `compiler.totality::app_call_name` | fn | `lib/compiler/totality.myc:396` | `fn app_call_name(head: Expr) => Option[Bytes]` | app_call_name: if `head` is a single-segment `Path`, its name; otherwise `None`. Mirrors the `if let Expr::Path(p) = head.as_ref() { if p.0.len() == 1 { ... } }` guard shared by `collect_calls`'s visitor and `descend_walk`'s `App` case. | Empirical/Declared |
| `compiler.totality::expr_is_smaller` | fn | `lib/compiler/totality.myc:410` | `fn expr_is_smaller(e: Expr, param: Bytes, smaller: Vec[Bytes]) => Bool` | expr_is_smaller: mirrors `match scrutinee.as_ref() { Expr::Path(p) if p.0.len()==1 => p.0[0] == param \|\| smaller.contains(&p.0[0]), _ => false }`. | Empirical/Declared |
| `compiler.totality::collect_calls_expr` | fn | `lib/compiler/totality.myc:429` | `fn collect_calls_expr(e: Expr, fn_names: Vec[Bytes], depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::collect_calls_list` | fn | `lib/compiler/totality.myc:501` | `fn collect_calls_list(xs: Vec[Expr], fn_names: Vec[Bytes], depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::collect_calls_arm_list` | fn | `lib/compiler/totality.myc:510` | `fn collect_calls_arm_list(xs: Vec[Arm], fn_names: Vec[Bytes], depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::collect_calls_hypha_list` | fn | `lib/compiler/totality.myc:519` | `fn collect_calls_hypha_list(xs: Vec[Hypha], fn_names: Vec[Bytes], depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::fn_table_names_acc` | fn | `lib/compiler/totality.myc:533` | `fn fn_table_names_acc(fns: Vec[Pair[Bytes, FnDecl]], acc: Vec[Bytes]) => Vec[Bytes]` | fn_table_names: extract just the names, ORDER PRESERVED (the caller's sortedness precondition, FLAG-totality-1, rides through unchanged). Direct-tail accumulator + rev_acc — bounded by the number of functions in the table (idiom #2). | Empirical/Declared |
| `compiler.totality::fn_table_names` | fn | `lib/compiler/totality.myc:539` | `fn fn_table_names(fns: Vec[Pair[Bytes, FnDecl]]) => Vec[Bytes]` | — | Empirical/Declared |
| `compiler.totality::build_calls` | fn | `lib/compiler/totality.myc:544` | `fn build_calls(fns: Vec[Pair[Bytes, FnDecl]], fn_names: Vec[Bytes], acc: Vec[Pair[Bytes, Vec[Bytes]]]) => Result[Vec[Pair[Bytes, Vec[Bytes]]], WalkDepthExceeded]` | build_calls: one collect_calls_expr per function, in table order (order irrelevant to the resulting lookup-only map — FLAG-totality-1). Direct-tail. | Empirical/Declared |
| `compiler.totality::reaches_step` | fn | `lib/compiler/totality.myc:556` | `fn reaches_step(target: Bytes, calls: Vec[Pair[Bytes, Vec[Bytes]]], seen: Vec[Bytes], stack: Vec[Bytes]) => Bool` | — | Empirical/Declared |
| `compiler.totality::reaches` | fn | `lib/compiler/totality.myc:573` | `fn reaches(from: Bytes, target: Bytes, calls: Vec[Pair[Bytes, Vec[Bytes]]]) => Bool` | — | Empirical/Declared |
| `compiler.totality::scc_inner_scan` | fn | `lib/compiler/totality.myc:579` | `fn scc_inner_scan(others: Vec[Bytes], calls: Vec[Pair[Bytes, Vec[Bytes]]], name: Bytes, assigned: Vec[Bytes], group: Vec[Bytes]) => Pair[Vec[Bytes], Vec[Bytes]]` | — | Empirical/Declared |
| `compiler.totality::scc_outer` | fn | `lib/compiler/totality.myc:594` | `fn scc_outer(remaining: Vec[Bytes], fn_names: Vec[Bytes], calls: Vec[Pair[Bytes, Vec[Bytes]]], assigned: Vec[Bytes], acc: Vec[Vec[Bytes]]) => Vec[Vec[Bytes]]` | — | Empirical/Declared |
| `compiler.totality::strongly_connected` | fn | `lib/compiler/totality.myc:607` | `fn strongly_connected(fn_names: Vec[Bytes], calls: Vec[Pair[Bytes, Vec[Bytes]]]) => Vec[Vec[Bytes]]` | — | Empirical/Declared |
| `compiler.totality::scc_members` | fn | `lib/compiler/totality.myc:612` | `fn scc_members(scc: Vec[Bytes], fns: Vec[Pair[Bytes, FnDecl]]) => Vec[FnDecl]` | — | Empirical/Declared |
| `compiler.totality::member_arities` | fn | `lib/compiler/totality.myc:625` | `fn member_arities(members: Vec[FnDecl]) => Vec[Binary{32}]` | — | Empirical/Declared |
| `compiler.totality::any_zero` | fn | `lib/compiler/totality.myc:631` | `fn any_zero(xs: Vec[Binary{32}]) => Bool` | — | Empirical/Declared |
| `compiler.totality::arity_product_capped_acc` | fn | `lib/compiler/totality.myc:639` | `fn arity_product_capped_acc(arities: Vec[Binary{32}], running: Binary{32}, cap: Binary{32}) => Option[Binary{32}]` | arity_product_capped: FLAG-totality-5's early-exit product (equivalent to, cheaper than, the oracle's "compute in full, compare after"). | Empirical/Declared |
| `compiler.totality::arity_product_capped` | fn | `lib/compiler/totality.myc:650` | `fn arity_product_capped(arities: Vec[Binary{32}], cap: Binary{32}) => Option[Binary{32}]` | — | Empirical/Declared |
| `compiler.totality::decode_pos_acc` | fn | `lib/compiler/totality.myc:656` | `fn decode_pos_acc(members: Vec[FnDecl], arities: Vec[Binary{32}], rem: Binary{32}, acc: Vec[Pair[Bytes, Binary{32}]]) => Vec[Pair[Bytes, Binary{32}]]` | decode_pos: the mixed-radix digit decomposition of `rem` over `arities`, zipped with `members`' names (mirrors group_descends's `pos.insert(fd.sig.name.as_str(), rem % arity); rem /= arity`). Order among `pos` entries is irrelevant (assoc list, lookup only — FLAG-totality-1). | Empirical/Declared |
| `compiler.totality::decode_pos` | fn | `lib/compiler/totality.myc:671` | `fn decode_pos(members: Vec[FnDecl], arities: Vec[Binary{32}], rem: Binary{32}) => Vec[Pair[Bytes, Binary{32}]]` | — | Empirical/Declared |
| `compiler.totality::app_call_ok` | fn | `lib/compiler/totality.myc:676` | `fn app_call_ok(head: Expr, args: Vec[Expr], pos: Vec[Pair[Bytes, Binary{32}]], smaller: Vec[Bytes], ok: Bool) => Bool` | — | Empirical/Declared |
| `compiler.totality::descend_walk` | fn | `lib/compiler/totality.myc:709` | `fn descend_walk(e: Expr, pos: Vec[Pair[Bytes, Binary{32}]], param: Bytes, smaller: Vec[Bytes], ok: Bool, depth: Binary{32}) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::descend_walk_list` | fn | `lib/compiler/totality.myc:779` | `fn descend_walk_list(xs: Vec[Expr], pos: Vec[Pair[Bytes, Binary{32}]], param: Bytes, smaller: Vec[Bytes], ok: Bool, depth: Binary{32}) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::descend_walk_hyphae` | fn | `lib/compiler/totality.myc:788` | `fn descend_walk_hyphae(xs: Vec[Hypha], pos: Vec[Pair[Bytes, Binary{32}]], param: Bytes, smaller: Vec[Bytes], ok: Bool, depth: Binary{32}) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::descend_walk_arms` | fn | `lib/compiler/totality.myc:799` | `fn descend_walk_arms(arms: Vec[Arm], pos: Vec[Pair[Bytes, Binary{32}]], param: Bytes, smaller_outer: Vec[Bytes], ok: Bool, scrut_small: Bool, depth: Binary{32}) => Result[Bool, WalkDepthExceeded]` | descend_walk_arms: mirrors the Match-arm shadow/promote-then-restore dance (see the file-level note above for why no explicit "restore" step is needed in this functional threading). | Empirical/Declared |
| `compiler.totality::pattern_binders_list` | fn | `lib/compiler/totality.myc:824` | `fn pattern_binders_list(xs: Vec[Pattern], depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | pattern_binders: mirrors totality.rs::pattern_binders (every variable a pattern binds, recursively). FLAG-totality-4 covers the `POr` dead-fallback. | Empirical/Declared |
| `compiler.totality::pattern_binders_acc` | fn | `lib/compiler/totality.myc:833` | `fn pattern_binders_acc(p: Pattern, depth: Binary{32}, acc: Vec[Bytes]) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::pattern_binders` | fn | `lib/compiler/totality.myc:847` | `fn pattern_binders(p: Pattern, depth: Binary{32}) => Result[Vec[Bytes], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::assignment_descends_scan` | fn | `lib/compiler/totality.myc:852` | `fn assignment_descends_scan(members: Vec[FnDecl], pos: Vec[Pair[Bytes, Binary{32}]]) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::assignment_descends` | fn | `lib/compiler/totality.myc:878` | `fn assignment_descends(members: Vec[FnDecl], pos: Vec[Pair[Bytes, Binary{32}]]) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::group_descends_search` | fn | `lib/compiler/totality.myc:881` | `fn group_descends_search(members: Vec[FnDecl], arities: Vec[Binary{32}], rem: Binary{32}, combos: Binary{32}) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::group_descends` | fn | `lib/compiler/totality.myc:895` | `fn group_descends(scc: Vec[Bytes], fns: Vec[Pair[Bytes, FnDecl]]) => Result[Bool, WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::scc_recursive_flag` | fn | `lib/compiler/totality.myc:909` | `fn scc_recursive_flag(scc: Vec[Bytes], calls: Vec[Pair[Bytes, Vec[Bytes]]]) => Bool` | — | Empirical/Declared |
| `compiler.totality::assign_totality_all` | fn | `lib/compiler/totality.myc:920` | `fn assign_totality_all(scc: Vec[Bytes], t: Totality, acc: Vec[Pair[Bytes, Totality]]) => Vec[Pair[Bytes, Totality]]` | — | Empirical/Declared |
| `compiler.totality::classify_sccs` | fn | `lib/compiler/totality.myc:926` | `fn classify_sccs(sccs: Vec[Vec[Bytes]], fns: Vec[Pair[Bytes, FnDecl]], calls: Vec[Pair[Bytes, Vec[Bytes]]], acc: Vec[Pair[Bytes, Totality]]) => Result[Vec[Pair[Bytes, Totality]], WalkDepthExceeded]` | — | Empirical/Declared |
| `compiler.totality::classify_all` | fn | `lib/compiler/totality.myc:942` | `fn classify_all(fns: Vec[Pair[Bytes, FnDecl]]) => Result[Vec[Pair[Bytes, Totality]], WalkDepthExceeded]` | classify_all: the public entry (mirrors totality.rs::classify_all/classify_all_inner). PRECONDITION (FLAG-totality-1, not enforced by this file): `fns` must already be sorted ascending by name. | Empirical/Declared |
...
| Symbol | Kind | File:Line | Signature | Summary | Tag |
|---|---|---|---|---|---|
| `std.content` | nodule | `lib/std/content.myc:8` | `nodule std.content` | Self-hosted surface of `std.content` — the content-addressing identity library | Empirical/Declared |
| `std.content::Option` | type | `lib/std/content.myc:87` | `type Option[A] = Some(A) \| None` | Enabler note (route to `enb`, recorded for the M-1006/M-935 ledger — ledger is orchestrator- owned, FLAGged up not edited here): no string/byte `find`/`split`/`contains` prim is registered (only `bytes.len`/`get`/`slice`/`concat`/`eq` + `hash.blake3`). `parse_ref`'s colon scan and `content_ref_from_str`'s `+`-split are therefore hand-rolled recursive byte scans over `Binary{32}` indices — expressible with today's primitive set, but genuine hand-written recursion with zero transpiler assist. A `bytes.find`/`bytes.split_once` prim would collapse these to a few lines; recorded so the enabler track knows this gap-class recurs (parser ports). ── Local prelude types (self-contained — the single-nodule harness has no cross-nodule import; the lib/std/text.myc + lib/std/core.myc self-containedness convention) ──────────────────────────── `Option`/`Result`/`Pair`/`Vec` are declared here rather than imported (no `use` surface in the D5 single-nodule harness). `Bool`/`True`/`False` and `Bytes`/`Binary{W}` are built-in. | Empirical/Declared |
| `std.content::Option::None` | ctor | `lib/std/content.myc:87` | `None` | — | Empirical/Declared |
| `std.content::Option::Some` | ctor | `lib/std/content.myc:87` | `Some(A)` | — | Empirical/Declared |
| `std.content::Result` | type | `lib/std/content.myc:89` | `type Result[A, E] = Ok(A) \| Err(E)` | — | Empirical/Declared |
| `std.content::Result::Err` | ctor | `lib/std/content.myc:89` | `Err(E)` | — | Empirical/Declared |
| `std.content::Result::Ok` | ctor | `lib/std/content.myc:89` | `Ok(A)` | — | Empirical/Declared |
| `std.content::Pair` | type | `lib/std/content.myc:91` | `type Pair[A, B] = Pr(A, B)` | — | Empirical/Declared |
| `std.content::Pair::Pr` | ctor | `lib/std/content.myc:91` | `Pr(A, B)` | — | Empirical/Declared |
| `std.content::Vec` | type | `lib/std/content.myc:93` | `type Vec[A] = Nil \| Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.content::Vec::Cons` | ctor | `lib/std/content.myc:93` | `Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.content::Vec::Nil` | ctor | `lib/std/content.myc:93` | `Nil` | — | Empirical/Declared |
| `std.content::bool_and` | fn | `lib/std/content.myc:96` | `fn bool_and(a: Bool, b: Bool) => Bool` | — | Empirical/Declared |
| `std.content::bool_or` | fn | `lib/std/content.myc:99` | `fn bool_or(a: Bool, b: Bool) => Bool` | — | Empirical/Declared |
| `std.content::bool_not` | fn | `lib/std/content.myc:102` | `fn bool_not(a: Bool) => Bool` | — | Empirical/Declared |
| `std.content::bit_true` | fn | `lib/std/content.myc:107` | `fn bit_true(x: Binary{1}) => Bool` | bit_true: bridge the `lt`/`eq` comparison bit (`Binary{1}`) to `Bool` (the M-747 bool-bridge pattern, lib/std/text.myc::is_ascii_byte). Total (Exact). | Empirical/Declared |
| `std.content::ContentHash` | type | `lib/std/content.myc:114` | `type ContentHash = ContentHash(Bytes)` | — | Empirical/Declared |
| `std.content::ContentHash::ContentHash` | ctor | `lib/std/content.myc:114` | `ContentHash(Bytes)` | — | Empirical/Declared |
| `std.content::content_hash_str` | fn | `lib/std/content.myc:117` | `fn content_hash_str(h: ContentHash) => Bytes` | content_hash_str: the address as `Bytes` (the kernel `as_str`). Field read — Exact. | Empirical/Declared |
| `std.content::digest_eq` | fn | `lib/std/content.myc:122` | `fn digest_eq(a: ContentHash, b: ContentHash) => Bool` | digest_eq: two content hashes are the same identity iff their addresses are byte-equal. Reflexive, symmetric, transitive (a pure `bytes_eq`). Exact, total (lib.rs::digest_eq). | Empirical/Declared |
| `std.content::RefKind` | type | `lib/std/content.myc:128` | `type RefKind = Value \| Def \| Operation \| Policy \| Spore \| Other` | — | Empirical/Declared |
| `std.content::RefKind::Def` | ctor | `lib/std/content.myc:128` | `Def` | — | Empirical/Declared |
| `std.content::RefKind::Operation` | ctor | `lib/std/content.myc:128` | `Operation` | — | Empirical/Declared |
| `std.content::RefKind::Other` | ctor | `lib/std/content.myc:128` | `Other` | — | Empirical/Declared |
| `std.content::RefKind::Policy` | ctor | `lib/std/content.myc:128` | `Policy` | — | Empirical/Declared |
| `std.content::RefKind::Spore` | ctor | `lib/std/content.myc:128` | `Spore` | — | Empirical/Declared |
| `std.content::RefKind::Value` | ctor | `lib/std/content.myc:128` | `Value` | — | Empirical/Declared |
| `std.content::content_ref_kind_prefix` | fn | `lib/std/content.myc:132` | `fn content_ref_kind_prefix(k: RefKind) => Bytes` | content_ref_kind_prefix: the canonical string prefix for a kind (lib.rs::kind_prefix). Match over a finite domain onto fixed `Bytes` literals — total, Exact. | Empirical/Declared |
| `std.content::ref_kind_from_prefix` | fn | `lib/std/content.myc:145` | `fn ref_kind_from_prefix(s: Bytes) => Option[RefKind]` | ref_kind_from_prefix: the inverse; `None` for an unrecognised prefix (lib.rs `RefKind::from_prefix`). Byte-equality against each known prefix (the M-912 `bytes_eq` prim; no string-match construct exists). Total → explicit `Option` (never-silent — G2), Exact. | Empirical/Declared |
| `std.content::ContentRef` | type | `lib/std/content.myc:164` | `type ContentRef = ContentRef(RefKind, ContentHash)` | — | Empirical/Declared |
| `std.content::ContentRef::ContentRef` | ctor | `lib/std/content.myc:164` | `ContentRef(RefKind, ContentHash)` | — | Empirical/Declared |
| `std.content::content_ref_new` | fn | `lib/std/content.myc:167` | `fn content_ref_new(k: RefKind, h: ContentHash) => ContentRef` | content_ref_new: build a ref from an explicit kind and hash (ContentRef::new). Total, Exact. | Empirical/Declared |
| `std.content::as_ref` | fn | `lib/std/content.myc:171` | `fn as_ref(h: ContentHash, k: RefKind) => ContentRef` | as_ref: the free-fn form, argument order `(hash, kind)` → `ContentRef(kind, hash)` (lib.rs::as_ref). | Empirical/Declared |
| `std.content::content_ref_kind` | fn | `lib/std/content.myc:176` | `fn content_ref_kind(r: ContentRef) => RefKind` | content_ref_kind / content_ref_hash / content_ref_into_hash: the C3 "no black boxes" accessors — both the kind and the inner hash are readable (ContentRef::kind/hash/into_hash). Field reads, Exact. | Empirical/Declared |
| `std.content::content_ref_hash` | fn | `lib/std/content.myc:179` | `fn content_ref_hash(r: ContentRef) => ContentHash` | — | Empirical/Declared |
| `std.content::content_ref_into_hash` | fn | `lib/std/content.myc:182` | `fn content_ref_into_hash(r: ContentRef) => ContentHash` | — | Empirical/Declared |
| `std.content::content_ref_as_str_repr` | fn | `lib/std/content.myc:188` | `fn content_ref_as_str_repr(r: ContentRef) => Bytes` | content_ref_as_str_repr: the canonical `<kind-prefix>+<algo>:<digest>` form (ContentRef:: as_str_repr). `bytes_concat` composition — total, Exact. Round-trips through `content_ref_from_str` (asserted three-way in std_content.rs). | Empirical/Declared |
| `std.content::MalformedDigest` | type | `lib/std/content.myc:197` | `type MalformedDigest = MalformedDigest(Bytes, Bytes)` | — | Empirical/Declared |
| `std.content::MalformedDigest::MalformedDigest` | ctor | `lib/std/content.myc:197` | `MalformedDigest(Bytes, Bytes)` | — | Empirical/Declared |
| `std.content::malformed_digest_new` | fn | `lib/std/content.myc:200` | `fn malformed_digest_new(input: Bytes, description: Bytes) => MalformedDigest` | malformed_digest_new: the constructor (MalformedDigest::new). Total, Exact. | Empirical/Declared |
| `std.content::malformed_digest_input` | fn | `lib/std/content.myc:203` | `fn malformed_digest_input(e: MalformedDigest) => Bytes` | — | Empirical/Declared |
| `std.content::malformed_digest_description` | fn | `lib/std/content.myc:206` | `fn malformed_digest_description(e: MalformedDigest) => Bytes` | — | Empirical/Declared |
| `std.content::malformed_digest_display` | fn | `lib/std/content.myc:213` | `fn malformed_digest_display(e: MalformedDigest) => Bytes` | malformed_digest_display: the Rust `Display` impl, `format!("malformed content address {:?}: {}", input, description)`. The `{:?}` on a `String` surrounds it with `"` quotes; reproduced byte- faithfully via the `\"` escape. `bytes_concat` composition — total, Exact. (Differentiated against the real Rust `to_string()` for inputs without embedded quotes/escapes — see std_content.rs.) | Empirical/Declared |
| `std.content::in_range8` | fn | `lib/std/content.myc:226` | `fn in_range8(x: Binary{8}, lo: Binary{8}, hi_excl: Binary{8}) => Bool` | — | Empirical/Declared |
| `std.content::byte_eq8` | fn | `lib/std/content.myc:230` | `fn byte_eq8(x: Binary{8}, v: Binary{8}) => Bool` | byte_eq8: byte `x` equals literal `v`. Total, Exact. | Empirical/Declared |
| `std.content::is_lower` | fn | `lib/std/content.myc:235` | `fn is_lower(x: Binary{8}) => Bool` | Character classes (ASCII): 'a'..='z' = [0x61,0x7B) · '0'..='9' = [0x30,0x3A) · 'A'..='Z' = [0x41,0x5B) · '_' = 0x5F · '-' = 0x2D. | Empirical/Declared |
| `std.content::is_digit_ascii` | fn | `lib/std/content.myc:238` | `fn is_digit_ascii(x: Binary{8}) => Bool` | — | Empirical/Declared |
| `std.content::is_upper` | fn | `lib/std/content.myc:241` | `fn is_upper(x: Binary{8}) => Bool` | — | Empirical/Declared |
| `std.content::is_algo_byte` | fn | `lib/std/content.myc:245` | `fn is_algo_byte(x: Binary{8}) => Bool` | is_algo_byte: `[a-z0-9]` — the algo charset (ContentHash::parse). | Empirical/Declared |
| `std.content::is_digest_byte` | fn | `lib/std/content.myc:249` | `fn is_digest_byte(x: Binary{8}) => Bool` | is_digest_byte: `[A-Za-z0-9_-]` — the digest charset (ContentHash::parse). | Empirical/Declared |
| `std.content::find_colon` | fn | `lib/std/content.myc:257` | `fn find_colon(s: Bytes, i: Binary{32}, len: Binary{32}) => Option[Binary{32}]` | find_colon: first index in `[i, len)` whose byte is `:` (0x3A), else `None`. Recurses on `i+1`; terminates at `i == len` (bounded by the input length; the L1 fuel guard is the never-silent budget). Total → explicit `Option`, Exact. | Empirical/Declared |
| `std.content::all_algo_class` | fn | `lib/std/content.myc:268` | `fn all_algo_class(s: Bytes, i: Binary{32}, end: Binary{32}) => Bool` | all_algo_class / all_digest_class: every byte in `[i, end)` is in the algo / digest charset. Structural-over-index recursion; `True` on the empty range. Total, Exact. | Empirical/Declared |
| `std.content::all_digest_class` | fn | `lib/std/content.myc:277` | `fn all_digest_class(s: Bytes, i: Binary{32}, end: Binary{32}) => Bool` | — | Empirical/Declared |
| `std.content::parse_ref` | fn | `lib/std/content.myc:289` | `fn parse_ref(s: Bytes) => Result[ContentHash, MalformedDigest]` | parse_ref: validate the `<algo>:<digest>` shape, reproducing `lib.rs::parse_ref`'s diagnostic order EXACTLY (missing-colon → algo-empty → digest-empty → algo-charset → digest-charset), so the error `description` bytes match the Rust oracle case-for-case. `Ok(ContentHash(s))` on success. | Empirical/Declared |
| `std.content::parse_ref_ok` | fn | `lib/std/content.myc:312` | `fn parse_ref_ok(s: Bytes) => Bool` | parse_ref_ok: whether the address is well-formed (the accept/reject bit for the differential). Exact. | Empirical/Declared |
| `std.content::parse_ref_description` | fn | `lib/std/content.myc:317` | `fn parse_ref_description(s: Bytes) => Bytes` | parse_ref_description: the error description bytes (or `""` for an accepted address) — lets the differential compare the diagnostic message against the Rust oracle. Exact. | Empirical/Declared |
| `std.content::find_plus` | fn | `lib/std/content.myc:325` | `fn find_plus(s: Bytes, i: Binary{32}, len: Binary{32}) => Option[Binary{32}]` | — | Empirical/Declared |
| `std.content::content_ref_from_str` | fn | `lib/std/content.myc:334` | `fn content_ref_from_str(s: Bytes) => Result[ContentRef, MalformedDigest]` | — | Empirical/Declared |
| `std.content::content_ref_from_str_ok` | fn | `lib/std/content.myc:350` | `fn content_ref_from_str_ok(s: Bytes) => Bool` | content_ref_from_str_ok: the accept/reject bit for the differential. Exact. | Empirical/Declared |
| `std.content::Fallibility` | type | `lib/std/content.myc:357` | `type Fallibility = Total \| Fallible` | — | Empirical/Declared |
| `std.content::Fallibility::Fallible` | ctor | `lib/std/content.myc:357` | `Fallible` | — | Empirical/Declared |
| `std.content::Fallibility::Total` | ctor | `lib/std/content.myc:357` | `Total` | — | Empirical/Declared |
| `std.content::Explainable` | type | `lib/std/content.myc:359` | `type Explainable = Yes \| NotApplicable` | — | Empirical/Declared |
| `std.content::Explainable::NotApplicable` | ctor | `lib/std/content.myc:359` | `NotApplicable` | — | Empirical/Declared |
| `std.content::Explainable::Yes` | ctor | `lib/std/content.myc:359` | `Yes` | — | Empirical/Declared |
| `std.content::MatrixRow` | type | `lib/std/content.myc:361` | `type MatrixRow = MatrixRow(Bytes, Bytes, Fallibility, Bytes, Bytes, Explainable)` | — | Empirical/Declared |
| `std.content::MatrixRow::MatrixRow` | ctor | `lib/std/content.myc:361` | `MatrixRow(Bytes, Bytes, Fallibility, Bytes, Bytes, Explainable)` | — | Empirical/Declared |
| `std.content::row_op` | fn | `lib/std/content.myc:363` | `fn row_op(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.content::row_guarantee` | fn | `lib/std/content.myc:366` | `fn row_guarantee(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.content::row_fallibility` | fn | `lib/std/content.myc:369` | `fn row_fallibility(r: MatrixRow) => Fallibility` | — | Empirical/Declared |
| `std.content::row_error_set` | fn | `lib/std/content.myc:372` | `fn row_error_set(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.content::row_effects` | fn | `lib/std/content.myc:375` | `fn row_effects(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.content::row_explainable` | fn | `lib/std/content.myc:378` | `fn row_explainable(r: MatrixRow) => Explainable` | — | Empirical/Declared |
| `std.content::row_hash_of_value` | fn | `lib/std/content.myc:384` | `fn row_hash_of_value() => MatrixRow` | The 7 rows, hand-transcribed verbatim from `guarantee_matrix::MATRIX` (same order). Every row is `"Exact"`, effect-free (`"none"`), and `NotApplicable` for EXPLAIN; `parse_ref`/`resolve_name` are `Fallible` with a named error set, the rest `Total`. | Empirical/Declared |
| `std.content::row_hash_of_def` | fn | `lib/std/content.myc:387` | `fn row_hash_of_def() => MatrixRow` | — | Empirical/Declared |
| `std.content::row_digest_eq` | fn | `lib/std/content.myc:390` | `fn row_digest_eq() => MatrixRow` | — | Empirical/Declared |
| `std.content::row_as_ref` | fn | `lib/std/content.myc:393` | `fn row_as_ref() => MatrixRow` | — | Empirical/Declared |
| `std.content::row_parse_ref` | fn | `lib/std/content.myc:396` | `fn row_parse_ref() => MatrixRow` | — | Empirical/Declared |
| `std.content::row_resolve_name` | fn | `lib/std/content.myc:406` | `fn row_resolve_name() => MatrixRow` | — | Empirical/Declared |
| `std.content::row_names_of` | fn | `lib/std/content.myc:409` | `fn row_names_of() => MatrixRow` | — | Empirical/Declared |
| `std.content::matrix` | fn | `lib/std/content.myc:412` | `fn matrix() => Vec[MatrixRow]` | — | Empirical/Declared |
| `std.content::matrix_len` | fn | `lib/std/content.myc:425` | `fn matrix_len(xs: Vec[MatrixRow]) => Binary{8}` | matrix_len: the row count as `Binary{8}` (7 fits trivially). O(n) spine-walk (Declared data, mirrors lib/std/core.myc::matrix_len). | Empirical/Declared |
| `std.content::all_guarantee_exact` | fn | `lib/std/content.myc:430` | `fn all_guarantee_exact(xs: Vec[MatrixRow]) => Bool` | all_guarantee_exact: every row's guarantee is EXACTLY `"Exact"` (full content equality via `bytes_eq`) — `guarantee_matrix::tests::every_row_is_exact`. | Empirical/Declared |
| `std.content::all_effects_none` | fn | `lib/std/content.myc:442` | `fn all_effects_none(xs: Vec[MatrixRow]) => Bool` | all_effects_none: every row's effects field is EXACTLY `"none"` — `guarantee_matrix::tests::every_row_declares_no_effects`. | Empirical/Declared |
| `std.content::fallible_count` | fn | `lib/std/content.myc:450` | `fn fallible_count(xs: Vec[MatrixRow]) => Binary{8}` | fallible_count: how many rows are `Fallible` (Rust: exactly 2 — `parse_ref`, `resolve_name`) — `guarantee_matrix::tests::exactly_two_fallible_ops`. | Empirical/Declared |
| `std.content::all_explain_na` | fn | `lib/std/content.myc:461` | `fn all_explain_na(xs: Vec[MatrixRow]) => Bool` | all_explain_na: every row is `NotApplicable` for EXPLAIN — `std.content` does no selection/conversion/approximation (`guarantee_matrix::tests::every_row_is_not_applicable_for_explain`). | Empirical/Declared |
| `std.content::Entry` | type | `lib/std/content.myc:472` | `type Entry = Entry(ContentHash, Bytes)` | — | Empirical/Declared |
| `std.content::Entry::Entry` | ctor | `lib/std/content.myc:472` | `Entry(ContentHash, Bytes)` | — | Empirical/Declared |
| `std.content::NameRegistry` | type | `lib/std/content.myc:474` | `type NameRegistry = Reg(Vec[Entry])` | — | Empirical/Declared |
| `std.content::NameRegistry::Reg` | ctor | `lib/std/content.myc:474` | `Reg(Vec[Entry])` | — | Empirical/Declared |
| `std.content::entry_hash` | fn | `lib/std/content.myc:476` | `fn entry_hash(e: Entry) => ContentHash` | — | Empirical/Declared |
| `std.content::entry_name` | fn | `lib/std/content.myc:479` | `fn entry_name(e: Entry) => Bytes` | — | Empirical/Declared |
| `std.content::name_registry_new` | fn | `lib/std/content.myc:482` | `fn name_registry_new() => NameRegistry` | — | Empirical/Declared |
| `std.content::name_registry_entries` | fn | `lib/std/content.myc:485` | `fn name_registry_entries(r: NameRegistry) => Vec[Entry]` | — | Empirical/Declared |
| `std.content::entries_lookup` | fn | `lib/std/content.myc:490` | `fn entries_lookup(es: Vec[Entry], h: ContentHash) => Option[Bytes]` | entries_lookup: the first entry whose hash `digest_eq`s `h` (the assoc-list resolve). Linear scan, `None` when unbound (never-silent — C1/G2). Exact. | Empirical/Declared |
| `std.content::name_registry_resolve_name` | fn | `lib/std/content.myc:500` | `fn name_registry_resolve_name(r: NameRegistry, h: ContentHash) => Option[Bytes]` | name_registry_resolve_name: `Some(name)` when bound, else `None` (NameRegistry::resolve_name; C1). | Empirical/Declared |
| `std.content::name_registry_names_of` | fn | `lib/std/content.myc:505` | `fn name_registry_names_of(r: NameRegistry, h: ContentHash) => Vec[Bytes]` | name_registry_names_of: 0-or-1-element list per the carried one-name limitation (NameRegistry:: names_of). Empty `Vec` when unbound — never a sentinel (C1). | Empirical/Declared |
| `std.content::entries_replace` | fn | `lib/std/content.myc:510` | `fn entries_replace(es: Vec[Entry], h: ContentHash, name: Bytes) => Pair[Vec[Entry], Option[Bytes]]` | entries_replace: bind `h -> name`, preserving the at-most-one-entry-per-hash invariant — replace in place if present (returning the previous name), else append. Returns `(entries, previous)`. | Empirical/Declared |
| `std.content::name_registry_bind` | fn | `lib/std/content.myc:524` | `fn name_registry_bind(r: NameRegistry, h: ContentHash, name: Bytes) => Pair[NameRegistry, Option[Bytes]]` | name_registry_bind: bind a name, returning `(new registry, previous name Option)` — the NameRegistry::bind contract (re-binding does not change identity; the previous name is returned). | Empirical/Declared |
| `std.content::name_registry_insert` | fn | `lib/std/content.myc:533` | `fn name_registry_insert(r: NameRegistry, h: ContentHash, name: Bytes) => NameRegistry` | name_registry_insert / name_registry_bind_prev: the two projections of `bind` (registry-after, and the previous-name Option) — ergonomic accessors for chaining in the differential. | Empirical/Declared |
| `std.content::name_registry_bind_prev` | fn | `lib/std/content.myc:536` | `fn name_registry_bind_prev(r: NameRegistry, h: ContentHash, name: Bytes) => Option[Bytes]` | — | Empirical/Declared |
| `std.content::entries_len` | fn | `lib/std/content.myc:541` | `fn entries_len(es: Vec[Entry]) => Binary{8}` | name_registry_len / name_registry_is_empty: the entry count and the emptiness predicate (NameRegistry::len/is_empty). O(n) spine-walk / nil-check. Exact. | Empirical/Declared |
| `std.content::name_registry_len` | fn | `lib/std/content.myc:544` | `fn name_registry_len(r: NameRegistry) => Binary{8}` | — | Empirical/Declared |
| `std.content::name_registry_is_empty` | fn | `lib/std/content.myc:547` | `fn name_registry_is_empty(r: NameRegistry) => Bool` | — | Empirical/Declared |
...
| Symbol | Kind | File:Line | Signature | Summary | Tag |
|---|---|---|---|---|---|
| `std.derive_prelude` | nodule | `lib/std/derive_prelude.myc:7` | `nodule std.derive_prelude` | DN-138 WU-1 — the missing primitive/std-type derive-instances: `Init`/`Ord3` over | Empirical/Declared |
| `std.derive_prelude::init` | fn | `lib/std/derive_prelude.myc:27` | `fn init() => Binary{64}` | — | Empirical/Declared |
| `std.derive_prelude::init` | fn | `lib/std/derive_prelude.myc:35` | `fn init() => Bool` | — | Empirical/Declared |
| `std.derive_prelude::init` | fn | `lib/std/derive_prelude.myc:41` | `fn init() => Bytes` | — | Empirical/Declared |
| `std.derive_prelude::cmp` | fn | `lib/std/derive_prelude.myc:57` | `fn cmp(a: Binary{64}, b: Binary{64}) => Binary{8}` | — | Empirical/Declared |
| `std.derive_prelude::cmp` | fn | `lib/std/derive_prelude.myc:66` | `fn cmp(a: Bool, b: Bool) => Binary{8}` | — | Empirical/Declared |
| `std.derive_prelude::bytes_cmp_from` | fn | `lib/std/derive_prelude.myc:73` | `fn bytes_cmp_from(a: Bytes, b: Bytes, i: Binary{32}, la: Binary{32}, lb: Binary{32}) => Binary{8}` | — | Empirical/Declared |
| `std.derive_prelude::cmp` | fn | `lib/std/derive_prelude.myc:102` | `fn cmp(a: Bytes, b: Bytes) => Binary{8}` | — | Empirical/Declared |
...
| `std.error::Result` | type | `lib/std/error.myc:87` | `type Result[A, E] = Ok(A) \| Err(E)` | Never-silent floor (C1/I1) carried over intact: every ported op either transforms the sum (the error/absence survives in the result), re-propagates it, or explicitly recovers it with an honest tag (never upgraded — VR-5). No ported op silently drops an `Err`/`None`. Result/Option — local mirrors of std.result/std.option (see the self-containment note above). | Empirical/Declared |
| `std.error::Result::Err` | ctor | `lib/std/error.myc:87` | `Err(E)` | — | Empirical/Declared |
| `std.error::Result::Ok` | ctor | `lib/std/error.myc:87` | `Ok(A)` | — | Empirical/Declared |
| `std.error::Option` | type | `lib/std/error.myc:89` | `type Option[A] = Some(A) \| None` | — | Empirical/Declared |
| `std.error::Option::None` | ctor | `lib/std/error.myc:89` | `None` | — | Empirical/Declared |
| `std.error::Option::Some` | ctor | `lib/std/error.myc:89` | `Some(A)` | — | Empirical/Declared |
| `std.error::map` | fn | `lib/std/error.myc:97` | `fn map[A, B, E](r: Result[A, E], f: A => B) => Result[B, E]` | map/map_err/and_then/or_else duplicate std.result's landed combinators of the same name (see the self-containment note above) — same semantics, same guarantee (Declared: type-level contract; differential agreement Empirical, std_error.rs), carried at the SAME strength (VR-5). map: apply `f` to the success value; an Err passes through untouched (error preserved in sum). | Empirical/Declared |
| `std.error::map_err` | fn | `lib/std/error.myc:101` | `fn map_err[A, E, F](r: Result[A, E], f: E => F) => Result[A, F]` | map_err: transform the error; an Ok passes through untouched (the Err-side mirror of map). | Empirical/Declared |
| `std.error::and_then` | fn | `lib/std/error.myc:106` | `fn and_then[A, B, E](r: Result[A, E], f: A => Result[B, E]) => Result[B, E]` | and_then: monadic bind — apply `f` to the success value; an Err short-circuits and propagates (never dropped). | Empirical/Declared |
| `std.error::or_else` | fn | `lib/std/error.myc:111` | `fn or_else[A, E](r: Result[A, E], f: E => Result[A, E]) => Result[A, E]` | or_else: explicit recovery hook — apply `f` to the error; an Ok passes through. `f` must yield a Result (recover or re-propagate — never a drop). | Empirical/Declared |
| `std.error::filter` | fn | `lib/std/error.myc:116` | `fn filter[A](o: Option[A], predicate: A => Bool) => Option[A]` | filter: Some(x) where predicate(x) is False becomes None — a typed transition (named absence), not a silent loss (I1/C1). | Empirical/Declared |
| `std.error::inspect` | fn | `lib/std/error.myc:124` | `fn inspect[A, E, B](r: Result[A, E], f: A => B) => Result[A, E]` | inspect: peek the Ok side; the value and sum shape are unchanged. `.myc` closures carry no declared-effect annotations in HOF stage 1 (RFC-0024 §3) — unlike the Rust crate's `FnOnce(&T)`, `f`'s return value is computed and then discarded (never observed further), so this combinator is honestly a structural peek, not an effect-tracked one (no RFC-0016 C6 obligation to discharge since `.myc` has no closure-effect surface yet — captured, not faked). | Empirical/Declared |
| `std.error::inspect_err` | fn | `lib/std/error.myc:133` | `fn inspect_err[A, E, B](r: Result[A, E], f: E => B) => Result[A, E]` | inspect_err: peek the Err side; the value and propagation are unchanged (mirror of inspect). | Empirical/Declared |
| `std.error::ok_or` | fn | `lib/std/error.myc:143` | `fn ok_or[A, E](o: Option[A], err: E) => Result[A, E]` | — | Empirical/Declared |
| `std.error::ok_or_else` | fn | `lib/std/error.myc:148` | `fn ok_or_else[A, E](o: Option[A], f: Unit => E) => Result[A, E]` | ok_or_else: None becomes Err(f(Unit)) — a lazily-computed error (see the Unit substitution note above; f is only invoked on the None arm, so laziness is preserved). | Empirical/Declared |
| `std.error::ok` | fn | `lib/std/error.myc:156` | `fn ok[A, E](r: Result[A, E]) => Option[A]` | ok: Result -> Option. FLAGGED LOSSY CONVERSION (spec §7-Q2/C3): the one op in this surface that discards `E` (Err -> None). C1-honest only because it is an explicitly-named, EXPLAIN-able lossy conversion (this doc comment), never an unflagged drop. Whether it should be gated behind an unmissable name (e.g. ok_discarding_err) awaits RFC-0016 §8-Q3 ratification (spec FLAG Q2, carried over unresolved — not this port's call to make). | Empirical/Declared |
| `std.error::flatten` | fn | `lib/std/error.myc:161` | `fn flatten[A, E](r: Result[Result[A, E], E]) => Result[A, E]` | flatten: collapse a nested Result; the inner Err propagates to the outer — no wrapping is discarded silently. | Empirical/Declared |
| `std.error::unwrap_or` | fn | `lib/std/error.myc:173` | `fn unwrap_or[A, E](r: Result[A, E], fallback: A) => A` | unwrap_or/unwrap_or_else duplicate std.result's landed combinators of the same name (Result side); unwrap_or_option/unwrap_or_else_option duplicate std.option's `unwrap_or`/generalize it with a lazy default (Option side) — see the self-containment note above. Guarantee: Declared (RFC-0014 I2/VR-5 — the substituted default is asserted, not proven; downgrade is the rule). Never-silent (I1/C1): the caller supplies the default/closure explicitly — no panic, no sentinel. unwrap_or: the success value, else the caller-supplied fallback. | Empirical/Declared |
| `std.error::unwrap_or_else` | fn | `lib/std/error.myc:177` | `fn unwrap_or_else[A, E](r: Result[A, E], f: E => A) => A` | unwrap_or_else: the success value, else a computed fallback from the error. | Empirical/Declared |
| `std.error::unwrap_or_option` | fn | `lib/std/error.myc:181` | `fn unwrap_or_option[A](o: Option[A], fallback: A) => A` | unwrap_or_option: the held value, else the caller-supplied fallback (Option variant). | Empirical/Declared |
| `std.error::unwrap_or_else_option` | fn | `lib/std/error.myc:186` | `fn unwrap_or_else_option[A](o: Option[A], f: Unit => A) => A` | unwrap_or_else_option: the held value, else a computed fallback (see the Unit substitution note above; f is only invoked on the None arm). | Empirical/Declared |
...
| `std.fmt::HexPair` | type | `lib/std/fmt.myc:30` | `type HexPair = HP(Binary{8}, Binary{8})` | plus (DN-127/M-1090 WU-1) decimal int-to-Bytes rendering (`digit_byte`/`to_dec`/`to_dec_s`) and `impl Show` for the primitive reprs (`Binary{64}`, `Bytes`, `Bool`) — the value-to-text PROBLEM's native answer (pure `render: T => Bytes`, no new kernel prim, KC-3 held). of a Binary{8} byte. M-717 (E13-1, #462). Self-contained: no import of std.collections or std.text (not available in this worktree; must not be imported per worktree discipline). Guarantee: `hex_digit` is total over its finite nibble domain (Binary{8} inputs 0..15) — the case split is match-defined via `lt` + `add_u` making each in-range arm Exact; nibble >= 16 returns '?' (0x3F), never-silent (G2: the fallback is explicit, not a silent wrap). `nibble_lo(x)` is Exact (total bit-mask over Binary{8}). `nibble_hi(x)` is Exact over the 16 possible masked values via a 4-level lt binary-search match tree — implementable without a right-shift prim. `to_hex` is Declared (structural composition of nibble_hi/nibble_lo + hex_digit). Differential agreement (L1-eval ≡ L0-interp ≡ AOT) is Empirical (trials, std_fmt.rs). Honesty boundary: no reflective `display(Value)` — .myc has no Value introspection; fmt is first-order over concrete types only. Grounding: hand-computed values, three-way verified (L1≡L0≡AOT). crates/mycelium-std-fmt exists but exposes a different Ring-2 surface (no hex_digit/to_hex) — it is not the structural oracle. ── HexPair — the two ASCII hex-digit bytes of a byte encoding ──────────────────────────────────── HexPair: carries the high-nibble digit and the low-nibble digit as ASCII bytes (Binary{8}). Used as the return type of `to_hex` so the result is a first-class value pair. (A Bytes return would require bytes_concat which is not surface-callable — see FLAG-text-3 in text.myc.) | Empirical/Declared |
| `std.fmt::HexPair::HP` | ctor | `lib/std/fmt.myc:30` | `HP(Binary{8}, Binary{8})` | — | Empirical/Declared |
| `std.fmt::hex_digit` | fn | `lib/std/fmt.myc:42` | `fn hex_digit(nibble: Binary{8}) => Binary{8}` | Guarantee: Exact over the finite in-range domain (0..15) — the `lt`/`add_u` computation is a closed-form arithmetic on the finite range; Declared for the fallback arm (structurally necessary; unreachable for well-formed nibbles, never-silent for malformed ones). The `lt` prim returns Binary{1}; we use a match { 0b1 => …, _ => … } bridge. | Empirical/Declared |
| `std.fmt::nibble_lo` | fn | `lib/std/fmt.myc:51` | `fn nibble_lo(x: Binary{8}) => Binary{8}` | — | Empirical/Declared |
| `std.fmt::nibble_hi` | fn | `lib/std/fmt.myc:63` | `fn nibble_hi(x: Binary{8}) => Binary{8}` | The masked value `m = and(x, 0b1111_0000)` is one of exactly 16 values; every Binary{8} produces exactly one leaf via the 4-branch binary search (Exact). Each leaf returns the nibble value (masked 0x00 → nibble 0, 0x10 → nibble 1, …, 0xf0 → nibble 15) as a Binary{8} constant. | Empirical/Declared |
| `std.fmt::to_hex` | fn | `lib/std/fmt.myc:95` | `fn to_hex(x: Binary{8}) => HexPair` | — | Empirical/Declared |
| `std.fmt::digit_byte` | fn | `lib/std/fmt.myc:125` | `fn digit_byte(d: Binary{8}) => Bytes` | SECOND CORRECTION over DN-127 §3's own sketch (found by `myc check`, not asserted — VR-5): the sketch wrote bare-decimal match patterns (`0 => "0", 1 => "1", …`), expecting RFC-0012 §4.3 ambient resolution to fill their width from the `Binary{8}` scrutinee. Empirically that does NOT happen outside an explicit `with paradigm Binary { … }` block: the ambient-resolution PRE-PASS (`crate::ambient::resolve`) only rewrites a bare `Literal::Int` to a width-resolvable `Literal::AmbientInt` when it is lexically inside such a block (`ambient.rs:761-784`, "with no ambient it stays `Literal::Int`" — the checker's \*own\* per-callsite anchoring, verified working for `lt(n, 10)`/`div_u(n, 10)`-style call ARGUMENTS above §6553-6588, never fires for a bare literal in this position because it never reaches the checker as `AmbientInt` in the first place). Confirmed by `myc check`: the original bare-decimal-pattern form failed with "a bare integer literal has no representation family (no cross-family defaulting, Q6)". Fixed here with explicit `Binary{8}` literals — no ambient dependency, and the same style every other `.myc` nodule in this tree already uses (`hex_digit` above, `text.myc`, `math.myc`) — never faked, never silently downgraded past what actually checks (VR-5/G2). `_ => "?"` is the never-silent fallback (G2) — unreachable for an in-range digit (`to_dec` below only ever narrows a value already proven `< 10` into this domain), explicit for a malformed one (mirrors `hex_digit`'s own `_ => 0b0011_1111` ('?')). | Empirical/Declared |
| `std.fmt::to_dec` | fn | `lib/std/fmt.myc:161` | `fn to_dec(n: Binary{64}) => Bytes` | Guarantee: Exact over the base case (a single in-range digit — `to_dec(0) = "0"`, the base case fires immediately since `0 < 10`); Declared for the recursive composition (structural, correct by construction for the itoa recurrence — decreasing, since `div_u(n, 10) < n` for `n >= 10` — not a machine-proven termination bound). Three-way differential agreement is Empirical (trials). | Empirical/Declared |
| `std.fmt::to_dec_s` | fn | `lib/std/fmt.myc:196` | `fn to_dec_s(n: Binary{64}) => Bytes` | Guarantee: Declared (structural composition of Exact/Declared parts). Honest residual (checked, not glossed): `neg_s` on the minimum representable `Binary{64}` value (two's-complement `i64::MIN`) has no positive-magnitude representation and is a never-silent runtime `Overflow` refusal (`neg_s`'s own contract, M-766) — `to_dec_s` inherits that refusal rather than silently wrapping or truncating (G2), for exactly that one boundary value. | Empirical/Declared |
| `std.fmt::render` | fn | `lib/std/fmt.myc:222` | `fn render(x: Binary{64}) => Bytes` | — | Empirical/Declared |
...
| Symbol | Kind | File:Line | Signature | Summary | Tag |
|---|---|---|---|---|---|
| `std.numerics` | nodule | `lib/std/numerics.myc:7` | `nodule std.numerics` | Self-hosted honesty-crux surface of `std.numerics` — the guarantee lattice + weakest-wins | Empirical/Declared |
| `std.numerics::Guarantee` | type | `lib/std/numerics.myc:77` | `type Guarantee = GExact \| GProven \| GEmpirical \| GDeclared` | — | Empirical/Declared |
| `std.numerics::Guarantee::GDeclared` | ctor | `lib/std/numerics.myc:77` | `GDeclared` | — | Empirical/Declared |
| `std.numerics::Guarantee::GEmpirical` | ctor | `lib/std/numerics.myc:77` | `GEmpirical` | — | Empirical/Declared |
| `std.numerics::Guarantee::GExact` | ctor | `lib/std/numerics.myc:77` | `GExact` | — | Empirical/Declared |
| `std.numerics::Guarantee::GProven` | ctor | `lib/std/numerics.myc:77` | `GProven` | — | Empirical/Declared |
| `std.numerics::rank` | fn | `lib/std/numerics.myc:81` | `fn rank(g: Guarantee) => Binary{8}` | rank: lattice rank 0..3, strongest = 0 (Exact) … weakest = 3 (Declared), as Binary{8}. Mirrors `GuaranteeStrength::rank`. Total, match-defined → Exact. | Empirical/Declared |
| `std.numerics::meet` | fn | `lib/std/numerics.myc:97` | `fn meet(a: Guarantee, b: Guarantee) => Guarantee` | Mutation witness: replacing any arm with a STRONGER result (e.g. GProven ∧ GDeclared => GProven) would upgrade a composed strength past its weakest input — the exact VR-5 violation this guards. | Empirical/Declared |
| `std.numerics::is_stronger_eq` | fn | `lib/std/numerics.myc:118` | `fn is_stronger_eq(a: Guarantee, b: Guarantee) => Bool` | is_stronger_eq: True iff `a` is at least as strong as `b` (rank(a) <= rank(b)). The lattice order as a total predicate; delegates to the Exact `lt` prim over the rank codes (same shape as lib/std/select.myc::is_at_least). Exact. | Empirical/Declared |
| `std.numerics::Vec` | type | `lib/std/numerics.myc:122` | `type Vec[A] = Nil \| Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.numerics::Vec::Cons` | ctor | `lib/std/numerics.myc:122` | `Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.numerics::Vec::Nil` | ctor | `lib/std/numerics.myc:122` | `Nil` | — | Empirical/Declared |
| `std.numerics::meet_all` | fn | `lib/std/numerics.myc:128` | `fn meet_all(xs: Vec[Guarantee]) => Guarantee` | meet_all: the meet-fold of a sequence of strengths, weakest-wins, from GExact (the TOP identity). The empty sequence yields GExact. Mirrors `GuaranteeStrength::meet_all` (= `propagate` with an Exact intrinsic) — the composition rule n-ary ops take over their inputs (FR-N2). Total (Exact by composition of the Exact `meet`). | Empirical/Declared |
| `std.numerics::BoundBasis` | type | `lib/std/numerics.myc:135` | `type BoundBasis = BProvenThm(Bytes) \| BEmpiricalFit(Binary{8}, Bytes) \| BUserDeclared` | — | Empirical/Declared |
| `std.numerics::BoundBasis::BEmpiricalFit` | ctor | `lib/std/numerics.myc:135` | `BEmpiricalFit(Binary{8}, Bytes)` | — | Empirical/Declared |
| `std.numerics::BoundBasis::BProvenThm` | ctor | `lib/std/numerics.myc:135` | `BProvenThm(Bytes)` | — | Empirical/Declared |
| `std.numerics::BoundBasis::BUserDeclared` | ctor | `lib/std/numerics.myc:135` | `BUserDeclared` | — | Empirical/Declared |
| `std.numerics::basis_strength` | fn | `lib/std/numerics.myc:144` | `fn basis_strength(b: BoundBasis) => Guarantee` | Mutation witness: mapping BUserDeclared to GProven (an upgrade with no checked basis) is the canonical VR-5 violation; the differential against BoundBasis::strength catches it. | Empirical/Declared |
| `std.numerics::Approx` | type | `lib/std/numerics.myc:158` | `type Approx[A] = MkApprox(A, BoundBasis, Guarantee)` | — | Empirical/Declared |
| `std.numerics::Approx::MkApprox` | ctor | `lib/std/numerics.myc:158` | `MkApprox(A, BoundBasis, Guarantee)` | — | Empirical/Declared |
| `std.numerics::attach` | fn | `lib/std/numerics.myc:164` | `fn attach[A](value: A, basis: BoundBasis) => Approx[A]` | attach: build an Approx by deriving the strength from the basis (spec §3 `attach`; VR-5 — the tag is exactly what the basis supports, never asserted). declared/empirical are the named aliases (in the Rust source their bodies are identical to attach — all three derive from the basis; only the omitted `proven` differs, FLAG-num-2). Declared (type-level carrier contract). | Empirical/Declared |
| `std.numerics::declared` | fn | `lib/std/numerics.myc:169` | `fn declared[A](value: A, basis: BoundBasis) => Approx[A]` | declared: the explicit, always-flagged escape hatch (spec §3 `declared` / M-I4). Strength is derived, not asserted — a UserDeclared basis yields GDeclared. Declared. | Empirical/Declared |
| `std.numerics::empirical` | fn | `lib/std/numerics.myc:174` | `fn empirical[A](value: A, basis: BoundBasis) => Approx[A]` | empirical: an Empirical-basis carrier; strength derived from the basis (an EmpiricalFit basis yields GEmpirical; a weaker basis is honored, never upgraded). Declared. | Empirical/Declared |
| `std.numerics::approx_strength` | fn | `lib/std/numerics.myc:178` | `fn approx_strength[A](a: Approx[A]) => Guarantee` | approx_strength: project the honest (basis-derived, invariant) strength. Total projection (Exact). | Empirical/Declared |
| `std.numerics::approx_value` | fn | `lib/std/numerics.myc:182` | `fn approx_value[A](a: Approx[A]) => A` | approx_value: project the carried value. Total (Exact). | Empirical/Declared |
| `std.numerics::approx_basis` | fn | `lib/std/numerics.myc:186` | `fn approx_basis[A](a: Approx[A]) => BoundBasis` | approx_basis: project the basis. Total (Exact). | Empirical/Declared |
| `std.numerics::combine_strength` | fn | `lib/std/numerics.myc:192` | `fn combine_strength[A](x: Approx[A], y: Approx[A]) => Guarantee` | combine_strength: the STRENGTH half of the FR-N2 `combine` — the meet of two carriers' strengths (weakest-wins). The ε-bound composition half (compose_error_bound, outward rounding) is FLAG-num-1 (kernel-owned, float-valued), so this ports only the never-upgrading strength rule. Exact. | Empirical/Declared |
| `std.numerics::NumErr` | type | `lib/std/numerics.myc:200` | `type NumErr = BadEps \| BadDelta \| NoRule \| NormMismatch \| Overflow` | — | Empirical/Declared |
| `std.numerics::NumErr::BadDelta` | ctor | `lib/std/numerics.myc:200` | `BadDelta` | — | Empirical/Declared |
| `std.numerics::NumErr::BadEps` | ctor | `lib/std/numerics.myc:200` | `BadEps` | — | Empirical/Declared |
| `std.numerics::NumErr::NoRule` | ctor | `lib/std/numerics.myc:200` | `NoRule` | — | Empirical/Declared |
| `std.numerics::NumErr::NormMismatch` | ctor | `lib/std/numerics.myc:200` | `NormMismatch` | — | Empirical/Declared |
| `std.numerics::NumErr::Overflow` | ctor | `lib/std/numerics.myc:200` | `Overflow` | — | Empirical/Declared |
| `std.numerics::num_err_code` | fn | `lib/std/numerics.myc:206` | `fn num_err_code(e: NumErr) => Binary{8}` | num_err_code: a stable Binary{8} discriminant for each refusal variant, in the Rust enum's own declaration order. Total, match-defined → Exact. The row-4 differential ties each code to the live `mycelium_std_numerics::NumErr` variant via an exhaustive Rust match (a variant add/remove breaks the differential at compile time — never silent). | Empirical/Declared |
| `std.numerics::CheckErr` | type | `lib/std/numerics.myc:220` | `type CheckErr = Rejected(Float, Float) \| Malformed` | CheckErr: structured verdict for the tier-i re-validation checker (spec §3; RFC-0013). Emitted clean by mycelium-transpile; graduated verbatim. `Rejected(Float, Float)` carries the (recomputed, claimed) ε pair — the Float TYPE parses + myc-checks, but a Float VALUE cannot be constructed in the `.myc` runtime yet (FLAG-num-1 / `enb` Gap A), so only `Malformed` is constructible/evaluable today; the `Rejected` arm is declared for surface fidelity, exercised once floats land. | Empirical/Declared |
| `std.numerics::CheckErr::Malformed` | ctor | `lib/std/numerics.myc:220` | `Malformed` | — | Empirical/Declared |
| `std.numerics::CheckErr::Rejected` | ctor | `lib/std/numerics.myc:220` | `Rejected(Float, Float)` | — | Empirical/Declared |
| `std.numerics::is_malformed` | fn | `lib/std/numerics.myc:224` | `fn is_malformed(c: CheckErr) => Bool` | is_malformed: True iff the checker verdict is the (Float-free) Malformed variant. Total → Exact. Never-silent: a rejected-vs-malformed distinction is explicit, never collapsed. | Empirical/Declared |
...
| `std.recover::Guarantee` | type | `lib/std/recover.myc:111` | `type Guarantee = GExact \| GProven \| GEmpirical \| GDeclared` | Unit — the missing `() => T` closure-domain form (std.error.myc precedent) is filled by the built-in PRELUDE `Unit` type since DN-137/M-1102 (a hand-seeded `type Unit = Unit;`, `checkty::unit_prelude`); the local nullary-marker redeclaration this file used to carry here is gone (it would now collide with the prelude — "duplicate type declaration"). Guarantee — the kernel `GuaranteeStrength` lattice `Exact ⊐ Proven ⊐ Empirical ⊐ Declared` as VALUE-level data, `G`-prefixed (reserved-keyword substitution; std.diag precedent). | Empirical/Declared |
...
| `std.spores::Result` | type | `lib/std/spore.myc:118` | `type Result[A, E] = Ok(A) \| Err(E)` | Never-silent floor (C1/G2) carried over intact: a hash mismatch is the explicit, named HashMismatch(expected, found) — both hashes carried (G11) — never a silent accept; a missing/ambiguous deploy input is the named DeployError; an over-strength resonator manifest is unrepresentable through `manifest_new`/`regrowth_new` (the FR-C2 ceiling, VR-5). Guarantee tags (VR-5, carried at the SAME strength as the Rust crate's matrix): identity/verify/ manifest/validate/display ops Exact (deterministic, no accuracy semantics); germinate Empirical (the native deploy path is not proven end-to-end — the oracle's own tag, never upgraded by translation); the no-opaque-lowering check Declared (structural assertion, MLIR pending); matrix row DATA Declared (hand-transcribed, live-oracle-checked in std_spore.rs); differential agreement Empirical (std_spore.rs). ── local mirrors (single-nodule harness — see the substitution notes above) ──────────────────── | Empirical/Declared |
| `std.spores::Result::Err` | ctor | `lib/std/spore.myc:118` | `Err(E)` | — | Empirical/Declared |
| `std.spores::Result::Ok` | ctor | `lib/std/spore.myc:118` | `Ok(A)` | — | Empirical/Declared |
| `std.spores::Option` | type | `lib/std/spore.myc:120` | `type Option[A] = Some(A) \| None` | — | Empirical/Declared |
| `std.spores::Option::None` | ctor | `lib/std/spore.myc:120` | `None` | — | Empirical/Declared |
| `std.spores::Option::Some` | ctor | `lib/std/spore.myc:120` | `Some(A)` | — | Empirical/Declared |
| `std.spores::Vec` | type | `lib/std/spore.myc:122` | `type Vec[A] = Nil \| Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.spores::Vec::Cons` | ctor | `lib/std/spore.myc:122` | `Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.spores::Vec::Nil` | ctor | `lib/std/spore.myc:122` | `Nil` | — | Empirical/Declared |
| `std.spores::Guarantee` | type | `lib/std/spore.myc:130` | `type Guarantee = GExact \| GProven \| GEmpirical \| GDeclared` | Unit — the nullary marker for `verify`'s `Ok(())` (std.error/std.recover precedent) is the built-in PRELUDE `Unit` type since DN-137/M-1102 (`type Unit = Unit;`, `checkty::unit_prelude`); the local redeclaration this file used to carry here is gone (would now collide with the prelude — "duplicate type declaration"). Guarantee — the kernel `GuaranteeStrength` lattice `Exact ⊐ Proven ⊐ Empirical ⊐ Declared` as VALUE-level data, `G`-prefixed (reserved-keyword substitution; std.diag precedent). | Empirical/Declared |
| `std.spores::Guarantee::GDeclared` | ctor | `lib/std/spore.myc:130` | `GDeclared` | — | Empirical/Declared |
| `std.spores::Guarantee::GEmpirical` | ctor | `lib/std/spore.myc:130` | `GEmpirical` | — | Empirical/Declared |
| `std.spores::Guarantee::GExact` | ctor | `lib/std/spore.myc:130` | `GExact` | — | Empirical/Declared |
| `std.spores::Guarantee::GProven` | ctor | `lib/std/spore.myc:130` | `GProven` | — | Empirical/Declared |
| `std.spores::bool_and` | fn | `lib/std/spore.myc:133` | `fn bool_and(a: Bool, b: Bool) => Bool` | — | Empirical/Declared |
| `std.spores::bool_not` | fn | `lib/std/spore.myc:136` | `fn bool_not(a: Bool) => Bool` | — | Empirical/Declared |
| `std.spores::bool_text` | fn | `lib/std/spore.myc:140` | `fn bool_text(b: Bool) => Bytes` | bool_text: the Rust `Display for bool` rendering ("true"/"false") — Exact. | Empirical/Declared |
| `std.spores::cat` | fn | `lib/std/spore.myc:144` | `fn cat(a: Bytes, b: Bytes) => Bytes` | cat: byte-string concatenation (the Exact, total bytes_concat prim — RFC-0032 D4). | Empirical/Declared |
| `std.spores::b32_zero` | fn | `lib/std/spore.myc:147` | `fn b32_zero() => Binary{32}` | — | Empirical/Declared |
| `std.spores::b32_one` | fn | `lib/std/spore.myc:150` | `fn b32_one() => Binary{32}` | — | Empirical/Declared |
| `std.spores::b32_two` | fn | `lib/std/spore.myc:153` | `fn b32_two() => Binary{32}` | — | Empirical/Declared |
| `std.spores::b32_three` | fn | `lib/std/spore.myc:156` | `fn b32_three() => Binary{32}` | — | Empirical/Declared |
| `std.spores::b32_four` | fn | `lib/std/spore.myc:159` | `fn b32_four() => Binary{32}` | — | Empirical/Declared |
| `std.spores::nonempty` | fn | `lib/std/spore.myc:164` | `fn nonempty(b: Bytes) => Bool` | nonempty: True iff `b` has at least one byte (bytes_len is the Exact prim at Binary{32}; std.diag/std.recover precedent). | Empirical/Declared |
| `std.spores::bytes_eq_from` | fn | `lib/std/spore.myc:171` | `fn bytes_eq_from(a: Bytes, b: Bytes, i: Binary{32}, n: Binary{32}) => Bool` | bytes_eq_from: equal-length tail comparison over [i, n) — 4 bytes per recursion frame so a canonical 71-byte content-hash string fits the L1 depth budget (see the substitution note). Every bytes_get is bounds-guarded by the preceding `eq(index, n)` check, so the prim's out-of-range refusal is unreachable by construction (total on its domain). Exact. | Empirical/Declared |
| `std.spores::bytes_eq` | fn | `lib/std/spore.myc:201` | `fn bytes_eq(a: Bytes, b: Bytes) => Bool` | bytes_eq: whole byte-string equality composed from the D4 prims (no equality prim exists — the gap std.diag FLAGged; this is the honest composition, not a fabricated prim). Exact. | Empirical/Declared |
| `std.spores::vec_is_empty` | fn | `lib/std/spore.myc:208` | `fn vec_is_empty[A](xs: Vec[A]) => Bool` | vec_is_empty: structural emptiness — Exact. | Empirical/Declared |
| `std.spores::strength_rank` | fn | `lib/std/spore.myc:213` | `fn strength_rank(g: Guarantee) => Binary{8}` | — | Empirical/Declared |
| `std.spores::guarantee_eq` | fn | `lib/std/spore.myc:222` | `fn guarantee_eq(a: Guarantee, b: Guarantee) => Bool` | guarantee_eq: lattice-tag equality via the injective rank map — Exact. | Empirical/Declared |
| `std.spores::SporeErr` | type | `lib/std/spore.myc:229` | `type SporeErr = HashMismatch(Bytes, Bytes) \| PublishErr(Bytes) \| IoErr(Bytes)` | — | Empirical/Declared |
| `std.spores::SporeErr::HashMismatch` | ctor | `lib/std/spore.myc:229` | `HashMismatch(Bytes, Bytes)` | — | Empirical/Declared |
| `std.spores::SporeErr::IoErr` | ctor | `lib/std/spore.myc:229` | `IoErr(Bytes)` | — | Empirical/Declared |
| `std.spores::SporeErr::PublishErr` | ctor | `lib/std/spore.myc:229` | `PublishErr(Bytes)` | — | Empirical/Declared |
| `std.spores::spore_err_display` | fn | `lib/std/spore.myc:233` | `fn spore_err_display(e: SporeErr) => Bytes` | spore_err_display: the oracle's `Display for SporeErr`, byte-for-byte (differentially checked against `mycelium-std-spore` in std_spore.rs). Exact (deterministic concat of Exact prims). | Empirical/Declared |
| `std.spores::SporeUnit` | type | `lib/std/spore.myc:249` | `type SporeUnit = Spore(Bytes, Vec[Bytes], Option[ReconManifest])` | — | Empirical/Declared |
| `std.spores::SporeUnit::Spore` | ctor | `lib/std/spore.myc:249` | `Spore(Bytes, Vec[Bytes], Option[ReconManifest])` | — | Empirical/Declared |
| `std.spores::spore_carry` | fn | `lib/std/spore.myc:254` | `fn spore_carry(id: Bytes, surface: Vec[Bytes], m: Option[ReconManifest]) => SporeUnit` | spore_carry: wrap a kernel-minted identity + surface + manifest as a SporeUnit. This is the carry seam for FLAG-spore-2/-3 (the Rust constructors from_manifest/from_value run the M-368 pipeline; this handle only ever CARRIES their output). Exact; total. | Empirical/Declared |
| `std.spores::identity` | fn | `lib/std/spore.myc:258` | `fn identity(s: SporeUnit) => Bytes` | identity: the spore's canonical content-addressed identity (ADR-003). Exact; total. | Empirical/Declared |
| `std.spores::spore_surface` | fn | `lib/std/spore.myc:262` | `fn spore_surface(s: SporeUnit) => Vec[Bytes]` | spore_surface: the germination surface (export list). Exact; total. | Empirical/Declared |
| `std.spores::manifest_of` | fn | `lib/std/spore.myc:267` | `fn manifest_of(s: SporeUnit) => Option[ReconManifest]` | manifest_of: the reconstruction manifest, if carried — None is honest absence, never a fabricated empty manifest (C1/G2). Exact. | Empirical/Declared |
| `std.spores::verify_identity` | fn | `lib/std/spore.myc:274` | `fn verify_identity(declared: Bytes, recomputed: Bytes) => Result[Unit, SporeErr]` | verify_identity: the COMPARISON half of verify (FLAG-spore-2 carries the recomputation half to the kernel): equal → Ok(Unit); divergent → the explicit, named HashMismatch(declared, recomputed) — never a silent accept (C1/G2). `expected` is the declared identity, `found` the recomputed one (the oracle's field convention). Exact (deterministic comparison). | Empirical/Declared |
| `std.spores::verify` | fn | `lib/std/spore.myc:281` | `fn verify(s: SporeUnit, recomputed: Bytes) => Result[Unit, SporeErr]` | verify: the spore self-check against a kernel-recomputed identity. Exact. | Empirical/Declared |
| `std.spores::ReconMode` | type | `lib/std/spore.myc:286` | `type ReconMode = IndexedRetrieval \| CompositionalReconstruction` | — | Empirical/Declared |
| `std.spores::ReconMode::CompositionalReconstruction` | ctor | `lib/std/spore.myc:286` | `CompositionalReconstruction` | — | Empirical/Declared |
| `std.spores::ReconMode::IndexedRetrieval` | ctor | `lib/std/spore.myc:286` | `IndexedRetrieval` | — | Empirical/Declared |
| `std.spores::DecodeProcedure` | type | `lib/std/spore.myc:289` | `type DecodeProcedure = Cleanup \| Resonator` | The kernel DecodeProcedure — Cleanup (nearest-atom) or Resonator (probabilistic-only, FR-C2). | Empirical/Declared |
| `std.spores::DecodeProcedure::Cleanup` | ctor | `lib/std/spore.myc:289` | `Cleanup` | — | Empirical/Declared |
| `std.spores::DecodeProcedure::Resonator` | ctor | `lib/std/spore.myc:289` | `Resonator` | — | Empirical/Declared |
| `std.spores::Basis` | type | `lib/std/spore.myc:294` | `type Basis = ProvenThm(Bytes) \| EmpiricalFit(Binary{32}, Bytes) \| UserDeclared` | Basis — the kernel BoundBasis: the evidence class behind a bound certificate. ProvenThm carries its citation; EmpiricalFit its trials (Binary{32} domain substitution) + method; UserDeclared is the unvalidated assertion (always flagged). | Empirical/Declared |
| `std.spores::Basis::EmpiricalFit` | ctor | `lib/std/spore.myc:294` | `EmpiricalFit(Binary{32}, Bytes)` | — | Empirical/Declared |
| `std.spores::Basis::ProvenThm` | ctor | `lib/std/spore.myc:294` | `ProvenThm(Bytes)` | — | Empirical/Declared |
| `std.spores::Basis::UserDeclared` | ctor | `lib/std/spore.myc:294` | `UserDeclared` | — | Empirical/Declared |
| `std.spores::basis_strength` | fn | `lib/std/spore.myc:298` | `fn basis_strength(b: Basis) => Guarantee` | basis_strength: the honest strength a basis implies (the basis IS the evidence class — kernel BoundBasis::strength, mirrored exactly; VR-5). Exact. | Empirical/Declared |
| `std.spores::exceeds_empirical_ceiling` | fn | `lib/std/spore.myc:303` | `fn exceeds_empirical_ceiling(b: Basis) => Bool` | exceeds_empirical_ceiling: True iff the basis implies a strength STRONGER than Empirical (rank < 2 — i.e. Exact or Proven): the FR-C2 probabilistic-regrowth ceiling test. Exact. | Empirical/Declared |
| `std.spores::resonator_over_strength` | fn | `lib/std/spore.myc:308` | `fn resonator_over_strength(dec: DecodeProcedure, b: Basis) => Bool` | resonator_over_strength: the FR-C2 violation predicate — a Resonator decode whose basis exceeds the Empirical ceiling. A Cleanup decode has no resonator ceiling. Exact. | Empirical/Declared |
| `std.spores::MalformedManifest` | type | `lib/std/spore.myc:312` | `type MalformedManifest = ResonatorOverStrength \| KernelWf` | MalformedManifest — a refusal from manifest validation, explicitly named (C1/G2/G11). | Empirical/Declared |
| `std.spores::MalformedManifest::KernelWf` | ctor | `lib/std/spore.myc:312` | `KernelWf` | — | Empirical/Declared |
| `std.spores::MalformedManifest::ResonatorOverStrength` | ctor | `lib/std/spore.myc:312` | `ResonatorOverStrength` | — | Empirical/Declared |
| `std.spores::malformed_display` | fn | `lib/std/spore.myc:315` | `fn malformed_display(e: MalformedManifest) => Bytes` | malformed_display: the oracle's `Display for MalformedManifest`, byte-for-byte. Exact. | Empirical/Declared |
| `std.spores::ReconManifest` | type | `lib/std/spore.myc:327` | `type ReconManifest = Manifest(ReconMode, Bytes, Binary{32}, Vec[Bytes], DecodeProcedure, Basis)` | ReconManifest — the RFC-0003 §6 record: mode, model, dim, codebooks (kernel-minted content hashes, carried — FLAG-spore-2), decode procedure, and the bound's Basis (its {ε,δ} scalars are FLAG-spore-4). Construction goes through manifest_new (validates at build time) or manifest_validate (the deserialized-carry-in re-check). | Empirical/Declared |
| `std.spores::ReconManifest::Manifest` | ctor | `lib/std/spore.myc:327` | `Manifest(ReconMode, Bytes, Binary{32}, Vec[Bytes], DecodeProcedure, Basis)` | — | Empirical/Declared |
| `std.spores::manifest_new` | fn | `lib/std/spore.myc:334` | `fn manifest_new(mode: ReconMode, model: Bytes, dim: Binary{32}, codebooks: Vec[Bytes], dec: DecodeProcedure, basis: Basis) => Result[ReconManifest, MalformedManifest]` | manifest_new: build + validate a manifest. Ports the EXPRESSIBLE kernel checks (FLAG-spore-5): model nonempty, dim >= 1, codebooks nonempty, and the FR-C2 ceiling — every kernel refusal maps to Err(KernelWf), exactly as the oracle's new() maps every `WfError` to `MalformedManifest::KernelWf` (the ResonatorOverStrength variant is validate's, below). Exact (a pure predicate; same inputs, same outcome). | Empirical/Declared |
| `std.spores::manifest_validate` | fn | `lib/std/spore.myc:360` | `fn manifest_validate(m: ReconManifest) => Result[ReconManifest, MalformedManifest]` | manifest_validate: the carry-in path (deserialized/kernel-built manifests) — the explicit defense-in-depth FR-C2 re-check (C1/G2: never silently trust a carry-in). Unlike Rust (where the private inner field makes an over-strength ReconInfo unrepresentable), `.myc` constructors are open, so this check is genuinely reachable here. Exact. | Empirical/Declared |
| `std.spores::manifest_mode` | fn | `lib/std/spore.myc:370` | `fn manifest_mode(m: ReconManifest) => ReconMode` | — | Empirical/Declared |
| `std.spores::manifest_model` | fn | `lib/std/spore.myc:373` | `fn manifest_model(m: ReconManifest) => Bytes` | — | Empirical/Declared |
| `std.spores::manifest_dim` | fn | `lib/std/spore.myc:376` | `fn manifest_dim(m: ReconManifest) => Binary{32}` | — | Empirical/Declared |
| `std.spores::manifest_codebooks` | fn | `lib/std/spore.myc:379` | `fn manifest_codebooks(m: ReconManifest) => Vec[Bytes]` | — | Empirical/Declared |
| `std.spores::manifest_decode` | fn | `lib/std/spore.myc:382` | `fn manifest_decode(m: ReconManifest) => DecodeProcedure` | — | Empirical/Declared |
| `std.spores::declared_strength` | fn | `lib/std/spore.myc:388` | `fn declared_strength(m: ReconManifest) => Guarantee` | declared_strength: the strength from the manifest's bound certificate — derived from the basis, never fabricated (VR-5); always weaker-or-equal to Empirical for a Resonator decode (enforced at construction). Exact. | Empirical/Declared |
| `std.spores::RegrowthResult` | type | `lib/std/spore.myc:394` | `type RegrowthResult[T] = Regrown(T, Basis)` | — | Empirical/Declared |
| `std.spores::RegrowthResult::Regrown` | ctor | `lib/std/spore.myc:394` | `Regrown(T, Basis)` | — | Empirical/Declared |
| `std.spores::regrowth_new` | fn | `lib/std/spore.myc:400` | `fn regrowth_new[T](payload: T, basis: Basis) => Result[RegrowthResult[T], MalformedManifest]` | regrowth_new: construct a regrowth result, REFUSING a basis stronger than Empirical (the FR-C2/VR-5 probabilistic-regrowth ceiling) with the explicit ResonatorOverStrength — never a silent accept. This makes an over-strength regrowth unrepresentable through this constructor (the ceiling holds by construction, not by comment). Exact. | Empirical/Declared |
| `std.spores::regrowth_strength` | fn | `lib/std/spore.myc:407` | `fn regrowth_strength[T](r: RegrowthResult[T]) => Guarantee` | regrowth_strength: derived from the basis — never fabricated, never upgraded (VR-5). Exact. | Empirical/Declared |
| `std.spores::is_empirical` | fn | `lib/std/spore.myc:411` | `fn is_empirical[T](r: RegrowthResult[T]) => Bool` | is_empirical: True iff the strength is exactly Empirical (the expected resonator case). Exact. | Empirical/Declared |
| `std.spores::is_declared` | fn | `lib/std/spore.myc:415` | `fn is_declared[T](r: RegrowthResult[T]) => Bool` | is_declared: True iff the strength is Declared (the weakest; user-asserted only). Exact. | Empirical/Declared |
| `std.spores::DeployTarget` | type | `lib/std/spore.myc:421` | `type DeployTarget = InMemory \| Local(Bytes)` | — | Empirical/Declared |
| `std.spores::DeployTarget::InMemory` | ctor | `lib/std/spore.myc:421` | `InMemory` | — | Empirical/Declared |
| `std.spores::DeployTarget::Local` | ctor | `lib/std/spore.myc:421` | `Local(Bytes)` | — | Empirical/Declared |
| `std.spores::DeployVerification` | type | `lib/std/spore.myc:427` | `type DeployVerification = Verification(Bool, Bool)` | Verification(content_hash_canonical, no_opaque_lowering): the EXPLAIN-able record of what a successful deploy checked (VR-4/C4). Both True on every success — germinate refuses first otherwise. The no_opaque_lowering half is Declared strength (structural assertion, MLIR toolchain pending). | Empirical/Declared |
| `std.spores::DeployVerification::Verification` | ctor | `lib/std/spore.myc:427` | `Verification(Bool, Bool)` | — | Empirical/Declared |
| `std.spores::DeployError` | type | `lib/std/spore.myc:432` | `type DeployError = MissingInput \| AmbiguousInput(Vec[Bytes]) \| DeployHashMismatch(Bytes, Bytes) \| OpaqueStepDetected(Bytes)` | DeployError — an explicit deploy refusal, each variant naming its exact condition (G2/G11). `DeployHashMismatch(expected, actual)` renames the oracle's `HashMismatch` (constructor names are nodule-global; SporeErr owns `HashMismatch` — see the substitution note). | Empirical/Declared |
| `std.spores::DeployError::MissingInput` | ctor | `lib/std/spore.myc:433` | `MissingInput` | — | Empirical/Declared |
| `std.spores::DeployError::AmbiguousInput` | ctor | `lib/std/spore.myc:434` | `AmbiguousInput(Vec[Bytes])` | — | Empirical/Declared |
| `std.spores::DeployError::DeployHashMismatch` | ctor | `lib/std/spore.myc:435` | `DeployHashMismatch(Bytes, Bytes)` | — | Empirical/Declared |
| `std.spores::DeployError::OpaqueStepDetected` | ctor | `lib/std/spore.myc:436` | `OpaqueStepDetected(Bytes)` | — | Empirical/Declared |
| `std.spores::DeployResult` | type | `lib/std/spore.myc:440` | `type DeployResult = Deployed(Bytes, DeployVerification) \| Failed(DeployError)` | DeployResult — Deployed(spore_id, verification) on success; Failed(err) is the explicit, never-silent failure carrier (no partial/best-effort variant exists — C1/G2). | Empirical/Declared |
| `std.spores::DeployResult::Deployed` | ctor | `lib/std/spore.myc:440` | `Deployed(Bytes, DeployVerification)` | — | Empirical/Declared |
| `std.spores::DeployResult::Failed` | ctor | `lib/std/spore.myc:440` | `Failed(DeployError)` | — | Empirical/Declared |
| `std.spores::detect_opaque_step` | fn | `lib/std/spore.myc:446` | `fn detect_opaque_step(target: DeployTarget) => Option[Bytes]` | detect_opaque_step: the VR-4 hook — the v0 clean pipeline (pack → hash → deploy) contains no opaque steps by construction for the supported targets, so this returns None for both (the oracle's stub, mirrored exactly). Declared (structural assertion; MLIR toolchain pending — the oracle's own tag, carried). | Empirical/Declared |
| `std.spores::surface_is_ambiguous` | fn | `lib/std/spore.myc:451` | `fn surface_is_ambiguous(xs: Vec[Bytes]) => Bool` | surface_is_ambiguous: more than one exported surface symbol (the v0 ambiguity condition for an InMemory target with no entry-point selector). Exact. | Empirical/Declared |
| `std.spores::germinate_checked` | fn | `lib/std/spore.myc:458` | `fn germinate_checked(s: SporeUnit, recomputed: Bytes, target: DeployTarget) => Result[DeployResult, DeployError]` | germinate_checked: steps 3–5 of the oracle's germinate — hash canonicality (C4/ADR-003; mismatch → the named DeployHashMismatch, both hashes carried; a non-mismatch SporeErr from verify means the spore is not well-formed → MissingInput, the oracle's mapping), then the VR-4 opaque-step check, then success with the fully-checked Verification record. | Empirical/Declared |
| `std.spores::germinate` | fn | `lib/std/spore.myc:482` | `fn germinate(s: SporeUnit, recomputed: Bytes, target: DeployTarget) => Result[DeployResult, DeployError]` | germinate: the ADR-013 native germination entry point (the oracle's check order, mirrored): (1) missing input — a Local target with an empty path refuses before any other work (G2); (2) ambiguous input — an InMemory target over a multi-symbol surface refuses with ALL candidates listed (G11); (3) hash canonicality; (4) no-opaque-lowering; (5) success. The kernel-recomputed identity is an explicit input (FLAG-spore-2). Empirical — the native deploy path is not proven end-to-end (the oracle's tag; MLIR pending; VR-5: never upgraded even though this structural mirror is a pure function). | Empirical/Declared |
| `std.spores::deploy_error_display` | fn | `lib/std/spore.myc:502` | `fn deploy_error_display(e: DeployError) => Option[Bytes]` | deploy_error_display: the oracle's `Display for DeployError` — byte-for-byte for MissingInput / DeployHashMismatch / OpaqueStepDetected (differentially checked); None for AmbiguousInput (FLAG-spore-6: its rendering interpolates the decimal candidate count, which is not expressible — explicit absence, never a fabricated rendering). Exact where Some. | Empirical/Declared |
| `std.spores::explain_deployed` | fn | `lib/std/spore.myc:529` | `fn explain_deployed(spore_id: Bytes, v: DeployVerification) => Bytes` | explain_deployed: the Deployed-arm EXPLAIN of explain_deploy — a total, deterministic function of the deployed id + verification record, byte-for-byte the oracle's rendering (VR-4/SC-3/C3: it always mentions both the content-hash check and the opaque-lowering check — no silent omission). The Failed arm rides deploy_error_display (FLAG-spore-6). Exact. | Empirical/Declared |
| `std.spores::MatrixRow` | type | `lib/std/spore.myc:547` | `type MatrixRow = Row(Bytes, Bytes, Bytes, Bytes, Bool, Bytes)` | — | Empirical/Declared |
| `std.spores::MatrixRow::Row` | ctor | `lib/std/spore.myc:547` | `Row(Bytes, Bytes, Bytes, Bytes, Bool, Bytes)` | — | Empirical/Declared |
| `std.spores::row_op` | fn | `lib/std/spore.myc:550` | `fn row_op(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.spores::row_guarantee` | fn | `lib/std/spore.myc:553` | `fn row_guarantee(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.spores::row_fallibility` | fn | `lib/std/spore.myc:556` | `fn row_fallibility(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.spores::row_effects` | fn | `lib/std/spore.myc:559` | `fn row_effects(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.spores::row_explainable` | fn | `lib/std/spore.myc:562` | `fn row_explainable(r: MatrixRow) => Bool` | — | Empirical/Declared |
| `std.spores::row_never_silent` | fn | `lib/std/spore.myc:565` | `fn row_never_silent(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.spores::row_build` | fn | `lib/std/spore.myc:569` | `fn row_build() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_build_value` | fn | `lib/std/spore.myc:579` | `fn row_build_value() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_identity` | fn | `lib/std/spore.myc:589` | `fn row_identity() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_explain` | fn | `lib/std/spore.myc:599` | `fn row_explain() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_manifest_of` | fn | `lib/std/spore.myc:609` | `fn row_manifest_of() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_validate` | fn | `lib/std/spore.myc:619` | `fn row_validate() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_manifest_hash` | fn | `lib/std/spore.myc:629` | `fn row_manifest_hash() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_mode` | fn | `lib/std/spore.myc:632` | `fn row_mode() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_declared_strength` | fn | `lib/std/spore.myc:642` | `fn row_declared_strength() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_reconstruct` | fn | `lib/std/spore.myc:652` | `fn row_reconstruct() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_deploy` | fn | `lib/std/spore.myc:662` | `fn row_deploy() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_germinate` | fn | `lib/std/spore.myc:672` | `fn row_germinate() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_verify_hash_canonical` | fn | `lib/std/spore.myc:682` | `fn row_verify_hash_canonical() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_no_opaque` | fn | `lib/std/spore.myc:692` | `fn row_no_opaque() => MatrixRow` | — | Empirical/Declared |
| `std.spores::row_explain_deploy` | fn | `lib/std/spore.myc:702` | `fn row_explain_deploy() => MatrixRow` | — | Empirical/Declared |
| `std.spores::matrix` | fn | `lib/std/spore.myc:713` | `fn matrix() => Vec[MatrixRow]` | matrix: the full 15-row table, in the same order as guarantee_matrix.rs::MATRIX. | Empirical/Declared |
| `std.spores::matrix_len` | fn | `lib/std/spore.myc:733` | `fn matrix_len(xs: Vec[MatrixRow]) => Binary{8}` | matrix_len: the row count as Binary{8} (15 rows fits trivially). O(n) spine-walk. Exact. | Empirical/Declared |
| `std.spores::all_never_silent_nonempty` | fn | `lib/std/spore.myc:739` | `fn all_never_silent_nonempty(xs: Vec[MatrixRow]) => Bool` | — | Empirical/Declared |
| `std.spores::all_ops_nonempty` | fn | `lib/std/spore.myc:746` | `fn all_ops_nonempty(xs: Vec[MatrixRow]) => Bool` | every_row_has_nonempty_op_and_guarantee (basic completeness). | Empirical/Declared |
| `std.spores::all_guarantees_nonempty` | fn | `lib/std/spore.myc:749` | `fn all_guarantees_nonempty(xs: Vec[MatrixRow]) => Bool` | — | Empirical/Declared |
| `std.spores::manifest_rows_are_explainable` | fn | `lib/std/spore.myc:757` | `fn manifest_rows_are_explainable() => Bool` | manifest_rows_are_explainable: the 12 selecting/converting rows expose a C3 EXPLAIN artifact (guarantee_matrix.rs::manifest_rows_are_explain_able, read off the typed Bool column). | Empirical/Declared |
| `std.spores::accessor_rows_not_explainable` | fn | `lib/std/spore.myc:773` | `fn accessor_rows_not_explainable() => Bool` | accessor_rows_not_explainable: the pure-read rows (manifest_hash / mode / declared_strength) have no selection/approximation needing EXPLAIN (guarantee_matrix.rs precedent). | Empirical/Declared |
...
| `std.testing::Option` | type | `lib/std/testing.myc:128` | `type Option[Opt] = Some(Opt) \| None` | Never-silent floor (C1/G2) carried over intact: Skipped and Undetermined are first-class variants aggregated DISTINCTLY from Pass ("green" = checked-and-passed, never did-not-check); a missing/mismatched golden baseline is Skipped(NeedsRecord), never an auto-accept; an unavailable differential backend is Skipped(BackendUnavailable), never a silent pass; a generator that produces nothing is Skipped(NeedsRecord). Guarantee tags (VR-5, carried at the SAME strength as the Rust crate's matrix): every harness op is an Exact MECHANISM (a verdict is an exact, deterministic function of the run) and the harness never inflates the subject's tag — a passing for_all backs an Empirical claim about the property, never Proven; there is no op here that upgrades "passed N trials" into a proof. Three-way differential agreement is Empirical (trials, std_testing.rs). ── local mirrors (single-nodule harness — see the substitution notes above) ──────────────────── | Empirical/Declared |
| `std.testing::Option::None` | ctor | `lib/std/testing.myc:128` | `None` | — | Empirical/Declared |
| `std.testing::Option::Some` | ctor | `lib/std/testing.myc:128` | `Some(Opt)` | — | Empirical/Declared |
| `std.testing::Vec` | type | `lib/std/testing.myc:130` | `type Vec[A] = Nil \| Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.testing::Vec::Cons` | ctor | `lib/std/testing.myc:130` | `Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.testing::Vec::Nil` | ctor | `lib/std/testing.myc:130` | `Nil` | — | Empirical/Declared |
| `std.testing::Guarantee` | type | `lib/std/testing.myc:134` | `type Guarantee = GExact \| GProven \| GEmpirical \| GDeclared` | Guarantee — the kernel GuaranteeStrength lattice as value-level data, G-prefixed (reserved-keyword substitution; std.diag/std.recover precedent). | Empirical/Declared |
| `std.testing::Guarantee::GDeclared` | ctor | `lib/std/testing.myc:134` | `GDeclared` | — | Empirical/Declared |
| `std.testing::Guarantee::GEmpirical` | ctor | `lib/std/testing.myc:134` | `GEmpirical` | — | Empirical/Declared |
| `std.testing::Guarantee::GExact` | ctor | `lib/std/testing.myc:134` | `GExact` | — | Empirical/Declared |
| `std.testing::Guarantee::GProven` | ctor | `lib/std/testing.myc:134` | `GProven` | — | Empirical/Declared |
| `std.testing::SkipReason` | type | `lib/std/testing.myc:138` | `type SkipReason = Ignored \| UnmetPrecondition \| NeedsRecord \| BackendUnavailable \| ToolMissing` | — | Empirical/Declared |
| `std.testing::SkipReason::BackendUnavailable` | ctor | `lib/std/testing.myc:138` | `BackendUnavailable` | — | Empirical/Declared |
| `std.testing::SkipReason::Ignored` | ctor | `lib/std/testing.myc:138` | `Ignored` | — | Empirical/Declared |
| `std.testing::SkipReason::NeedsRecord` | ctor | `lib/std/testing.myc:138` | `NeedsRecord` | — | Empirical/Declared |
| `std.testing::SkipReason::ToolMissing` | ctor | `lib/std/testing.myc:138` | `ToolMissing` | — | Empirical/Declared |
| `std.testing::SkipReason::UnmetPrecondition` | ctor | `lib/std/testing.myc:138` | `UnmetPrecondition` | — | Empirical/Declared |
| `std.testing::UndetReason` | type | `lib/std/testing.myc:142` | `type UndetReason = OracleUnavailable \| BudgetExhaustedInconclusive \| NonDeterministicInput` | UndetReason — ran but could not decide; distinct from both Pass and Skipped. A non-decision is never a Pass (C1/G2). | Empirical/Declared |
| `std.testing::UndetReason::BudgetExhaustedInconclusive` | ctor | `lib/std/testing.myc:142` | `BudgetExhaustedInconclusive` | — | Empirical/Declared |
| `std.testing::UndetReason::NonDeterministicInput` | ctor | `lib/std/testing.myc:142` | `NonDeterministicInput` | — | Empirical/Declared |
| `std.testing::UndetReason::OracleUnavailable` | ctor | `lib/std/testing.myc:142` | `OracleUnavailable` | — | Empirical/Declared |
| `std.testing::FailRecord` | type | `lib/std/testing.myc:148` | `type FailRecord[F] = FRec(F, Binary{64}, Binary{8}, Bytes)` | FailRecord — the structured failure record: FRec(payload, seed, trial, context). The payload (generic F) is the REIFIED counterexample/diff value substituting Rust's Debug-formatted description String (FLAG-testing-3 — the EXPLAIN artifact in value form); the seed makes the failure reproducible (RT3); the trial index locates it; the context Bytes names the op. | Empirical/Declared |
| `std.testing::FailRecord::FRec` | ctor | `lib/std/testing.myc:148` | `FRec(F, Binary{64}, Binary{8}, Bytes)` | — | Empirical/Declared |
| `std.testing::Verdict` | type | `lib/std/testing.myc:153` | `type Verdict[V] = Pass \| Fail(FailRecord[V]) \| Skipped(SkipReason) \| Undetermined(UndetReason)` | Verdict — the outcome sum. Skipped and Undetermined are FIRST-CLASS variants, not flavours of Pass (the honesty crux): the aggregator below keeps their counts distinct, so "green" can never silently include "did not actually check". | Empirical/Declared |
| `std.testing::Verdict::Fail` | ctor | `lib/std/testing.myc:153` | `Fail(FailRecord[V])` | — | Empirical/Declared |
| `std.testing::Verdict::Pass` | ctor | `lib/std/testing.myc:153` | `Pass` | — | Empirical/Declared |
| `std.testing::Verdict::Skipped` | ctor | `lib/std/testing.myc:153` | `Skipped(SkipReason)` | — | Empirical/Declared |
| `std.testing::Verdict::Undetermined` | ctor | `lib/std/testing.myc:153` | `Undetermined(UndetReason)` | — | Empirical/Declared |
| `std.testing::Summary` | type | `lib/std/testing.myc:158` | `type Summary = Counts(Binary{8}, Binary{8}, Binary{8}, Binary{8})` | Summary — the aggregated outcome: Counts(passed, failed, skipped, undetermined). skipped and undetermined are DISTINCT from passed (C1/G2). Counts are Binary{8} (see the width substitution note: add_u refuses at 256 — never a silent wrap). | Empirical/Declared |
| `std.testing::Summary::Counts` | ctor | `lib/std/testing.myc:158` | `Counts(Binary{8}, Binary{8}, Binary{8}, Binary{8})` | — | Empirical/Declared |
| `std.testing::Budget` | type | `lib/std/testing.myc:163` | `type Budget = Trials(Binary{8})` | — | Empirical/Declared |
| `std.testing::Budget::Trials` | ctor | `lib/std/testing.myc:163` | `Trials(Binary{8})` | — | Empirical/Declared |
| `std.testing::GenStep` | type | `lib/std/testing.myc:168` | `type GenStep[G] = Stepped(Option[G], Binary{64})` | — | Empirical/Declared |
| `std.testing::GenStep::Stepped` | ctor | `lib/std/testing.myc:168` | `Stepped(Option[G], Binary{64})` | — | Empirical/Declared |
| `std.testing::GoldenBaseline` | type | `lib/std/testing.myc:173` | `type GoldenBaseline[S] = GB(Binary{8}, S)` | — | Empirical/Declared |
| `std.testing::GoldenBaseline::GB` | ctor | `lib/std/testing.myc:173` | `GB(Binary{8}, S)` | — | Empirical/Declared |
| `std.testing::GoldenFail` | type | `lib/std/testing.myc:177` | `type GoldenFail[W] = GFail(Binary{8}, W, W)` | GoldenFail — the reified golden mismatch: GFail(name_code, expected, produced) — the typed substitute for Rust's rendered "expected …, got …; diff: …" description (FLAG-testing-3). | Empirical/Declared |
| `std.testing::GoldenFail::GFail` | ctor | `lib/std/testing.myc:177` | `GFail(Binary{8}, W, W)` | — | Empirical/Declared |
| `std.testing::DiffFail` | type | `lib/std/testing.myc:181` | `type DiffFail[D] = DFail(D, D)` | DiffFail — the reified differential disagreement: DFail(lhs, rhs) — both outputs, inspectable (C3); the input description rides in the FailRecord context. | Empirical/Declared |
| `std.testing::DiffFail::DFail` | ctor | `lib/std/testing.myc:181` | `DFail(D, D)` | — | Empirical/Declared |
| `std.testing::CertMode` | type | `lib/std/testing.myc:185` | `type CertMode = Fast \| Balanced \| Certified` | — | Empirical/Declared |
| `std.testing::CertMode::Balanced` | ctor | `lib/std/testing.myc:185` | `Balanced` | — | Empirical/Declared |
| `std.testing::CertMode::Certified` | ctor | `lib/std/testing.myc:185` | `Certified` | — | Empirical/Declared |
| `std.testing::CertMode::Fast` | ctor | `lib/std/testing.myc:185` | `Fast` | — | Empirical/Declared |
| `std.testing::CertScope` | type | `lib/std/testing.myc:189` | `type CertScope = ScGlobal \| ScPhylum \| ScNodule` | CertScope — the @certification declaration scopes (mycelium_proj::CertScope), least-specific first. | Empirical/Declared |
| `std.testing::CertScope::ScGlobal` | ctor | `lib/std/testing.myc:189` | `ScGlobal` | — | Empirical/Declared |
| `std.testing::CertScope::ScNodule` | ctor | `lib/std/testing.myc:189` | `ScNodule` | — | Empirical/Declared |
| `std.testing::CertScope::ScPhylum` | ctor | `lib/std/testing.myc:189` | `ScPhylum` | — | Empirical/Declared |
| `std.testing::CertDecl` | type | `lib/std/testing.myc:192` | `type CertDecl = Decl(CertScope, CertMode)` | CertDecl — one @certification declaration: Decl(scope, mode). | Empirical/Declared |
| `std.testing::CertDecl::Decl` | ctor | `lib/std/testing.myc:192` | `Decl(CertScope, CertMode)` | — | Empirical/Declared |
| `std.testing::ResolvedMode` | type | `lib/std/testing.myc:196` | `type ResolvedMode = RMode(CertMode, Option[CertScope])` | ResolvedMode — the resolution EXPLAIN: RMode(mode, source). source None = the project default (Fast) or a granular override (above the scope lattice), exactly as in mycelium_proj. | Empirical/Declared |
| `std.testing::ResolvedMode::RMode` | ctor | `lib/std/testing.myc:196` | `RMode(CertMode, Option[CertScope])` | — | Empirical/Declared |
| `std.testing::ModeScope` | type | `lib/std/testing.myc:200` | `type ModeScope = MScope(Bool, Bool, Bool)` | ModeScope — the typed per-mode predicate set: MScope(fast_in, balanced_in, certified_in) in CertMode::ALL order. Guarantee: Declared — a scope is a declaration, not a checked theorem. | Empirical/Declared |
| `std.testing::ModeScope::MScope` | ctor | `lib/std/testing.myc:200` | `MScope(Bool, Bool, Bool)` | — | Empirical/Declared |
| `std.testing::ModeTestConfig` | type | `lib/std/testing.myc:204` | `type ModeTestConfig = MTC(Vec[CertDecl], Option[CertMode])` | ModeTestConfig — MTC(decls, granular): the configurable per-test scope; the granular override wins over all scope tiers (above the lattice). | Empirical/Declared |
| `std.testing::ModeTestConfig::MTC` | ctor | `lib/std/testing.myc:204` | `MTC(Vec[CertDecl], Option[CertMode])` | — | Empirical/Declared |
| `std.testing::ModeVisit` | type | `lib/std/testing.myc:207` | `type ModeVisit = MVisit(Vec[CertMode], Vec[CertMode])` | ModeVisit — MVisit(visited, skipped): the never-silent audit of which tiers ran. | Empirical/Declared |
| `std.testing::ModeVisit::MVisit` | ctor | `lib/std/testing.myc:207` | `MVisit(Vec[CertMode], Vec[CertMode])` | — | Empirical/Declared |
| `std.testing::ModeViolation` | type | `lib/std/testing.myc:212` | `type ModeViolation = NegativeFires(CertMode) \| PositiveAbsent(CertMode)` | ModeViolation — the reified cross-mode assertion failure (FLAG-testing-6): the property fires where the scope says it should not (NegativeFires) or is absent where it must fire (PositiveAbsent) — exactly the two panic directions of the Rust assert. | Empirical/Declared |
| `std.testing::ModeViolation::NegativeFires` | ctor | `lib/std/testing.myc:212` | `NegativeFires(CertMode)` | — | Empirical/Declared |
| `std.testing::ModeViolation::PositiveAbsent` | ctor | `lib/std/testing.myc:212` | `PositiveAbsent(CertMode)` | — | Empirical/Declared |
| `std.testing::FallibilityClass` | type | `lib/std/testing.myc:217` | `type FallibilityClass = FalTotal \| FalVerdict` | — | Empirical/Declared |
| `std.testing::FallibilityClass::FalTotal` | ctor | `lib/std/testing.myc:217` | `FalTotal` | — | Empirical/Declared |
| `std.testing::FallibilityClass::FalVerdict` | ctor | `lib/std/testing.myc:217` | `FalVerdict` | — | Empirical/Declared |
| `std.testing::EffectsClass` | type | `lib/std/testing.myc:221` | `type EffectsClass = EffNone \| EffIo \| EffPerBackend` | EffectsClass — the typed half of the effects column: pure, io-declaring, or per-backend io (the "only golden and differential declare IO" discipline, typed instead of substring-matched). | Empirical/Declared |
| `std.testing::EffectsClass::EffIo` | ctor | `lib/std/testing.myc:221` | `EffIo` | — | Empirical/Declared |
| `std.testing::EffectsClass::EffNone` | ctor | `lib/std/testing.myc:221` | `EffNone` | — | Empirical/Declared |
| `std.testing::EffectsClass::EffPerBackend` | ctor | `lib/std/testing.myc:221` | `EffPerBackend` | — | Empirical/Declared |
| `std.testing::MatrixRow` | type | `lib/std/testing.myc:226` | `type MatrixRow = Row(Bytes, Guarantee, FallibilityClass, Bytes, EffectsClass, Bytes, Bool)` | MatrixRow — Row(op, tag, fallibility_class, fallibility_prose, effects_class, effects_prose, explainable). Row DATA is Declared (hand-transcribed from guarantee_matrix.rs::MATRIX); the structural checks below are Empirical over it (std_testing.rs three-way + live Rust oracle). | Empirical/Declared |
| `std.testing::MatrixRow::Row` | ctor | `lib/std/testing.myc:226` | `Row(Bytes, Guarantee, FallibilityClass, Bytes, EffectsClass, Bytes, Bool)` | — | Empirical/Declared |
| `std.testing::bin_eq` | fn | `lib/std/testing.myc:230` | `fn bin_eq(a: Binary{8}, b: Binary{8}) => Bool` | — | Empirical/Declared |
| `std.testing::bool_and` | fn | `lib/std/testing.myc:234` | `fn bool_and(a: Bool, b: Bool) => Bool` | bool_and / bool_or / bool_not / bool_eq: local total connectives (std.diag precedent). | Empirical/Declared |
| `std.testing::bool_or` | fn | `lib/std/testing.myc:237` | `fn bool_or(a: Bool, b: Bool) => Bool` | — | Empirical/Declared |
| `std.testing::bool_not` | fn | `lib/std/testing.myc:240` | `fn bool_not(a: Bool) => Bool` | — | Empirical/Declared |
| `std.testing::bool_eq` | fn | `lib/std/testing.myc:243` | `fn bool_eq(a: Bool, b: Bool) => Bool` | — | Empirical/Declared |
| `std.testing::bool_to_bin` | fn | `lib/std/testing.myc:247` | `fn bool_to_bin(a: Bool) => Binary{8}` | bool_to_bin: the 0/1 indicator (for scope_count). | Empirical/Declared |
| `std.testing::zero64` | fn | `lib/std/testing.myc:252` | `fn zero64() => Binary{64}` | zero64: the Binary{64} zero (seed/trial placeholder in golden/differential FailRecords — Rust sets seed: 0, trial: 0 there). | Empirical/Declared |
| `std.testing::vec_len` | fn | `lib/std/testing.myc:257` | `fn vec_len[A](xs: Vec[A]) => Binary{8}` | vec_len: the number of elements as Binary{8}; add_u refuses at 256 on all paths — never a silent wrap (G2; the std.collections `len` contract, Empirical). | Empirical/Declared |
| `std.testing::rng_default_seed` | fn | `lib/std/testing.myc:263` | `fn rng_default_seed() => Binary{64}` | — | Empirical/Declared |
| `std.testing::rng_new` | fn | `lib/std/testing.myc:268` | `fn rng_new(seed: Binary{64}) => Binary{64}` | rng_new: construct the initial state from a fixed seed; 0 is promoted (never a silent degenerate state). Exact. | Empirical/Declared |
| `std.testing::rng_next` | fn | `lib/std/testing.myc:274` | `fn rng_next(state: Binary{64}) => Binary{64}` | rng_next: advance the state and return it — Xorshift64 (x ^= x<<13; x ^= x>>7; x ^= x<<17), bit-exact vs Rust's Rng::next_u64 (whose output IS its new state). Exact: deterministic; the same state always yields the same output (C4/RT3). | Empirical/Declared |
| `std.testing::rng_next_u32` | fn | `lib/std/testing.myc:286` | `fn rng_next_u32(state: Binary{64}) => Binary{32}` | rng_next_u32: the high 32 bits of the next draw, as Binary{32} — mirrors Rust's `(next_u64() >> 32) as u32`. The narrow is in-range by construction (the high 32 bits are zero after the shift), so the never-silent width_cast prim narrows Exactly (its second operand is a width witness whose bits are ignored). The caller threads the advanced state via rng_next. | Empirical/Declared |
| `std.testing::budget_new` | fn | `lib/std/testing.myc:298` | `fn budget_new(trials: Binary{8}) => Option[Budget]` | — | Empirical/Declared |
| `std.testing::budget_trials` | fn | `lib/std/testing.myc:302` | `fn budget_trials(b: Budget) => Binary{8}` | budget_trials: the number of trials this budget permits. Exact. | Empirical/Declared |
| `std.testing::budget_default` | fn | `lib/std/testing.myc:306` | `fn budget_default() => Budget` | budget_default: the default budget (100 trials — Budget::DEFAULT). Exact. | Empirical/Declared |
| `std.testing::budget_min` | fn | `lib/std/testing.myc:310` | `fn budget_min() => Budget` | budget_min: the minimum budget (1 trial — Budget::MIN). Exact. | Empirical/Declared |
| `std.testing::find_failing` | fn | `lib/std/testing.myc:315` | `fn find_failing[T](candidates: Vec[T], prop: T => Bool) => Option[T]` | — | Empirical/Declared |
| `std.testing::shrink_loop` | fn | `lib/std/testing.myc:323` | `fn shrink_loop[T](best: T, candidates: Vec[T], shrink: T => Vec[T], prop: T => Bool, steps: Binary{16}) => T` | shrink_loop: descend through shrink candidates keeping the smallest still-failing value; the step count is bounded (Rust's literal 1000, at Binary{16}) so shrinking itself is bounded (C6). | Empirical/Declared |
| `std.testing::shrink_to_minimal` | fn | `lib/std/testing.myc:340` | `fn shrink_to_minimal[T](initial: T, shrink: T => Vec[T], prop: T => Bool) => T` | shrink_to_minimal: shrink a failing value to a minimal counterexample (spec §3). The result is the REIFIED minimal value (FLAG-testing-3 — Rust renders it into the description String). | Empirical/Declared |
| `std.testing::for_all_loop` | fn | `lib/std/testing.myc:346` | `fn for_all_loop[T](gen: Binary{64} => GenStep[T], shrink: T => Vec[T], prop: T => Bool, state: Binary{64}, seed: Binary{64}, remaining: Binary{8}, trial: Binary{8}, generated_any: Bool) => Verdict[T]` | for_all_loop: the trial spine — generate, check, recurse. generated_any distinguishes "budget done, everything passed" (Pass) from "the generator never produced" (Skipped(NeedsRecord) — C1: a property that could not run is reported, never a silent pass). | Empirical/Declared |
| `std.testing::for_all` | fn | `lib/std/testing.myc:388` | `fn for_all[T](gen: Binary{64} => GenStep[T], shrink: T => Vec[T], seed: Binary{64}, budget: Budget, prop: T => Bool) => Verdict[T]` | for_all: run a property over `budget` generated inputs. Returns the first failure (shrunk to a minimal counterexample, with the reproducing seed + trial index), Skipped(NeedsRecord) if the generator cannot produce, or Pass. The gen/shrink fn pair substitutes the Gen<T> trait instance (FLAG-testing-1). Guarantee: Exact as a MECHANISM (the verdict is an exact function of the run); a passing for_all backs an EMPIRICAL claim about the property — never Proven; no op here performs that upgrade (C2/VR-5 — the crux this module exists to prevent). | Empirical/Declared |
| `std.testing::golden` | fn | `lib/std/testing.myc:403` | `fn golden{N}(baseline: Option[GoldenBaseline[Binary{N}]], name: Binary{8}, produced: Binary{N}) => Verdict[GoldenFail[Binary{N}]]` | — | Empirical/Declared |
| `std.testing::diff_compare` | fn | `lib/std/testing.myc:425` | `fn diff_compare{N}(input_desc: Bytes, lhs_out: Binary{N}, rhs_out: Binary{N}) => Verdict[DiffFail[Binary{N}]]` | — | Empirical/Declared |
| `std.testing::differential` | fn | `lib/std/testing.myc:447` | `fn differential{N}(input_desc: Bytes, lhs_available: Bool, lhs: Unit => Binary{N}, rhs_available: Bool, rhs: Unit => Binary{N}) => Verdict[DiffFail[Binary{N}]]` | differential: require lhs() == rhs() through two thunked backends. An unavailable backend is Skipped(BackendUnavailable) — NEVER a silent pass (C1/G2, the differential honesty crux); the thunks are only forced when both backends are available. Exact mechanism; the two-level §8-Q5 bar's tag/EXPLAIN level is the Rust crate's own FLAG-Q5, carried over unresolved. | Empirical/Declared |
| `std.testing::bump_pass` | fn | `lib/std/testing.myc:462` | `fn bump_pass(s: Summary) => Summary` | — | Empirical/Declared |
| `std.testing::bump_fail` | fn | `lib/std/testing.myc:465` | `fn bump_fail(s: Summary) => Summary` | — | Empirical/Declared |
| `std.testing::bump_skipped` | fn | `lib/std/testing.myc:468` | `fn bump_skipped(s: Summary) => Summary` | — | Empirical/Declared |
| `std.testing::bump_undetermined` | fn | `lib/std/testing.myc:471` | `fn bump_undetermined(s: Summary) => Summary` | — | Empirical/Declared |
| `std.testing::summarize` | fn | `lib/std/testing.myc:477` | `fn summarize[V](vs: Vec[Verdict[V]]) => Summary` | summarize: aggregate verdicts keeping Skipped/Undetermined counts DISTINCT from Pass — "green" can never silently include "did not actually check" (C1/G2). Exact (a total function over verdicts). | Empirical/Declared |
| `std.testing::is_green` | fn | `lib/std/testing.myc:491` | `fn is_green(s: Summary) => Bool` | is_green: True iff there are no failures. Skipped/undetermined counts are SURFACED in the Summary (the caller can inspect them), not hidden and not treated as failures — treating "could not run" as "failed" would itself violate C1 (the Rust doc's exact reasoning). Exact. | Empirical/Declared |
| `std.testing::summary_passed` | fn | `lib/std/testing.myc:495` | `fn summary_passed(s: Summary) => Binary{8}` | summary field accessors (the .myc idiom for Rust's named struct fields). Exact. | Empirical/Declared |
| `std.testing::summary_failed` | fn | `lib/std/testing.myc:498` | `fn summary_failed(s: Summary) => Binary{8}` | — | Empirical/Declared |
| `std.testing::summary_skipped` | fn | `lib/std/testing.myc:501` | `fn summary_skipped(s: Summary) => Binary{8}` | — | Empirical/Declared |
| `std.testing::summary_undetermined` | fn | `lib/std/testing.myc:504` | `fn summary_undetermined(s: Summary) => Binary{8}` | — | Empirical/Declared |
| `std.testing::summary_total` | fn | `lib/std/testing.myc:510` | `fn summary_total(s: Summary) => Binary{8}` | summary_total: the total verdict count. Substitution note: Rust saturates (saturating_add); add_u instead REFUSES past 255 on all paths — a louder, never-silent overflow discipline (G2), documented rather than hidden. | Empirical/Declared |
| `std.testing::mode_code` | fn | `lib/std/testing.myc:516` | `fn mode_code(m: CertMode) => Binary{8}` | — | Empirical/Declared |
| `std.testing::mode_eq` | fn | `lib/std/testing.myc:520` | `fn mode_eq(a: CertMode, b: CertMode) => Bool` | mode_eq: ADT equality via the injective code map. Exact. | Empirical/Declared |
| `std.testing::modes_all` | fn | `lib/std/testing.myc:524` | `fn modes_all() => Vec[CertMode]` | modes_all: CertMode::ALL — every tier, weakest first. Exact. | Empirical/Declared |
| `std.testing::scope_specificity` | fn | `lib/std/testing.myc:528` | `fn scope_specificity(sc: CertScope) => Binary{8}` | scope_specificity: mirrors CertScope::specificity (Global 0 < Phylum 1 < Nodule 2). Exact. | Empirical/Declared |
| `std.testing::scope_all_modes` | fn | `lib/std/testing.myc:535` | `fn scope_all_modes() => ModeScope` | — | Empirical/Declared |
| `std.testing::scope_fast_only` | fn | `lib/std/testing.myc:538` | `fn scope_fast_only() => ModeScope` | — | Empirical/Declared |
| `std.testing::scope_non_fast` | fn | `lib/std/testing.myc:541` | `fn scope_non_fast() => ModeScope` | — | Empirical/Declared |
| `std.testing::scope_certified_only` | fn | `lib/std/testing.myc:544` | `fn scope_certified_only() => ModeScope` | — | Empirical/Declared |
| `std.testing::scope_emit_modes` | fn | `lib/std/testing.myc:548` | `fn scope_emit_modes() => ModeScope` | scope_emit_modes: alias for scope_non_fast (the certificate-emitting modes — RFC-0034 §5). | Empirical/Declared |
| `std.testing::scope_balanced_only` | fn | `lib/std/testing.myc:551` | `fn scope_balanced_only() => ModeScope` | — | Empirical/Declared |
| `std.testing::scope_contains` | fn | `lib/std/testing.myc:555` | `fn scope_contains(s: ModeScope, m: CertMode) => Bool` | scope_contains: True iff the mode is in scope. Exact (total, match-defined). | Empirical/Declared |
| `std.testing::scope_filter` | fn | `lib/std/testing.myc:559` | `fn scope_filter(s: ModeScope, ms: Vec[CertMode], want: Bool) => Vec[CertMode]` | scope_filter: the modes whose membership equals `want`, in CertMode::ALL order. Exact. | Empirical/Declared |
| `std.testing::modes_in_scope` | fn | `lib/std/testing.myc:569` | `fn modes_in_scope(s: ModeScope) => Vec[CertMode]` | modes_in_scope / modes_out_of_scope: the membership split (0..=3 elements each). Exact. | Empirical/Declared |
| `std.testing::modes_out_of_scope` | fn | `lib/std/testing.myc:572` | `fn modes_out_of_scope(s: ModeScope) => Vec[CertMode]` | — | Empirical/Declared |
| `std.testing::scope_count` | fn | `lib/std/testing.myc:576` | `fn scope_count(s: ModeScope) => Binary{8}` | scope_count: the number of modes in scope (0..=3). Exact. | Empirical/Declared |
| `std.testing::scope_is_empty` | fn | `lib/std/testing.myc:580` | `fn scope_is_empty(s: ModeScope) => Bool` | scope_is_empty: an empty scope is surfaced explicitly, not silently ignored (C1/G2). Exact. | Empirical/Declared |
| `std.testing::scope_union` | fn | `lib/std/testing.myc:584` | `fn scope_union(a: ModeScope, b: ModeScope) => ModeScope` | scope_union / scope_intersect: composable derived scopes. Exact. | Empirical/Declared |
| `std.testing::scope_intersect` | fn | `lib/std/testing.myc:592` | `fn scope_intersect(a: ModeScope, b: ModeScope) => ModeScope` | — | Empirical/Declared |
| `std.testing::from_resolved_mode` | fn | `lib/std/testing.myc:602` | `fn from_resolved_mode(r: ResolvedMode) => ModeScope` | from_resolved_mode: the resolver→scope bridge — "the modes at or above the resolved depth": Fast → ALL_MODES, Balanced → NON_FAST, Certified → CERTIFIED_ONLY. Exact. | Empirical/Declared |
| `std.testing::decl_scope` | fn | `lib/std/testing.myc:613` | `fn decl_scope(d: CertDecl) => CertScope` | — | Empirical/Declared |
| `std.testing::decl_mode` | fn | `lib/std/testing.myc:616` | `fn decl_mode(d: CertDecl) => CertMode` | — | Empirical/Declared |
| `std.testing::decl_beats` | fn | `lib/std/testing.myc:622` | `fn decl_beats(new_d: CertDecl, cur: CertDecl) => Bool` | decl_beats: True iff the NEW declaration's specificity is >= the current winner's — the LAST maximal element wins, mirroring Iterator::max_by_key's documented tie behaviour (the resolve_mode fold this port reimplements — FLAG-testing-7). Exact. | Empirical/Declared |
| `std.testing::resolve_fold` | fn | `lib/std/testing.myc:629` | `fn resolve_fold(decls: Vec[CertDecl], acc: Option[CertDecl]) => ResolvedMode` | resolve_fold: the left-to-right max-specificity fold. Exact. | Empirical/Declared |
| `std.testing::resolve_decls` | fn | `lib/std/testing.myc:648` | `fn resolve_decls(decls: Vec[CertDecl]) => ResolvedMode` | resolve_decls: the reimplemented most-specific-wins resolution (mycelium_proj::resolve_mode's observable contract: highest specificity wins; no declaration → the Fast project default with source None). Pinned against the Rust resolver by the live oracle differential (FLAG-testing-7 — the KC-3 shared-resolver property does not carry over). Exact mechanism. | Empirical/Declared |
| `std.testing::config_new` | fn | `lib/std/testing.myc:652` | `fn config_new(decls: Vec[CertDecl]) => ModeTestConfig` | config_new: a config from @certification scope declarations (no granular override). Exact. | Empirical/Declared |
| `std.testing::config_default` | fn | `lib/std/testing.myc:657` | `fn config_default() => ModeTestConfig` | config_default: no declarations → project default Fast → ALL_MODES scope (widening from fast is always safe — the Rust Default impl). Exact. | Empirical/Declared |
| `std.testing::config_with_granular` | fn | `lib/std/testing.myc:662` | `fn config_with_granular(cfg: ModeTestConfig, m: CertMode) => ModeTestConfig` | config_with_granular: add/replace the granular per-test override — wins over all scope tiers. Exact. | Empirical/Declared |
| `std.testing::config_provenance` | fn | `lib/std/testing.myc:668` | `fn config_provenance(cfg: ModeTestConfig) => ResolvedMode` | config_provenance: the resolved mode + its source scope — the EXPLAIN of the scope decision (never ambient, G2). A granular override reports source None (above the lattice), exactly as in Rust. Exact. | Empirical/Declared |
| `std.testing::config_resolve` | fn | `lib/std/testing.myc:673` | `fn config_resolve(cfg: ModeTestConfig) => ModeScope` | config_resolve: the effective ModeScope (provenance → from_resolved_mode — the canonical shared mapping). Exact. | Empirical/Declared |
| `std.testing::visit_visited` | fn | `lib/std/testing.myc:678` | `fn visit_visited(v: ModeVisit) => Vec[CertMode]` | — | Empirical/Declared |
| `std.testing::visit_skipped` | fn | `lib/std/testing.myc:681` | `fn visit_skipped(v: ModeVisit) => Vec[CertMode]` | — | Empirical/Declared |
| `std.testing::visited_all` | fn | `lib/std/testing.myc:685` | `fn visited_all(v: ModeVisit) => Bool` | visited_all: True iff all three modes ran. Exact. | Empirical/Declared |
| `std.testing::mode_list_eq` | fn | `lib/std/testing.myc:689` | `fn mode_list_eq(xs: Vec[CertMode], ys: Vec[CertMode]) => Bool` | mode_list_eq: element-wise Vec[CertMode] equality. Exact. | Empirical/Declared |
| `std.testing::matches_scope` | fn | `lib/std/testing.myc:699` | `fn matches_scope(v: ModeVisit, s: ModeScope) => Bool` | matches_scope: True iff exactly the in-scope modes were visited (no more, no fewer). Exact. | Empirical/Declared |
| `std.testing::map_modes` | fn | `lib/std/testing.myc:705` | `fn map_modes[B](ms: Vec[CertMode], f: CertMode => B) => Vec[B]` | for_each_mode: visit EVERY mode (weakest → strongest) — the pure-map substitution for Rust's FnMut visitor: the per-mode results are RETURNED in order (the visit evidence; never-silent about what ran). There is no way to skip a mode through this fn (use for_each_mode_in). Exact. | Empirical/Declared |
| `std.testing::for_each_mode` | fn | `lib/std/testing.myc:708` | `fn for_each_mode[B](f: CertMode => B) => Vec[B]` | — | Empirical/Declared |
| `std.testing::visit_apply` | fn | `lib/std/testing.myc:714` | `fn visit_apply[B](s: ModeScope, ms: Vec[CertMode], f: CertMode => B) => Vec[CertMode]` | visit_apply: apply `f` per in-scope mode (result computed then discarded — the std.error `inspect` technique; `.myc` has no effect surface, so this preserves "f runs per visited mode"), collecting the visited modes. | Empirical/Declared |
| `std.testing::for_each_mode_in` | fn | `lib/std/testing.myc:727` | `fn for_each_mode_in[B](s: ModeScope, f: CertMode => B) => ModeVisit` | for_each_mode_in: run `f` for each mode IN scope, returning the never-silent visited/skipped audit (the caller can assert matches_scope and inspect the skips — C1/G2). Exact. | Empirical/Declared |
| `std.testing::scope_check_walk` | fn | `lib/std/testing.myc:733` | `fn scope_check_walk(s: ModeScope, pred: CertMode => Bool, ms: Vec[CertMode], neg_only: Bool) => Option[ModeViolation]` | — | Empirical/Declared |
| `std.testing::mode_scope_violation` | fn | `lib/std/testing.myc:759` | `fn mode_scope_violation(s: ModeScope, pred: CertMode => Bool) => Option[ModeViolation]` | mode_scope_violation: the decision core of assert_mode_scope — the FIRST violation of either direction (the property fires outside scope, or is absent inside it), reified; None = the assertion holds. The panicking wrapper awaits a host refusal primitive (FLAG-testing-6). Exact. | Empirical/Declared |
| `std.testing::mode_negative_violation` | fn | `lib/std/testing.myc:764` | `fn mode_negative_violation(s: ModeScope, pred: CertMode => Bool) => Option[ModeViolation]` | mode_negative_violation: the negative-only half — the property must be ABSENT outside scope (the "invariant fires where it doesn't apply" check). Exact. | Empirical/Declared |
| `std.testing::guarantee_code` | fn | `lib/std/testing.myc:769` | `fn guarantee_code(g: Guarantee) => Binary{8}` | — | Empirical/Declared |
| `std.testing::guarantee_eq` | fn | `lib/std/testing.myc:777` | `fn guarantee_eq(a: Guarantee, b: Guarantee) => Bool` | — | Empirical/Declared |
| `std.testing::row_op` | fn | `lib/std/testing.myc:781` | `fn row_op(r: MatrixRow) => Bytes` | row accessors. Exact. | Empirical/Declared |
| `std.testing::row_tag` | fn | `lib/std/testing.myc:784` | `fn row_tag(r: MatrixRow) => Guarantee` | — | Empirical/Declared |
| `std.testing::row_fallibility_class` | fn | `lib/std/testing.myc:787` | `fn row_fallibility_class(r: MatrixRow) => FallibilityClass` | — | Empirical/Declared |
| `std.testing::row_fallibility` | fn | `lib/std/testing.myc:790` | `fn row_fallibility(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.testing::row_effects_class` | fn | `lib/std/testing.myc:793` | `fn row_effects_class(r: MatrixRow) => EffectsClass` | — | Empirical/Declared |
| `std.testing::row_effects` | fn | `lib/std/testing.myc:796` | `fn row_effects(r: MatrixRow) => Bytes` | — | Empirical/Declared |
| `std.testing::row_explainable` | fn | `lib/std/testing.myc:799` | `fn row_explainable(r: MatrixRow) => Bool` | — | Empirical/Declared |
| `std.testing::is_fal_total` | fn | `lib/std/testing.myc:804` | `fn is_fal_total(f: FallibilityClass) => Bool` | typed column predicates (the ADT analogs of the Rust tests' substring assertions — FLAG-recover precedent: comparisons over typed vocabulary, never over strings). Exact. | Empirical/Declared |
| `std.testing::declares_io` | fn | `lib/std/testing.myc:807` | `fn declares_io(e: EffectsClass) => Bool` | — | Empirical/Declared |
| `std.testing::row_for_all` | fn | `lib/std/testing.myc:812` | `fn row_for_all() => MatrixRow` | — | Empirical/Declared |
| `std.testing::row_golden` | fn | `lib/std/testing.myc:823` | `fn row_golden() => MatrixRow` | — | Empirical/Declared |
| `std.testing::row_differential` | fn | `lib/std/testing.myc:834` | `fn row_differential() => MatrixRow` | — | Empirical/Declared |
| `std.testing::row_summarize` | fn | `lib/std/testing.myc:845` | `fn row_summarize() => MatrixRow` | — | Empirical/Declared |
| `std.testing::row_is_green` | fn | `lib/std/testing.myc:848` | `fn row_is_green() => MatrixRow` | — | Empirical/Declared |
| `std.testing::matrix` | fn | `lib/std/testing.myc:852` | `fn matrix() => Vec[MatrixRow]` | matrix: the full 5-row table, in guarantee_matrix.rs::MATRIX order. | Empirical/Declared |
| `std.testing::all_rows_exact` | fn | `lib/std/testing.myc:858` | `fn all_rows_exact(xs: Vec[MatrixRow]) => Bool` | — | Empirical/Declared |
| `std.testing::all_rows_explainable` | fn | `lib/std/testing.myc:865` | `fn all_rows_explainable(xs: Vec[MatrixRow]) => Bool` | all_rows_explainable: every op surfaces an EXPLAIN artifact (C3/G11/SC-3 — no black boxes). | Empirical/Declared |
| `std.testing::only_golden_and_differential_declare_io` | fn | `lib/std/testing.myc:873` | `fn only_golden_and_differential_declare_io() => Bool` | only_golden_and_differential_declare_io: the spec §4 effects discipline, read off the typed column (no substring matching). | Empirical/Declared |
| `std.testing::aggregator_rows_are_total` | fn | `lib/std/testing.myc:881` | `fn aggregator_rows_are_total() => Bool` | aggregator_rows_are_total: summarize/is_green are total functions over verdicts (spec §4). | Empirical/Declared |
...
| Symbol | Kind | File:Line | Signature | Summary | Tag |
|---|---|---|---|---|---|
| `std.time` | nodule | `lib/std/time.myc:7` | `nodule std.time` | Self-hosted `std.time` port (M-1021, kickoff spw / Wave-0 pilot) — the value-semantic | Empirical/Declared |
| `std.time::TimeErr` | type | `lib/std/time.myc:77` | `type TimeErr = Overflow \| ClockUnavailable \| NonMonotonic` | Guarantee basis (transparency rule / VR-5), grounded in the Rust oracle's guarantee matrix (crates/mycelium-std-time/src/lib.rs §10, RFC-0016 §4.5): - constructors/accessors + the functional-update setters are bit-preserving/value-copying — \*\*Exact\*\*, total. - the Duration comparison surface wraps the `eq`/`lt_s` kernel prims — \*\*Exact\*\* over the finite Binary{128} domain (signed two's-complement ordering, RFC-0033 §4.1.2), total; the instant comparison surface wraps `eq`/`lt` (unsigned) — \*\*Exact\*\* over Binary{64}. - the declared-effect newtype `new`/`into_inner` and the `ManualClock` reads carry a \*\*Declared\*\* type-level contract (the effect is declared by the wrapper's TYPE — structural, not a theorem; matches the oracle's C6). The reads are the module's honesty crux. - the guarantee-matrix ROW DATA is \*\*Declared\*\* (asserted, hand-transcribed — matches the oracle's own `#[cfg(test)]` structural assertions); each row's `tag` field is itself GExact/GDeclared per the oracle. Three-way differential + Rust-oracle agreement are \*\*Empirical\*\* (trials, std_time.rs). ── §1. Error discriminant (C1/G2; payloads FLAGged — see honesty boundary above) ────────────────── Every explicit failure IDENTITY from a `std.time` operation. Discriminant-only port: the Rust oracle's diagnostic payloads are dropped pending a string + signed-int surface (FLAG → enb). Total, finite (Exact identity). | Empirical/Declared |
| `std.time::TimeErr::ClockUnavailable` | ctor | `lib/std/time.myc:77` | `ClockUnavailable` | — | Empirical/Declared |
| `std.time::TimeErr::NonMonotonic` | ctor | `lib/std/time.myc:77` | `NonMonotonic` | — | Empirical/Declared |
| `std.time::TimeErr::Overflow` | ctor | `lib/std/time.myc:77` | `Overflow` | — | Empirical/Declared |
| `std.time::MonoInstant` | type | `lib/std/time.myc:84` | `type MonoInstant = MonoInstant(Binary{64})` | — | Empirical/Declared |
| `std.time::MonoInstant::MonoInstant` | ctor | `lib/std/time.myc:84` | `MonoInstant(Binary{64})` | — | Empirical/Declared |
| `std.time::mono_from_nanos` | fn | `lib/std/time.myc:87` | `fn mono_from_nanos(nanos: Binary{64}) => MonoInstant` | mono_from_nanos: construct from a raw nanosecond tick. Exact, total (bit-preserving). | Empirical/Declared |
| `std.time::mono_as_nanos` | fn | `lib/std/time.myc:91` | `fn mono_as_nanos(self: MonoInstant) => Binary{64}` | mono_as_nanos: the raw nanosecond tick (canonical representation). Exact, total. | Empirical/Declared |
| `std.time::LogicalInstant` | type | `lib/std/time.myc:97` | `type LogicalInstant = LogicalInstant(Binary{64})` | A point on the RFC-0008 LOGICAL clock (a deterministic monotonic tick). Rust: `tick: u64` — genuinely UNSIGNED, so `Binary{64}` is faithful. Reading surface only; advancement is runtime-owned (M-521/M-356), FLAGged in the oracle. | Empirical/Declared |
| `std.time::LogicalInstant::LogicalInstant` | ctor | `lib/std/time.myc:97` | `LogicalInstant(Binary{64})` | — | Empirical/Declared |
| `std.time::logical_from_tick` | fn | `lib/std/time.myc:100` | `fn logical_from_tick(tick: Binary{64}) => LogicalInstant` | logical_from_tick: construct from a logical tick. Exact, total. | Empirical/Declared |
| `std.time::logical_as_tick` | fn | `lib/std/time.myc:104` | `fn logical_as_tick(self: LogicalInstant) => Binary{64}` | logical_as_tick: the raw tick value. Exact, total. | Empirical/Declared |
| `std.time::WallInstant` | type | `lib/std/time.myc:111` | `type WallInstant = WallInstant(Binary{128})` | A point on the WALL-CLOCK (civil/UTC time, an entropy source). Rust: `nanos_since_epoch: i128` — SIGNED (can be negative for pre-1970 dates). Stored here as the `Binary{128}` two's-complement bit pattern: the constructor/accessor are bit-preserving round-trips (Exact) that do NOT interpret the sign, so they are faithful regardless of the signed-arithmetic gap above. | Empirical/Declared |
| `std.time::WallInstant::WallInstant` | ctor | `lib/std/time.myc:111` | `WallInstant(Binary{128})` | — | Empirical/Declared |
| `std.time::wall_from_nanos_since_epoch` | fn | `lib/std/time.myc:115` | `fn wall_from_nanos_since_epoch(nanos: Binary{128}) => WallInstant` | wall_from_nanos_since_epoch: construct from a signed nanosecond count since the Unix epoch (stored as its two's-complement bits). Exact round-trip, total. | Empirical/Declared |
| `std.time::wall_as_nanos_since_epoch` | fn | `lib/std/time.myc:119` | `fn wall_as_nanos_since_epoch(self: WallInstant) => Binary{128}` | wall_as_nanos_since_epoch: the raw signed nanosecond count (its two's-complement bits). Exact, total. | Empirical/Declared |
| `std.time::Duration` | type | `lib/std/time.myc:127` | `type Duration = Duration(Binary{128})` | — | Empirical/Declared |
| `std.time::Duration::Duration` | ctor | `lib/std/time.myc:127` | `Duration(Binary{128})` | — | Empirical/Declared |
| `std.time::dur_from_nanos` | fn | `lib/std/time.myc:130` | `fn dur_from_nanos(nanos: Binary{128}) => Duration` | dur_from_nanos: construct from a signed nanosecond count (stored as two's-complement bits). Exact. | Empirical/Declared |
| `std.time::dur_as_nanos` | fn | `lib/std/time.myc:134` | `fn dur_as_nanos(self: Duration) => Binary{128}` | dur_as_nanos: the raw signed nanosecond count (its two's-complement bits). Exact, total. | Empirical/Declared |
| `std.time::dur_zero` | fn | `lib/std/time.myc:138` | `fn dur_zero() => Duration` | dur_zero: the zero span (Rust `Duration::ZERO`). Exact, total — the all-zero 128-bit pattern. | Empirical/Declared |
| `std.time::Ordering` | type | `lib/std/time.myc:145` | `type Ordering = Lt \| Eq \| Gt` | — | Empirical/Declared |
| `std.time::Ordering::Eq` | ctor | `lib/std/time.myc:145` | `Eq` | — | Empirical/Declared |
| `std.time::Ordering::Gt` | ctor | `lib/std/time.myc:145` | `Gt` | — | Empirical/Declared |
| `std.time::Ordering::Lt` | ctor | `lib/std/time.myc:145` | `Lt` | — | Empirical/Declared |
| `std.time::dur_cmp` | fn | `lib/std/time.myc:150` | `fn dur_cmp(a: Duration, b: Duration) => Ordering` | dur_cmp: SIGNED three-way comparison of two spans (Rust `duration_cmp`, i.e. `i128::cmp`). Exact, total — `eq` is bit-identity and `lt_s` is the signed two's-complement less-than prim (RFC-0033 §4.1.2, uncapped). Eq if equal, Lt if a < b (signed), Gt otherwise. | Empirical/Declared |
| `std.time::dur_eq` | fn | `lib/std/time.myc:157` | `fn dur_eq(a: Duration, b: Duration) => Bool` | dur_eq: signed equality of two spans. Exact, total (`eq` is bit-identity, signedness-agnostic). | Empirical/Declared |
| `std.time::dur_lt` | fn | `lib/std/time.myc:161` | `fn dur_lt(a: Duration, b: Duration) => Bool` | dur_lt: SIGNED less-than (`a < b`). Exact, total. | Empirical/Declared |
| `std.time::dur_le` | fn | `lib/std/time.myc:165` | `fn dur_le(a: Duration, b: Duration) => Bool` | dur_le: SIGNED less-than-or-equal — projection of dur_cmp. Exact, total. | Empirical/Declared |
| `std.time::dur_is_zero` | fn | `lib/std/time.myc:169` | `fn dur_is_zero(d: Duration) => Bool` | dur_is_zero: whether this span is zero (Rust `Duration::is_zero`). Exact, total. | Empirical/Declared |
| `std.time::dur_is_negative` | fn | `lib/std/time.myc:174` | `fn dur_is_negative(d: Duration) => Bool` | dur_is_negative: whether this span is negative (Rust `Duration::is_negative`, i.e. `nanos < 0`). Exact, total — `lt_s` against zero is the faithful signed test. | Empirical/Declared |
| `std.time::mono_cmp` | fn | `lib/std/time.myc:180` | `fn mono_cmp(a: MonoInstant, b: MonoInstant) => Ordering` | — | Empirical/Declared |
| `std.time::logical_cmp` | fn | `lib/std/time.myc:188` | `fn logical_cmp(a: LogicalInstant, b: LogicalInstant) => Ordering` | logical_cmp: UNSIGNED three-way comparison of two LOGICAL instants (Rust: derived `Ord` on `u64`). Exact, total. | Empirical/Declared |
| `std.time::wall_cmp` | fn | `lib/std/time.myc:196` | `fn wall_cmp(a: WallInstant, b: WallInstant) => Ordering` | wall_cmp: SIGNED three-way comparison of two WALL instants (Rust: derived `Ord` on the `i128` nanos-since-epoch). Exact, total — `lt_s` is signed. | Empirical/Declared |
| `std.time::DeclaredTime` | type | `lib/std/time.myc:210` | `type DeclaredTime[T] = DeclaredTime(T)` | — | Empirical/Declared |
| `std.time::DeclaredTime::DeclaredTime` | ctor | `lib/std/time.myc:210` | `DeclaredTime(T)` | — | Empirical/Declared |
| `std.time::declared_time_new` | fn | `lib/std/time.myc:213` | `fn declared_time_new[T](inner: T) => DeclaredTime[T]` | declared_time_new: wrap a value produced by an effectful, time-only read. Declared, total. | Empirical/Declared |
| `std.time::declared_time_into_inner` | fn | `lib/std/time.myc:217` | `fn declared_time_into_inner[T](d: DeclaredTime[T]) => T` | declared_time_into_inner: unwrap to the inner value. Declared, total. | Empirical/Declared |
| `std.time::DeclaredTimeEntropy` | type | `lib/std/time.myc:222` | `type DeclaredTimeEntropy[T] = DeclaredTimeEntropy(T)` | Declared-effect wrapper for a WALL-CLOCK read (effect: `{ time, entropy }`). The typed distinction from `DeclaredTime`: a wall read is an entropy source (RT3). Only the pure `new`/`into_inner` port. | Empirical/Declared |
| `std.time::DeclaredTimeEntropy::DeclaredTimeEntropy` | ctor | `lib/std/time.myc:222` | `DeclaredTimeEntropy(T)` | — | Empirical/Declared |
| `std.time::declared_entropy_new` | fn | `lib/std/time.myc:225` | `fn declared_entropy_new[T](inner: T) => DeclaredTimeEntropy[T]` | declared_entropy_new: wrap a wall-clock read result, naming both `time` and `entropy`. Declared. | Empirical/Declared |
| `std.time::declared_entropy_into_inner` | fn | `lib/std/time.myc:229` | `fn declared_entropy_into_inner[T](d: DeclaredTimeEntropy[T]) => T` | declared_entropy_into_inner: unwrap to the inner value. Declared, total. | Empirical/Declared |
| `std.time::ManualClock` | type | `lib/std/time.myc:239` | `type ManualClock = ManualClock(MonoInstant, WallInstant, LogicalInstant)` | — | Empirical/Declared |
| `std.time::ManualClock::ManualClock` | ctor | `lib/std/time.myc:239` | `ManualClock(MonoInstant, WallInstant, LogicalInstant)` | — | Empirical/Declared |
| `std.time::manual_clock_default` | fn | `lib/std/time.myc:243` | `fn manual_clock_default() => ManualClock` | manual_clock_default: the all-zero clock (Rust `ManualClock::default`; `default` is a reserved word, hand-port workaround naming). Exact, total. | Empirical/Declared |
| `std.time::manual_clock_new` | fn | `lib/std/time.myc:257` | `fn manual_clock_new(m: MonoInstant, w: WallInstant, l: LogicalInstant) => ManualClock` | manual_clock_new: construct a clock from explicit component instants. Exact, total. | Empirical/Declared |
| `std.time::manual_get_mono` | fn | `lib/std/time.myc:261` | `fn manual_get_mono(c: ManualClock) => MonoInstant` | Field projections (Exact, total). | Empirical/Declared |
| `std.time::manual_get_wall` | fn | `lib/std/time.myc:264` | `fn manual_get_wall(c: ManualClock) => WallInstant` | — | Empirical/Declared |
| `std.time::manual_get_logical` | fn | `lib/std/time.myc:267` | `fn manual_get_logical(c: ManualClock) => LogicalInstant` | — | Empirical/Declared |
| `std.time::manual_set_mono` | fn | `lib/std/time.myc:272` | `fn manual_set_mono(c: ManualClock, m: MonoInstant) => ManualClock` | manual_set_mono/set_wall/set_logical: functional-update setters (Rust `&mut self` setters, ported as value-semantic new-clock returns). Exact, total. | Empirical/Declared |
| `std.time::manual_set_wall` | fn | `lib/std/time.myc:275` | `fn manual_set_wall(c: ManualClock, w: WallInstant) => ManualClock` | — | Empirical/Declared |
| `std.time::manual_set_logical` | fn | `lib/std/time.myc:278` | `fn manual_set_logical(c: ManualClock, l: LogicalInstant) => ManualClock` | — | Empirical/Declared |
| `std.time::manual_mono_now` | fn | `lib/std/time.myc:282` | `fn manual_mono_now(c: ManualClock) => DeclaredTime[MonoInstant]` | manual_mono_now: read the MONOTONIC clock. Declares effect `time` (the wrapper type). Declared. | Empirical/Declared |
| `std.time::manual_wall_now` | fn | `lib/std/time.myc:287` | `fn manual_wall_now(c: ManualClock) => DeclaredTimeEntropy[WallInstant]` | manual_wall_now: read the WALL clock. Declares `{ time, entropy }` (the ENTROPY wrapper — the typed distinction that structurally enforces RT2/RT3). Declared. | Empirical/Declared |
| `std.time::manual_logical_now` | fn | `lib/std/time.myc:291` | `fn manual_logical_now(c: ManualClock) => DeclaredTime[LogicalInstant]` | manual_logical_now: read the LOGICAL clock. Declares effect `time` only (deterministic). Declared. | Empirical/Declared |
| `std.time::Guarantee` | type | `lib/std/time.myc:304` | `type Guarantee = GExact \| GProven \| GEmpirical \| GDeclared` | — | Empirical/Declared |
| `std.time::Guarantee::GDeclared` | ctor | `lib/std/time.myc:304` | `GDeclared` | — | Empirical/Declared |
| `std.time::Guarantee::GEmpirical` | ctor | `lib/std/time.myc:304` | `GEmpirical` | — | Empirical/Declared |
| `std.time::Guarantee::GExact` | ctor | `lib/std/time.myc:304` | `GExact` | — | Empirical/Declared |
| `std.time::Guarantee::GProven` | ctor | `lib/std/time.myc:304` | `GProven` | — | Empirical/Declared |
| `std.time::is_exact` | fn | `lib/std/time.myc:307` | `fn is_exact(g: Guarantee) => Bool` | is_exact / is_declared: project a Guarantee tag. Total over the finite domain (Exact). | Empirical/Declared |
| `std.time::is_declared` | fn | `lib/std/time.myc:310` | `fn is_declared(g: Guarantee) => Bool` | — | Empirical/Declared |
| `std.time::GuaranteeRow` | type | `lib/std/time.myc:316` | `type GuaranteeRow = Row(Bytes, Guarantee, Bytes, Bytes, Bool)` | GuaranteeRow: Row(op, tag, fallibility, effects, explainable) — field-for-field mirror of mycelium_std_time::GuaranteeRow. `op`/`fallibility`/`effects` are free-form prose (Bytes); `tag` is the typed lattice ADT; `explainable` is Bool. | Empirical/Declared |
| `std.time::GuaranteeRow::Row` | ctor | `lib/std/time.myc:316` | `Row(Bytes, Guarantee, Bytes, Bytes, Bool)` | — | Empirical/Declared |
| `std.time::row_tag` | fn | `lib/std/time.myc:319` | `fn row_tag(r: GuaranteeRow) => Guarantee` | Field accessors (total, Exact). | Empirical/Declared |
| `std.time::row_effects` | fn | `lib/std/time.myc:322` | `fn row_effects(r: GuaranteeRow) => Bytes` | — | Empirical/Declared |
| `std.time::row_explainable` | fn | `lib/std/time.myc:325` | `fn row_explainable(r: GuaranteeRow) => Bool` | — | Empirical/Declared |
| `std.time::bool_and` | fn | `lib/std/time.myc:329` | `fn bool_and(a: Bool, b: Bool) => Bool` | bool_and: local total Bool combinator (self-containment convention). Total (Exact). | Empirical/Declared |
| `std.time::row_duration_add` | fn | `lib/std/time.myc:334` | `fn row_duration_add() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_duration_sub` | fn | `lib/std/time.myc:337` | `fn row_duration_sub() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_duration_scale` | fn | `lib/std/time.myc:340` | `fn row_duration_scale() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_duration_cmp` | fn | `lib/std/time.myc:343` | `fn row_duration_cmp() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_duration_as_unit` | fn | `lib/std/time.myc:346` | `fn row_duration_as_unit() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_mono_diff` | fn | `lib/std/time.myc:350` | `fn row_mono_diff() => GuaranteeRow` | Instant differences (Exact, same-source). | Empirical/Declared |
| `std.time::row_wall_diff` | fn | `lib/std/time.myc:353` | `fn row_wall_diff() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_logical_diff` | fn | `lib/std/time.myc:356` | `fn row_logical_diff() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_mono_now` | fn | `lib/std/time.myc:361` | `fn row_mono_now() => GuaranteeRow` | Clock reads (Declared — the crux). `mono_now`/`logical_now` declare `time`; `wall_now` declares `{ time, entropy }` — the ONLY entropy-declaring row (the typed distinction). | Empirical/Declared |
| `std.time::row_wall_now` | fn | `lib/std/time.myc:364` | `fn row_wall_now() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::row_logical_now` | fn | `lib/std/time.myc:367` | `fn row_logical_now() => GuaranteeRow` | — | Empirical/Declared |
| `std.time::Vec` | type | `lib/std/time.myc:377` | `type Vec[A] = Nil \| Cons(A, Vec[A])` | Vec (local cons-list, self-contained; the std.core/std.diag convention). | Empirical/Declared |
| `std.time::Vec::Cons` | ctor | `lib/std/time.myc:377` | `Cons(A, Vec[A])` | — | Empirical/Declared |
| `std.time::Vec::Nil` | ctor | `lib/std/time.myc:377` | `Nil` | — | Empirical/Declared |
| `std.time::matrix` | fn | `lib/std/time.myc:380` | `fn matrix() => Vec[GuaranteeRow]` | matrix: the full 11-row table, same order as mycelium_std_time::GUARANTEE_MATRIX. | Empirical/Declared |
| `std.time::matrix_len` | fn | `lib/std/time.myc:397` | `fn matrix_len(xs: Vec[GuaranteeRow]) => Binary{8}` | — | Empirical/Declared |
| `std.time::count_exact` | fn | `lib/std/time.myc:401` | `fn count_exact(xs: Vec[GuaranteeRow]) => Binary{8}` | count_exact: how many rows tag GExact — the "pure-arithmetic rows are Exact" invariant (8 rows). | Empirical/Declared |
| `std.time::count_declared` | fn | `lib/std/time.myc:412` | `fn count_declared(xs: Vec[GuaranteeRow]) => Binary{8}` | count_declared: how many rows tag GDeclared — the "clock reads are Declared" invariant (3 rows, VR-5, the crux — a clock read tagged Exact/Proven/Empirical would itself be an overclaim). | Empirical/Declared |
| `std.time::explainable_count` | fn | `lib/std/time.myc:422` | `fn explainable_count(xs: Vec[GuaranteeRow]) => Binary{8}` | explainable_count: how many rows surface an inspectable EXPLAIN artifact (8 rows). | Empirical/Declared |
| `std.time::entropy_distinction_holds` | fn | `lib/std/time.myc:436` | `fn entropy_distinction_holds() => Bool` | entropy_distinction_holds: the typed distinction — `wall_now`'s effect declaration differs from `mono_now`'s. Rust: `wall_now.effects` ("{ time, entropy }", 16 bytes) != `mono_now.effects` ("time", 4 bytes). Checked here by BYTE-LENGTH inequality (no general bytes_eq prim; a length difference is sufficient to witness the two rows carry distinct effect declarations, and full content of "time" is spot-checked in std_time.rs). True (Exact by composition of `bytes_len`/`eq`). | Empirical/Declared |
| `std.time::crux_holds` | fn | `lib/std/time.myc:446` | `fn crux_holds() => Bool` | crux_holds: the whole honesty crux in one predicate — the wall row is Declared AND declares an effect distinct from the mono row's, while the mono row is also Declared (all three clock reads are ambient, none is dressed up as Exact). Mirrors the spirit of assert_matrix_invariants (Declared over asserted data; Empirical under the std_time.rs differential). | Empirical/Declared |
...
| Item | Reason |
|---|---|
| `std.derive_prelude (line 101)` | unextracted top-level construct `impl` at line 101 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.derive_prelude (line 26)` | unextracted top-level construct `impl` at line 26 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.derive_prelude (line 34)` | unextracted top-level construct `impl` at line 34 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.derive_prelude (line 40)` | unextracted top-level construct `impl` at line 40 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.derive_prelude (line 56)` | unextracted top-level construct `impl` at line 56 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.derive_prelude (line 65)` | unextracted top-level construct `impl` at line 65 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.fmt (line 221)` | unextracted top-level construct `impl` at line 221 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.fmt (line 227)` | unextracted top-level construct `impl` at line 227 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
| `std.fmt (line 234)` | unextracted top-level construct `impl` at line 234 — only nodule/fn/type are indexed today; not silently dropped, just out of scope (no `lib/` nodule uses this DN-26 `Item` form yet) |
