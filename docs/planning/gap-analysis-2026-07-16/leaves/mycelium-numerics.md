# Gap report — `mycelium-numerics`

| Field | Value |
|---|---|
| **Crate** | `mycelium-numerics` |
| **Scope group** | kernel |
...
Verified numerics: error + prob bound kernels and tier-i checker (ADR-010/E2-4). Part of Wave G2 PARTITION (kernel group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-numerics/src/` (~886 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-numerics)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-numerics`.
