# Gap report — `mycelium-select`

| Field | Value |
|---|---|
| **Crate** | `mycelium-select` |
| **Scope group** | kernel |
...
RFC-0005 selection policy language with mandatory EXPLAIN (E2-6). Part of Wave G2 PARTITION (kernel group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-select/src/` (~944 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-select)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-select`.
