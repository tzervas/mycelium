# Gap report — `mycelium-workstack`

| Field | Value |
|---|---|
| **Crate** | `mycelium-workstack` |
| **Scope group** | runtime |
...
Shared RecursionBudget + ProcessArena leaf for interp/L1/AOT (RFC-0041). Part of Wave G2 PARTITION (runtime group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-workstack/src/` (~1271 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-workstack)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-workstack`.
