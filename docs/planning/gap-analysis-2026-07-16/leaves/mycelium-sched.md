# Gap report — `mycelium-sched`

| Field | Value |
|---|---|
| **Crate** | `mycelium-sched` |
| **Scope group** | runtime |
...
Work-stealing Scheduler relocated below interp (M-861/M-864). Part of Wave G2 PARTITION (runtime group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-sched/src/` (~1763 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-sched)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-sched`.
