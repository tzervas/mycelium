# Gap report — `mycelium-std-select`

| Field | Value |
|---|---|
| **Crate** | `mycelium-std-select` |
| **Scope group** | stdlib |
...
Std adapters for representation selection policies. Part of Wave G2 PARTITION (stdlib group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-std-select/src/` (~1111 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | high | `no TODO/FIXME in src grep (mycelium-std-select)` | DN-99 / enb | Spec ratification vs lib/*.myc graduation |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-std-select`.
