# Gap report — `mycelium-std-recover`

| Field | Value |
|---|---|
| **Crate** | `mycelium-std-recover` |
| **Scope group** | stdlib |
...
Recovery / cert-mode surfaces for error paths. Part of Wave G2 PARTITION (stdlib group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-std-recover/src/` (~2453 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | high | `no TODO/FIXME in src grep (mycelium-std-recover)` | DN-99 / enb | Spec ratification vs lib/*.myc graduation |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-std-recover`.
