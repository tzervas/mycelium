# Gap report — `mycelium-std-runtime`

| Field | Value |
|---|---|
| **Crate** | `mycelium-std-runtime` |
| **Scope group** | stdlib |
...
Colony/hypha runtime; re-exports sched + rt-abi. Part of Wave G2 PARTITION (stdlib group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-std-runtime/src/` (~5436 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | high | `no TODO/FIXME in src grep (mycelium-std-runtime)` | DN-99 / enb | Spec ratification vs lib/*.myc graduation |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-std-runtime`.
