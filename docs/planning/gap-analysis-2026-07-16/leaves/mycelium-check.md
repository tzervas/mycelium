# Gap report — `mycelium-check`

| Field | Value |
|---|---|
| **Crate** | `mycelium-check` |
...
myc-check project driver over L1 check_nodule (M-365). Part of Wave G2 PARTITION (toolchain group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-check/src/` (~1438 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| `checked_fraction` gate for emitted lib | block | G1 SYNTHESIS; `just transpile-vet` | M-1000 | Oracle for all `.myc` ports |
| Diagnostic class granularity | low | `lib.rs:12-14` VR-5 | none | Flat CheckError by design |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-check`.
