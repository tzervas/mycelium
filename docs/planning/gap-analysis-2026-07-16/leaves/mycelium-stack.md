# Gap report — `mycelium-stack`

| Field | Value |
|---|---|
| **Crate** | `mycelium-stack` |
| **Scope group** | runtime |
...
Host-stack grow/deep worker for L1 recursive passes (RFC-0041 §4.3). Part of Wave G2 PARTITION (runtime group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-stack/src/` (~471 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-stack)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-stack`.
