# Gap report — `mycelium-std-sys-host`

| Field | Value |
|---|---|
| **Crate** | `mycelium-std-sys-host` |
| **Scope group** | stdlib |
...
Host syscall adapters behind effect gates. Part of Wave G2 PARTITION (stdlib group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-std-sys-host/src/` (~152 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | high | `no TODO/FIXME in src grep (mycelium-std-sys-host)` | DN-99 / enb | Host effect semantics cross std-sys-host |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-std-sys-host`.
