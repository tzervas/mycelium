# Gap report — `mycelium-bench`

| Field | Value |
|---|---|
| **Crate** | `mycelium-bench` |
| **Scope group** | bench |
...
Honest multi-backend benchmark harness (E-BENCH). Part of Wave G2 PARTITION (bench group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-bench/src/` (~4607 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | low | `no TODO/FIXME in src grep (mycelium-bench)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-bench`.
...
| Prior | Now on tip |
|---|---|
| `language-completeness-gap-inventory.md` | Transpile path still early; this crate's Rust bar largely met for bench |
