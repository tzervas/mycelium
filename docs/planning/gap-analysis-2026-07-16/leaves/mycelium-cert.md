...
Swap certificates, certified binary↔ternary swap, unified check() (M-120/M-210). Part of Wave G2 PARTITION (kernel group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-cert/src/` (~1705 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-cert)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-cert`.
