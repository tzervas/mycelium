# Gap report — `mycelium-vsa-decode`

| Field | Value |
|---|---|
| **Crate** | `mycelium-vsa-decode` |
| **Scope group** | kernel |
...
RFC-0010 decode methodology selection; cycle-break extract from vsa (M-971). Part of Wave G2 PARTITION (kernel group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-vsa-decode/src/` (~394 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | med | `no TODO/FIXME in src grep (mycelium-vsa-decode)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-vsa-decode`.
