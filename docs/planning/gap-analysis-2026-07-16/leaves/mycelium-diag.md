...
Diagnostic routing crate (split from lsp for layering). Part of Wave G2 PARTITION (toolchain group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-diag/src/` (~753 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Residual domain work | low | `no TODO/FIXME in src grep (mycelium-diag)` | DN-99 / enb | ADR-045 bounded diffs only |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-diag`.
