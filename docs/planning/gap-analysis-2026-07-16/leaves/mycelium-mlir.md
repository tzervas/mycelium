...
AOT: env-machine, direct LLVM, optional mlir-dialect (M-150/M-301/M-601). Part of Wave G2 PARTITION (aot group). Rust reference surface is primary; transpile impact is mostly indirect unless noted.

## 2. Public surface snapshot
- Crate root `crates/mycelium-mlir/src/` (~28425 LOC Rust, repo count 2026-07-16).
...
|---|---|---|---|---|
| Full calculus vs bit/trit MLIR fragment | high | `lib.rs:6-11`; bench capability LOSS | M-601/M-602 | Direct-LLVM carries richer fragment |
| Toolchain optional skip | med | ADR-019 `mlir-dialect` feature off default | none | Graceful skip, not silent |
...
## 5. Tests / witnesses
- Crate tests under `src/tests/` or `tests/` per house layout; run via `cargo test -p mycelium-mlir`.
