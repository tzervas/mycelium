...
| Rust→Mycelium transpiler PoC + surface-feature backlog | `crates/mycelium-transpile` (syn AST-walk → best-effort `.myc` + never-silent gap report) | reads one Rust crate, emits the expressible fraction and a structured `{file,line,rust_construct,reason,category}` gap report; diffed against the hand-written `.myc` twin. On `mycelium-std-cmp`: **3.6% expressible** against the current surface *without* macro expansion (`Empirical`, a lower bound); **~55% of gaps are macro-generated** → the top lever is transpiler-side macro expansion | `syn`/`quote`/`serde` (scoped to the transpiler crate only, KC-3 — not the kernel/self-hosting surface) | **Acceleration tooling, not a port target** — this is the DN-34 *mechanism* that will do the bulk rewrite, plus the first-class **prioritized surface-feature backlog** (DN-34 §8.3) that grounds E18-1's `needs-design` work with real demand data. Measured spike cost **~0.85–0.95M tokens** (converts the assessment's `Declared` build/PoC rows to `Empirical`, §5a). Still a spike: DN-34 remains Draft; the full transpiler phase is gated on surface maturity | M-873 (DN-34 §8) |
