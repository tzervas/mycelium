...
|---|---|---|---|---|---|
| Cross-nodule runtime link | `crates/mycelium-l1/src/checkty.rs:1086` (`PhylumEnv::link`) | fold every nodule's CHECKED decls into one linked `Env` the existing elab/mono/eval pipeline consumes unchanged; v0 flat-namespace (one decl per simple name; a collision is a never-silent `CheckError`, G2) | cross-nodule three-way plus `tests/phylum_exec.rs` witnesses (DN-101) | AOT parity = M-1024 follow-up; `.myc` runtime mirror N/A until the `.myc` evaluator (M-986/M-987); qualified per-nodule scoping to DISAMBIGUATE a collision = M-982. **2026-07-11 (PR #1427):** `mycelium-cli`'s own duplicate flatten-by-name linker collapsed onto this canonical `link` (DRY, one linker); a future self-hosted `myc` front door should call the same `.myc`-mirror linker for both `check` and `run`, never a second one | M-1024 (DN-101) |
| Expression-position sugar-rule facility, Stage 0+1 (value-parametric `lower`/`derive`) | `crates/mycelium-l1/src/{ast,checkty,elab}.rs` (`LowerDecl::value_params`, `Cx::check_sugar_call`, `elaborate_lower_rule_with_args`) | Stage 0: call-site recognition of `Name(args)` against a registered `lower` rule's value params, arity/type-matched then unconditionally refused (a provably-inert skeleton). Stage 1: a two-pass hygienic expansion — ordinary elaboration of the RHS with value params bound to their bare spelling, then `sugar_expand` freshens every RHS-introduced binder via a global fresh-name counter under a site-qualified namespace and substitutes value-parameter placeholders with the matched argument `Node`s | `src/tests/facility_stage1_hygiene.rs` (7 fixtures, each `alpha_eq`-vs-oracle plus `Interpreter::eval` agreement plus a disable-freshening negative control) (DN-110/DN-110-8.2) | Def-site resolution (C) and affine-on-expanded-L0 (D) unbuilt (Stage 2/3); NOT wired into ordinary program-elaboration dispatch — no ordinary `.myc` program can invoke it yet; a `lower`-rule RHS that is a lambda IIFE cannot elaborate at all (pre-existing, orthogonal); a future self-hosted elaborator must reproduce the global (not per-call-reset) fresh-name discipline to avoid cross-expansion capture | M-1054 (DN-110/DN-110-8.2) |
| `reveal` Increment-3 — certified-mode round-trip | `crates/mycelium-l1/src/reveal.rs` (`certified_roundtrip`) | always compute the L0-term-level identity witness (`reelaborate` + `alpha_eq`); as a secondary best-effort convenience, attempt a genuine surface round-trip only on the narrow `Const`/`Var`/`Let`(-of-those) reparseable fragment, reporting an explicit outcome rather than trusting a render-time flag alone (G2) | `src/tests/reveal_roundtrip_e3.rs` (6 tests: independently-spelled oracle, `eval` differential, disable-freshening mutation self-check) (DN-38 §5, DN-110-8.2) | `App`/`Match`-built terms render `reparseable = true` but real re-check refuses them — the surface-round-trippable fragment is narrower than the render flag alone indicates; a future self-hosted `reveal` must reproduce the L0-witness-first, surface-as-convenience-only design (OQ-H3 option 1/(a)), not trust `reparseable` as sufficient | M-1051 (DN-38 §5) |
