# Appendix — Layer-4/Layer-5 Idiom + Structural Design (source draft for DN-109)

> This is the working draft the L4/L5 analysis produced 2026-07-10; it was reconciled and landed
> as **`docs/notes/DN-109-Idiom-Optimal-Transpilation-And-Structural-Remapping.md`** (Status:
> Draft). Kept here verbatim (only the note-number placeholder and slot-selection text below are
> corrected to reflect the landed number) as the appendix record for
> `docs/planning/zero-hand-port-delta-ledger.md` §4. **DN-109 is the canonical document** — read
> that one for citation; this file is the grounding trail.

---

## Design Note DN-109 (source draft) — Layer-4 Idiom-Optimal Transpilation and Layer-5 Structural Remapping (module to nodule / crate to phylum)

| Field | Value |
|---|---|
| **Note** | DN-109 *(landed slot — the highest landed note on `dev` at reconciliation time was DN-108; DN-109 was confirmed free by grep before filing.)* |
...
## §9 FLAGs (append-only rows the integrating parent applies, not edited by this draft)

- **CHANGELOG.md** — an "Added (design, pending ratification)" row for DN-109, applied by the
  integrator landing this batch.
...
- **Maintainer note:** the original directive referenced "DN-99"; DN-99 was already taken (the
  spw stdlib-port surface-gap register). This design landed as **DN-109**, confirmed free by grep
