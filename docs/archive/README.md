...
| original `CHANGELOG.md` (entries 2026-06-27 and earlier) | `docs/archive/changelog/CHANGELOG-2026-06.md` | 2026-07 | historical — periodic sweep; `CHANGELOG.md` had grown to 7,916 lines of most-recent-first entries with no releases yet cut (ADR-018, versioning not started), so the file was split at the newest `2026-06-27` block boundary — `CHANGELOG.md` keeps `[Unreleased]` + the rolling recent window (2026-06-28 onward), the rest moved byte-for-byte with a pointer back from the front file |
