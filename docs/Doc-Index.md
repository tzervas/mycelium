...
| **RFC-0011 — L0 `Match` & the L1-in-Core-IR Revision** | proposes the named RFC-0001 revision (RFC-0006 §4.4 step 2): fold the L1 data-and-matching core (`Construct` + flat `Match` + content-addressed registry; WF6/WF7/WF8) into frozen L0 as **r3**, staged ahead of an r4 (`Lam/App/Fix`); flat `Match` recommended as the kernel node (Maranget tree stays untrusted); unblocks M-320 leaf-emission + M-310 sync (M-320) | **Draft** (ratification + the RFC-0001 r3 bump are the maintainer's; frozen-L0 not flipped) |
