...
- **KC-1.** If the P0 bundling probe shows *no* core VSA op admits a usefully tight, honestly-statable bound (**R1**'s stated falsifier), VSA leaves the **core** and becomes a clearly-demarcated approximate sublanguage — or the project redirects away from native VSA. **— Checked at the research gate: PASSED.** Proven non-asymptotic bundling bounds exist (Clarkson-Ubaru-Yang 2023; Thomas-Dasgupta-Rosing 2021; **T0.2**), so MAP-I/sparse `bundle` carry honest `Proven` tags. One confirming build remains (the Liquid-Haskell instantiation, RFC-0003 §5).
...
**Status (June 2026): largely complete.** The two research passes resolved P0.3 (coverage gaps closed), delivered the **KC-1 verdict (PASSED)**, and settled the foundational design questions (ADR-010, DN-01, RFCs 0001–0005). **Remaining P0 items:** the P0.1 *formalization* artifact (the Liquid-Haskell `bundle` instantiation, RFC-0003 §5) and the P0.2 LLM-leverage experiment (**KC-2**, still open). P0.4/P0.5/P0.6 are specified in the RFCs and carry into the build.
