...
- **2026-06-21 — Accepted (framework) → Enacted (framework) (M-648 editorial sweep / M-380).** The `LlmCanonical` renderer is landed in `crates/mycelium-lsp/src/project.rs`: total over all 11 L1 node kinds, P1–P6 invariants (meaning-preserving, guarantee-tags-survive, Swap-never-elided, identity-content-addressed) each unit-tested. The projection framework + registry (§4.1/§4.4), dual human/machine architecture (§4.6), and §4.7 supersession mechanism are all implemented. LLM-leverage claim stays `Declared`/open in the isolated non-blocking track (RP-1 / M-381) — this enactment asserts no leverage result (VR-5). Append-only.
