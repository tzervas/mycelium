...
|---|---|
| **RFC** | 0034 |
| **Status** | **Enacted — with code (Rust-first)** (2026-06-24) — advanced from **Enacted (design-driven)** once the §13 conformance suite landed green (M-794), **pending maintainer ratification of this advance** (see changelog). The prior design-driven enactment (below) is preserved: ratified **Proposed → Accepted → Enacted** by the maintainer (house rule #3, stepped, not skipped); a **doc-and-design-driven** enactment *paired with TDD*, the **corpus amendments applied + governing** (ADR-032; 21 amendments across 13 files via `tools/dn29_apply.py`). **Implementation status: the runtime mode mechanism is now landed Rust-first** — `CertMode` + the never-silent mode tag (M-786), mode-gated provenance tagging/`gate_result` (M-787), mode-gated swap-cert emission+checking (M-788), the compile/runtime spore phase split (M-789), `@certification` resolution/scoping (M-790), the `wrapping` Axis-B opt-out (M-791), EXPLAIN-of-mode + generation≠consumption (M-792), the memory-safe-by-default trusted base (M-793) — and the **RFC-0034 §13 conformance clauses (a)–(f) are verified end-to-end, mode-parametrically over `fast`/`balanced`/`certified` with the cross-mode negatives** (M-794: `crates/mycelium-cert/tests/conformance.rs`). Runtime-behaviour claims that the suite verifies are now `Proven`/`Empirical` **at the strength the code establishes** (e.g. memory-safety `Proven`-by-`forbid(unsafe_code)`; the floors checked by the suite); anything not yet wired stays `Declared` (VR-5/G2). **Residual (not yet code):** the developer-facing native scoped mode-parametric testing toolkit (M-796) and per-op/per-knob granularity (§14) remain deferred. |
...
> (Rust-first)**, *pending maintainer ratification of that advance* (append-only — the prior
...
> the developer-facing scoped testing toolkit (M-796) and per-op/per-knob granularity (§14) remain
