...
- **2026-06-27 — §4.1 type-param bracket superseded by RFC-0037; §4.5 `FieldSpec` form now designed by ADR-033 (append-only, status unchanged).** Two lang-design-wave pointers, neither a rewrite: (1) the §4.1 **`<…>` type-parameter/argument bracket is superseded** — **RFC-0037** (the grammar deconfliction RFC, Proposed) moves type params/args to **`[…]`**, freeing `<>` for comparison/shift operators (the `<>` triple-load fix); RFC-0019's trait/coherence/monomorphization semantics are unchanged, only the surface bracket moves. (2) The **deferred §4.5 runtime-dictionary `Construct` form** (the abstract-parameter `FieldSpec` trusted-core change this note flagged as "a separate ADR") is now **designed by ADR-033** (`FieldSpec::Fn`, Proposed; KC-3-significant, sign-off-gated; FLAG-1 soundness open). The deferral stands until ADR-033 is ratified + its soundness established (a DN-56 freeze-gate item). No semantics changed here. Append-only.
- **2026-06-23 — Enacted (M-657/M-659/M-662/M-673; maintainer-approved).** The §4.1 grammar surface (single-parameter `trait`; `impl Trait<args> for T`; bounded fn type-params), §4.5 coherence discipline (global uniqueness + orphan rule, phylum-wide as of M-662), and L0 elaboration (monomorphization + dictionary-free static resolution, EXPLAIN-reified in `mono.rs`) are all implemented in `crates/mycelium-l1/src/{checkty.rs,mono.rs,elab.rs}`. **Deferred (normative target, not a ratification gate):** the *literal* §4.5 runtime-dictionary `Construct` form (a `Construct` record of method values) is not yet v0-expressible — the trusted core's `FieldSpec` is `Repr | Data` only, with no function-typed field — and remains the deferred normative target, gated on an abstract-parameter `FieldSpec` trusted-core change (a separate ADR, recorded in the 2026-06-23 M-673 entry below). Multi-parameter traits and associated types are deferred to v2 as decided at ratification. Coherence stays **Declared-with-argument** (not machine-checked) — no result upgraded (VR-5). Append-only.
