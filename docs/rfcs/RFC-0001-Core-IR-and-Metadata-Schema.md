...
>
> **Footnote — L0-floor freeze lifted for the gap-closure window (ADR-045, Accepted 2026-07-10; append-only).** This RFC's Core IR (L0) is elsewhere described as the frozen trusted-base floor (KC-3, ADR-007) — new node kinds need their own RFC. That NOW-horizon freeze, along with the L1 kernel primitive/type set, the L2/L3 surface grammar, and the stdlib lexicon, is **lifted for a bounded, re-freezable window** per **ADR-045**, so the language can close its expressibility gaps early rather than after the DN-88 decomposition fans the stdlib into per-component repos. This document's decision body is **unchanged** — the r0–r5 node grammar and its append-only revision history above stand exactly as recorded; only the *freeze* around further RFC-gated growth is suspended for the window. The **ADR-042 §2.1(b) END-STATE** (full first-party `.myc` self-hosting, kernel included) is untouched, and re-freeze is bound to the **DN-99** residual worklist (ADR-045 §2.4). See **ADR-045** for the full decision record.
