...
| Date | Status | Note |
|---|---|---|
| 2026-06-27 | **Accepted** | **D5 width-generics (M-753) landed + the D1/D2 prim surface now consumed self-hosted (M-718), implemented (Rust-first), pending ratification** (status unchanged — an implementation note, not a transition). The D5 width-generics owned by E11-1/`s10` landed as **M-753** (DN-42 Option A): `Ty::Binary(Width)`/`Ty::Ternary(Width)` with `Width::{Lit,Var}`, positional-by-use `fn f<N>(x: Binary{N})`, `unify` binding width-vars same-paradigm-only (incl. width-var↔width-var pass-through), and mono pinning `N` per call site (undetermined → explicit `Residual`). On that, the **full D1/D2 prim surface** — `cmp.eq`/`cmp.lt` (D1) and the complete `bit.{and,or,xor,not,add,sub}` + `trit.{add,sub,mul,neg}` (D2; `prims.rs` lines 79–92, checked `checkty.rs` 4012–4038) — is now consumed by the **self-hosted width-generic** `lib/std/cmp.myc` + `lib/std/math.myc` (M-718, RFC-0031 §5 D4 Tier-1), executing three-way (L1-eval ≡ L0-interp ≡ AOT; `crates/mycelium-l1/tests/std_{cmp,math,generic_conformance}.rs`) with never-silent width-mismatch and overflow refusals (G2). No new kernel node beyond the ratified D1–D4 set (KC-3). RFC stays **Accepted** (not Enacted). |
| 2026-06-26 | **Accepted** | **Tier-2 implemented (Rust-first), pending ratification** (M-749 + M-750; status unchanged — an implementation note, not a transition; the KC-3/`core 1.0.0` trusted-base growth was maintainer-signed-off per the `kpr` kickoff gate). **D3 `Repr::Seq`** (`{ elem: Box<Repr>, len: u32 }` + `Payload::Seq`) and **D4 `Repr::Bytes`** (+ `Payload::Bytes`) land in `crates/mycelium-core` with recursive/injective content-addressing (append-only tags `0x14`/`0x15`/`0x24`/`0x25`), never-silent indexing/slice/decode (`Option`/`Result`, G2), and the over-allocation cap applied to `Seq.len`. Interp prims `seq.{get,len}` / `bytes.{len,get,slice,concat}` + AOT lowering. **Surface** (maintainer-ratified): `[e1,e2,e3]` → `Seq{T,N}` (filling the previously-deferred list-literal path; homogeneous, never-silent) and `0x…` hex → `Bytes` (even-hex, never-silent), across the L1 lexer/parser/checker/elaborator. Full **three-way differential** (L1-eval ≡ L0-interp ≡ AOT over parsed `.myc`) + never-silent rejects in `crates/mycelium-l1/tests/enablement.rs`. Two trusted-core pr-reviews (T2) gated the merge: a `seq.get` VR-5 guarantee-composition leak was found and fixed before landing. **D5 width-generics (M-753)** remains E11-1/`s10`'s. RFC stays **Accepted** (not Enacted). |
| 2026-06-23 | **Accepted** | **Tier-1 implemented (Rust-first), pending ratification** (M-747 + M-748; status unchanged — an implementation note, not a transition). `cmp.eq`/`cmp.lt` (D1) + the surfaced `bit.and`/`bit.or` and never-silent `bit.add`/`bit.sub` (D2) land in `crates/mycelium-interp/src/prims.rs`, surfaced via `prim_kernel_name`/`prim_sig` + a dedicated width-collapsing checker branch, and declared in the Π table with a new `WidthRel::Collapse`. **D1 realization:** a kernel prim returns a representation value, never a `.myc` data value, so the decision's `Bool` bottoms out as `Binary{1}` (`0b1` = true) at the prim boundary; the `.myc` `Bool` lift is a one-line match in the consumer (demonstrated, not asserted). Three-way differential (L1-eval ≡ L0-interp ≡ AOT) + never-silent refusal coverage in `crates/mycelium-l1/tests/enablement.rs` (the M-752 Tier-1 conformance). Tier-2 (D3 `Repr::Seq` / D4 `Repr::Bytes`, M-749/M-750) + width-generics (D5/M-753) are **not** implemented — KC-3-significant, maintainer-sign-off-gated. RFC stays **Accepted** (not Enacted). |
