...
|---|---|
| **RFC** | 0016 |
| **Status** | **Draft** (2026-06-17 — scope + module taxonomy + the per-op contract presented; ratification is the maintainer's append-only decision, M-501) |
...
To be **traced into `research/`** before ratification (the standing pre-ratification grounding obligation):
the cross-language stdlib comparison (Rust/Python/Go/OCaml module sets) and the "honest stdlib" prior art
(refinement-typed / verified standard libraries), as a Research Record, so the taxonomy choices are
grounded, not asserted (§8 must clear this first).

## 8. Unresolved questions (FLAGGED — resolve before ratification)
