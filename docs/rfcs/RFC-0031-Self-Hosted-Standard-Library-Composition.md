...
> Mycelium, and **ADR-038** (Proposed, 2026-07-01) records that supersession explicitly and fixes
...
| Date | Status | Note |
|---|---|---|
| 2026-07-01 | **Accepted** (unchanged — D1 scope note added) | **D1's compiler-forever-Rust permanence superseded** (append-only; the boundary itself stays operative through Phase I). ADR-036 §2.2's comprehensive-dogfooding track already scoped the full toolchain for progressive Mycelium reimplementation, implicitly superseding D1's "`mycelium-l1` stays Rust forever / self-hosted compiler out of scope" permanence claim; **ADR-038** (Proposed) makes the supersession explicit and fixes its terms — compiler self-hosting is a **deferred, post-public (Phase II), doubly-conditional aspiration** (only if demonstrably better on stability/perf; only after the transpiler is fully polished). No module crosses the D1 boundary by this note; D3 staging unchanged. See the §5 D1 scope note, ADR-036, ADR-038. |
