...
> **Scope note (added 2026-07-01 — append-only; D1's boundary stays operative, its *permanence* is
> superseded).** D1's "stays Rust **forever**" framing — specifically the `mycelium-l1` row's "a
> self-hosted *compiler* is explicitly **out of scope**" — is **superseded as a permanent boundary
> by ADR-036**: ADR-036 §2.2's comprehensive-dogfooding track already scopes the *full toolchain*
> (lexer/parser/checker/elaborator/monomorphizer/codegen) for progressive reimplementation in
> Mycelium, and **ADR-038** (Accepted, 2026-07-01) records that supersession explicitly and fixes
> its terms: **compiler self-hosting is a valid *aspiration*, deferred to post-public Phase II, and
> doubly conditional** — it proceeds only if it demonstrably improves stability and/or performance
> over the Rust frontend, and only after the Rust→Mycelium transpiler is fully polished. Through
> Phase I (the function-first `lang 1.0.0` + public-release track, ADR-038) **this D1 table remains
> the operative engineering rule** — every listed module stays Rust and nothing crosses the boundary
> by this note; D3's no-circularity staging holds until a bootstrap protocol is ratified at Phase-II
> time. Append-only: D1's original text and criterion are preserved above, unrewritten (house rule
> #3). See ADR-036 §2.2 and ADR-038.
...
| Date | Status | Note |
|---|---|---|
| 2026-07-01 | **Accepted** (unchanged — D1 scope note added) | **D1's compiler-forever-Rust permanence superseded** (append-only; the boundary itself stays operative through Phase I). ADR-036 §2.2's comprehensive-dogfooding track already scoped the full toolchain for progressive Mycelium reimplementation, implicitly superseding D1's "`mycelium-l1` stays Rust forever / self-hosted compiler out of scope" permanence claim; **ADR-038** (Accepted, 2026-07-01) makes the supersession explicit and fixes its terms — compiler self-hosting is a **deferred, post-public (Phase II), doubly-conditional aspiration** (only if demonstrably better on stability/perf; only after the transpiler is fully polished). No module crosses the D1 boundary by this note; D3 staging unchanged. The supersession is now **in force**. See the §5 D1 scope note, ADR-036, ADR-038. |
