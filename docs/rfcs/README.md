...
| 0024 | Higher-Order Functions via Static Defunctionalization | **Proposed** (2026-06-23; narrows RFC-0007 §4.4's no-function-values clause for the named-fn case; lowers HOF by static defunctionalization in `mono.rs` — no kernel change, KC-3; impl E7-3 / M-684…688; moves to "implemented, pending ratification" as leaves land — not silently Accepted) | RFC-0007 §4.4 (the deferral it narrows); RFC-0019 §4.4 (mono vehicle); RFC-0001 r4 (`Lam`/`App`, left unused); RFC-0006 LR-2; DN-23; ADR-003/006; KC-3; M-649 (first consumer) |
