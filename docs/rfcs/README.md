...
| 0024 | Higher-Order Functions via Static Defunctionalization | **Proposed** (2026-06-23; narrows RFC-0007 §4.4's no-function-values clause for the named-fn case; lowers HOF by static defunctionalization in `mono.rs` — no kernel change, KC-3; impl E7-3 / M-684…688; moves to "implemented, pending ratification" as leaves land — not silently Accepted) **implemented, pending ratification (M-685…688/M-649)** | RFC-0007 §4.4 (the deferral it narrows); RFC-0019 §4.4 (mono vehicle); RFC-0001 r4 (`Lam`/`App`, left unused); RFC-0006 LR-2; DN-23; ADR-003/006; KC-3; M-649 (first consumer) |
| 0025 | Operator Syntax: Symbolic Infix Sugar & Precedence Table | **Proposed** (2026-06-23; M-705 — adopts DN-23's hybrid: frontend-only infix/prefix sugar desugaring to canonical word functions, Rust-derived precedence table §4.1, no L0/L1 change KC-3; implemented Rust-first in `mycelium-l1`, pending maintainer ratification; angle-bracket ops deferred → M-745) | RFC-0006 §3/§4.3; RFC-0007 §4.1–§4.8; RFC-0024 (word operators as first-class values); DN-23 (recommendation: hybrid word-kernel + optional symbol sugar, frontend-only desugaring, no L0/L1 change); E7-5 (M-705) |
| 0026 | Editor Syntax Highlighting Grammar | **Draft** (2026-06-23; planning stub; binding format/scope-name decision deferred to this RFC; feeds E9-1 / E16-1) | DN-24 (design space + layered-stack recommendation); RFC-0006 (surface language + lexer); `crates/mycelium-l1/src/token.rs` (source of truth for token classes); E9-1 (M-693) |
| 0027 | Memory Management and Reclamation | **Draft** (2026-06-23; planning stub; reclamation model, ownership/lifetime semantics, and EXPLAIN contract for the runtime are open questions; feeds E12-1) | RFC-0008 RT7 (structured runtime lifetimes); ADR-020 (runtime/colony phylum placement); G2/VR-5; E12-1 (M-712) |
...
| 0029 | AOT Optimization, Codegen Maturity, and JIT | **Draft** (2026-06-23; planning stub; optimization passes, full libMLIR lowering path, JIT policy, and BitNet acceleration surface are open questions; feeds E15-1) | RFC-0004 §2/§5/§6; ADR-019 (libMLIR toolchain); ADR-009 (JIT deferred — the deferral this RFC lifts); ADR-006 (EXPLAIN obligation); DN-15 (direct-LLVM decomposition plan); E15-1 (M-725) |
| 0030 | Concrete Surface Grammar & L3 Ratification | **Draft** (2026-06-23; planning stub; committed EBNF reference grammar and ratification of the full L3 surface are open questions pending E11-1; feeds E11-1) | RFC-0006 §3–§4.3/§8 (grammar discipline + open questions Q3/Q8); RFC-0007 §4.1–§4.8; RFC-0020 §4 (L2 desugaring layer); RFC-0025 (operator syntax, to be integrated); E11-1 (M-706) |
| 0028 | FFI and System Interface | **Draft** (2026-06-23; planning stub; capability-based Mycelium↔C/Rust FFI model, `wild`/`@std-sys` execution floor, and syscall binding strategy are open questions; feeds E14-1) | RFC-0006 LR-9; RFC-0016 §8-Q6; ADR-014 (unsafe-floor confinement); DN-14 row 9 (`wild` execution staged); E14-1 (M-720) |
| 0029 | AOT Optimization, Codegen Maturity, and JIT | **Accepted** (2026-06-23; §5 resolved against `crates/mycelium-mlir/` — E6-1 `done`, so JIT/BitNet/MLIR-dialect/three-way mechanisms already landed; §7 sanctions inlining/CSE/DCE as EXPLAIN-able never-silent passes [M-726, the one new mechanism], JIT as a never-silent first-class mode, the BitNet capability gate, and the mutant-witnessed `interp ≡ AOT ≡ JIT` durability gate; feeds E15-1) | RFC-0004 §2/§5/§6; ADR-019 (libMLIR toolchain — Enacted); ADR-009 (hybrid execution — already sanctions interpreter/JIT for dynamic VSA); ADR-006 (EXPLAIN obligation); ADR-014 (JIT confined FFI); DN-15 (direct-LLVM decomposition plan); E15-1 (M-725) |
| 0030 | Concrete Surface Grammar & L3 Ratification | **Draft** (2026-06-23; M-706 partial decision — §4.1 proposes Q8 = ratify `wild`; §4.2 corrects the Q3 framing (RFC-0006 Q3 = LR-6 grading, discharged by RFC-0018; representation is RFC-0012's); M-705 operator grammar integrated into mycelium.ebnf. Draft → Proposed gated on M-707 + M-745 — full L3 ratification awaits the RFC-0020 carve-outs + angle-bracket operators) | RFC-0006 §3–§4.3/§8 (grammar discipline + open questions Q3/Q8); RFC-0007 §4.1–§4.8; RFC-0020 §4 (L2 desugaring layer); RFC-0025 (operator syntax, integrated); E11-1 (M-706) |
| 0031 | Self-Hosted Standard Library Composition | **Draft** (2026-06-23; planning stub; composition model, phylum layout, irreducible Rust vs `.myc` split, and migration order are open questions; feeds E13-1) | RFC-0016 (Core Library — **Enacted**); RFC-0006/RFC-0007 (surface + L1 kernel); RFC-0019 (traits); RFC-0024 (HOF — unblocks combinators); ADR-003/ADR-013; KC-2/KC-3/G2/VR-5; `lib/std/result.myc` (M-649 — first self-hosted nodule); E13-1 (M-714) |
