...
| Date | Status | Note |
|---|---|---|
| 2026-07-02 | **Accepted** (unchanged — progress note) | **Phase-I Gap-A/Gap-B enablers landed on `dev`** (kickoff `enb`, E28-1). Gap B (§4.1.2/§4.1.3 integer surface): the never-silent two's-complement prims **`bin.mul`** (M-887), **`bin.div`/`bin.rem`** (M-888, explicit div-by-zero error), **`bin.shl`/`bin.shr`** (M-889, explicit out-of-range shift-amount), plus the signed op set **M-766** (neg + overflow-detect) and **M-767** (signed div/rem/shift variants) landed in `crates/mycelium-interp/src/prims.rs` with property + conformance (accept + reject) green. Gap A (scalar-float enabler): the route-(ii) float value form landed and **ADR-040 is now Enacted** (M-896…M-900). These are the below-grammar `math`/`numerics` enablers this RFC's 2026-07-01 pull-forward named. **The §7 dogfood-gate discipline is unchanged** — no content-address-identity change is spent (single-rehash-deferred-to-first-value-persistence stands); the V1–V5 kernel implementation stays post-1.0-deferred. Design unchanged; no status flip by this note (guarantee tags at `Empirical`, VR-5). |
| 2026-07-01 | **Accepted** (unchanged — prioritization note) | **Phase-I pull-forward per ADR-038** (Proposed, same day): **M-766/M-767** (binary two's-complement `add`/`sub`/`mul`/`neg` with never-silent overflow; the signedness-split `div`/`cmp`/`shift`/overflow-detect op set — §4.1.2/§4.1.3) **plus the scalar-float `Repr` enabler work** (ADR-038 route (ii): a future float ADR and a DN-39 promotion review) are **pulled forward from the post-1.0-deferred V-wave** into ADR-038's Phase-I functional-usability track — they are the below-grammar enablers for `math`/`numerics` (readiness-§0 gaps). The **§7 dogfood-gate discipline is honored unchanged**: any content-address-identity change still lands as a **single rehash** (the M-780 pattern), coordinated with the deferred ADR-030/031 one-way doors so the rehash happens **once**, and the rehash itself **defers to the first value-persistence feature**. No other V-wave task moves; no task status flips by this note; the V1–V5 deferral otherwise stands. |
