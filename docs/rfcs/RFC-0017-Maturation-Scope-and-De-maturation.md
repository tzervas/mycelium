...
- **2026-06-25 — Decision + scope erratum (post corpus-alignment audit; Status unchanged — Enacted, scoped).** The corpus audit flagged that "Enacted" over-claims the **full** §4 design. **Honest current state:** the §4.2 scope-maturation gate + the §4.3 `thaw` de-maturation marker are **built and tested** in `mycelium-l1` (the M-648 sweep below) — that part is genuinely Enacted; but the §4.1 **top-down `@matured` header/manifest inheritance** and the §4.4 **maturation record** are **currently deferred in code**: `crates/mycelium-proj/src/resolve.rs:136-141` resolves `@matured` **local-only** (manifest-level `[project].matured` not yet enacted, R17-Q1; ancestor-nodule inheritance out of scope in the single-file resolver), and the `// @matured:` header is lexed only as a comment. **Maintainer decision (2026-06-25): IMPLEMENT** the full §4.1 inheritance (top-down header/manifest, locally overridable per §4.1) **and** the §4.4 maturation record — this is **not** a deferral. The work is flagged as a **tracked implementation epic** (forthcoming; *no code is implemented by this note*). Until it lands, read "Enacted" as scoped to **§4.2 gate + §4.3 `thaw`**, with §4.1 inheritance + §4.4 record **decided-to-implement, pending**. Append-only; no normative §4 text rewritten (VR-5/G2).
