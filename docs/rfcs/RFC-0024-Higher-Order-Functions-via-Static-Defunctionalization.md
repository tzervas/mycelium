...
### 4A.11 Addendum — §4A.8's tuple-type prerequisite, ratified (DN-73, M-921, 2026-07-02)

> **Append-only note.** §4A.8/§4A.10(f)/§5 above are historical text, preserved unchanged (house rule
> #3). This subsection is the forward reconciliation DN-73 D4 directs, recording how the "tuple **or**
> currying" disjunction those sections posed was actually resolved by the landed implementation.

§4A.8 framed the multi-argument arrow as gated on **a tuple type — or, equivalently, currying**,
and deferred ratifying which branch of that disjunction to take "separately." The 2026-06-29 serial-
lane closeout landed **both** branches without that formal ratification: **M-822** landed multi-argument
**partial application via currying** — its `landed_basis` records "the RFC-0024 §4A.8 tuple-gated
premise proved unnecessary (currying needs no tuple type)" — and **M-826** landed the **first-class
tuple/product type** `(T, U, …)` as an independent surface feature (not as the multi-arg mechanism).

**DN-73** (`docs/notes/DN-73-Tuple-Type-Decision-Ratification.md`, Accepted 2026-07-02, delegated by the
maintainer to the wave orchestrator) is the formal ratification §4A.10(f) required. It resolves the
disjunction as follows — **Option A, adopted**:

1. **The curried arrow `A => B => C` is the canonical multi-argument function-*value* type** (M-822).
   No tuple-type prerequisite was needed for this — §4A.8's disjunction is resolved in favor of
   currying alone for the multi-arg-value case.
2. **The tuple type `(T, U, …)` is an orthogonal first-class product** (M-826), not the multi-arg
   mechanism — a separate surface feature that happened to land in the same closeout.
3. **`(A, B) => C` is a *distinct* type** — an arrow whose domain is the product (`Ty::Fn` over a
   `BaseType::Tuple`/`Ty::Data("Tuple$2", …)` domain) — with **no implicit interconversion** to/from the
   curried form, in either direction. A mismatch (e.g. passing a curried 2-ary value where a
   tuple-domain arrow is expected, or vice versa) is an explicit, never-silent type error naming both
   types (G2/VR-5) — never a silent auto-curry/uncurry coercion. Verified end-to-end (three-way
   differential + a dedicated distinctness-reject fixture) in
   `crates/mycelium-l1/tests/closures.rs` (M-921).
4. **§4A.10(f)'s "ratified separately" gate is discharged by this addendum.** No further multi-arg/
   partial-application work is blocked on a tuple-type decision — there is none left to make; the
   residual open questions (positional `.0` projection as an expression, the unit type `()`,
   1-tuples, and a generic multi-param fn-as-value) are **explicitly out of scope** here — separate
   future decisions, not silently granted (DN-73 §7/D5).

This addendum makes **no semantics or grammar change** — it ratifies the already-landed, already
three-way-verified (`Empirical`) state as the formal decision; the delta was reconciliation-only
(doc addendum + two stale doc-comment fixes in `crates/mycelium-l1/src/{ast,checkty}.rs` + one
conformance fixture upgrading DN-73 §2.4's `Declared` by-construction claim to `Empirical`).
...
- **Implemented (M-704, 2026-06-29 — Rust-first, pending ratification) — `Residual` *resolved* for:** **closures/lambdas** (environment capture: captureless, single-, multi-capture, and closure-capturing-closure), and a function value that flows **dynamically** (out of a `match`, a data field). The four never-silent `Residual`s (`checkty`/`mono`/`elab`/`eval`) become the §4A lowering: `Expr::Lambda` checks to `Ty::Fn`, mono lowers each escaping closure to a `Fn$A$B` tag-sum + an `apply$A$B` dispatcher over existing L0 (no new kernel node — KC-3). Three-way agreement (`Empirical`) confirmed per shape (`crates/mycelium-l1/tests/closures.rs`).
- **Still `Residual` after M-704 (the honest tuple-gated boundary, §4A.8):** **multi-argument lambdas / partial application** — the v0 surface has **no tuple/product type**, so currying (the §4A.5 multi-arg route) is gated on that **separate surface prerequisite**. A multi-parameter `lambda` is an **explicit, never-silent refusal** at the checker (G2/VR-5), not a silent accept — a correct single-arg implementation with an honest residual for multi-arg, never a fragile full one. (FLAG: the tuple type is the gating decision; not granted here.)
...
> **Addendum (append-only, DN-73/M-921, 2026-07-02):** the "tuple-type prerequisite" gate this section
> and §4A.8 describe is **ratified** — see §4A.11. It resolved as *currying, not a tuple type, is the
> multi-arg mechanism*; the tuple landed as an independent product. This paragraph's historical text is
> unchanged (house rule #3).
...
| 2026-06-29 | **implemented (Rust-first), pending ratification** (Accepted → *pending Enacted on the `main` landing*) | **M-704 — §4A implemented (closures via Reynolds defunctionalization).** `crates/mycelium-l1` now type-checks `Expr::Lambda` to `Ty::Fn` (`checkty::check_lambda`) and `mono.rs` lowers each escaping closure to a per-arrow **tag-sum data type `Fn$A$B`** (one constructor per distinct closure; fields = the captured free variables, first-occurrence order; reuses `L1Value::Data`) + a generated **`apply$A$B(clo, x)` dispatcher** whose body is a `match` over the whole-program-closed constructor set — both over **existing L0 constructs (data + match + call), no new `mycelium-core` node** (KC-3 verified: zero `mycelium-core` diff; node budget unchanged). A lambda at its def site → a constructor application of the captured values (value-snapshot); an escaping fn-value application `f(x)` → `apply$A$B(f, x)`; the §4 static named-fn path is **unchanged** (the §4A.2 hybrid — try §4 first). **Three-way differential (L1-eval ≡ L0-interp ≡ AOT) passes per closure shape** (`crates/mycelium-l1/tests/closures.rs`): captureless, single-/multi-capture, closure-capturing-closure, dynamic-fn-out-of-`match`, dynamic-fn-as-data-field, and a **capturing `map`** combinator (the consuming proof) — plus a mutant-witness distinguishing captured environments and a capture-set α-renaming-invariance unit test (`src/tests/mono.rs`). The capture set + generated ctor + dispatcher are EXPLAIN-recorded (`MonoSelections::closure_specs` / `ClosureSpecialization`; house rule #2). **§5 closures / dynamic-fn-flow residual → resolved.** **Still a never-silent tuple-gated `Residual`** (the honest M-704 boundary, §4A.8): **multi-argument lambdas / partial application** — the v0 surface has no tuple/product type, so currying-via-§4A.5 is gated on that separate surface prerequisite (FLAG — never a silent dependency, G2). **Tags (honest):** the construction is `Declared` (a type-level contract + a structural rewrite); the three-way agreement is `Empirical` (trials) — nothing `Proven` (VR-5). Status held at *implemented (Rust-first), pending ratification* — the `Accepted → Enacted` move is the maintainer's/orchestrator's at the `main` landing (house rule #3 — never skip, never silently Enacted). |
| 2026-07-02 | **Accepted** (§4A.8 addendum — §4A.11 added; status field updated at the top; historical §4A.8/§4A.10(f)/§5/§6 text preserved unchanged) | **DN-73 ratifies §4A.8's tuple-type-prerequisite disjunction (M-921, the kickoff `grm` post-decision follow-through).** The 2026-06-29 closeout had already landed both branches of §4A.8's "tuple **or** currying" disjunction — **M-822** (multi-arg partial application via currying; its `landed_basis` records the tuple-type premise "proved unnecessary") and **M-826** (the first-class tuple/product type, landed as an independent surface feature) — without the formal "ratified separately" gate §4A.10(f) demanded. **DN-73** (Accepted 2026-07-02, delegated by the maintainer to the wave orchestrator) supplies that ratification: **Option A** — the curried arrow `A => B => C` stays canonical for multi-argument function *values* (D1); the tuple type is an orthogonal product, not the multi-arg mechanism; `(A, B) => C` is a **distinct** tuple-domain arrow with **no implicit interconversion** to/from the curried form, in either direction — a mismatch is an explicit never-silent type error naming both types (D2, G2/VR-5); `fn f(a: A, b: B) => C` remains the n-ary declaration form (D3); the residual surface (`.0` projection, unit type, 1-tuples, generic multi-param fn-as-value) stays **explicitly open**, not silently granted (D5). **M-921's delta is reconciliation-only** (no semantics/grammar change, per DN-73 §6): (1) this §4A.11 addendum; (2) two stale doc-comments fixed — `crates/mycelium-l1/src/ast.rs` (`BaseType::Fn`, previously claimed multi-arg `(A,B) -> C` is an unimplemented parser refusal) and `crates/mycelium-l1/src/checkty.rs` (the `resolve_ty`/`BaseType::Fn` arm and the `Expr::Lambda` check arm, both previously claiming a tuple-type prerequisite still gates multi-arg); (3) a new closure-corpus shape (`tuple-domain-arrow-as-value`) proving `(Binary{8}, Binary{8}) => Binary{8}` parses, checks, monomorphizes, and evaluates three-way (L1-eval ≡ L0-interp ≡ AOT), upgrading DN-73 §2.4's by-construction `Declared` claim to `Empirical`; and (4) a dedicated distinctness-reject test (`tuple_domain_arrow_rejects_a_curried_value_naming_both_types`) pinning that a curried 2-ary value does **not** satisfy a tuple-domain-arrow parameter, with the checker's actual error message naming both the curried and tuple-domain types verbatim (`crates/mycelium-l1/tests/closures.rs`). Guarantee tags: the ratification is `Declared` (a surface-contract commitment, as all such decisions are — VR-5); the newly-`Empirical` composition claim is now backed by an executed fixture, not by-construction inference alone. No `mycelium-core` change (KC-3); no new `Ty`/L0 node. |
