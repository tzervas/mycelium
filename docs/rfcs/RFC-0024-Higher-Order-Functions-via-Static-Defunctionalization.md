...
- **Still out of scope (not specified here):** a **still-generic** function passed as a value (needs
  type-argument context to defunctionalize — FLAG `generic-fn-as-arg`, `mono.rs` `resolve_fn_args`);
  and **multi-argument arrows / partial application's runtime form**, which are §4A.8-gated on a
  **tuple type the v0 surface does not have** (a separate surface decision — never a silent dependency).
