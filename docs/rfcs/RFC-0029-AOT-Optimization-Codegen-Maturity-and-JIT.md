...
|---|---|
| **RFC** | 0029 |
| **Status** | **Enacted** (2026-07-01 — M-863 ratification act: E15-1 + E25-1 close with every child `done` and a checked three-way differential (M-858's unified mutant-witnessed harness, PR #851); the path this RFC sanctions is complete and stable per house rule #3 — see the changelog for the evidence chain) · **Accepted** (2026-06-23) · Draft (2026-06-23) |
...
| Date | Status | Note |
|---|---|---|
| 2026-07-01 | **Enacted** | **M-863 ratification act.** E25-1's remaining children close this wave — **M-856b** (Dense/VSA through the MLIR-dialect path), **M-860** (parallel per-function AOT codegen via `mycelium-sched`), and **M-862** (parallel pure-fragment interpreter eval) — leaving **every E15-1 (M-725…M-729) and E25-1 (M-850…M-862) child `done`** with a checked three-way differential. The path this RFC sanctions (optimization-pass discipline, full libMLIR lowering coverage, the JIT mode, the BitNet surface, and the unified `interp ≡ AOT ≡ JIT` durability gate) is now **complete and stable** — the condition this RFC's Posture note (line 20) reserved for `Enacted`. Evidence: M-858 (PR #851, mutant-witnessed, 0-missed) closes the durability gate §7.5 requires; M-856/M-856b close the §7.1 dialect-coverage widening for the full fragment (Construct/Match/Swap/Dense/VSA); M-855 delivers the §7.3 JIT mode for dynamic VSA/HDC (cleanup/resonator loops explicitly, never-silently, deferred — not part of this RFC's 1.0.0 scope); M-728's BitNet capability gate and M-726's transform-log optimization passes were already landed prior waves. `Accepted → Enacted` per house rule #3 (must step through Accepted first — done at r0). This does **not** claim the interpreter is superseded — it stays the trusted-base reference throughout (ADR-007/NFR-7); this RFC governs only the native performance layer. Task: E25-1/M-863. |
| 2026-06-30 | **Accepted** (unchanged — staging-tier close-out note) | **M-855/856/858 landed** this second wave (PRs #848/#850/#851): M-855 dynamic-VSA JIT (cleanup/resonator loops explicitly deferred), M-856 MLIR-dialect catch-up for Construct/Match + Swap (Dense/VSA-dialect split out honestly as new **M-856b**, not silently dropped), M-858 the unified mutant-witnessed three-way differential that closes 5 dialect mutant survivors and earns the codegen claim its checked **Empirical** basis (subsuming M-856's own dialect-witness obligation). A stale bench Swap-capability-loss assertion was also corrected (PR #849, M-852 follow-up). **RFC-0029's Status stays Accepted, NOT Enacted** — Enacted remains reserved for E25-1's full close (M-856b new, plus M-860/862/863 still open) per house rule #3 (Enacted = complete and stable, not a partial landing). Task: E25-1. |
