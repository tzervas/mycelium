...
**Tag justification.** Every row is `Exact` and none is `Proven`/`Empirical`/`Declared`: `fmt` carries **no** accuracy/precision/probability semantics (C2 — an op with no accuracy semantics is simply `Exact`, like `len`). The *substantive* honest claim here is not a numeric bound but the **round-trip invariant**: `from_json(to_json(v))` recovers a value with the **same content-id** as `v` (ADR-003 / RFC-0001 §4.6). That is checked, so the round-trip is `Exact`, and on malformed input `from_json` is an explicit `Err` (C1) — never a best-effort coercion. `display_bounded` stays `Exact` because its result is faithful *to what it claims to render*: an elided rendering does not assert completeness — it carries the `Elided{omitted, marker}` evidence (so it is not a downgrade to `Declared`, it is an honest partial view whose partiality is in the type). No op is `Proven`, so no theorem side-conditions are claimed; downgrading rather than overclaiming (VR-5) is moot because nothing here reaches above `Exact`.
...
- **(Q1) Is `fmt`'s JSON view the *same* projection mechanism as `serialize`'s, or a sibling?** RFC-0016 §4.4 lists "the dual human/machine projection" against **both** `fmt` and `serialize`/`encoding` (M-514). The boundary needs maintainer sign-off: does `fmt.to_json` *delegate* to the `serialize` codec (one canonical JSON, two entry points) or is the display-JSON a distinct, lossier view? Ties to RFC-0016 §8-Q1 (the v0 module set / overlap) and §8-Q3 (ergonomics vs the contract). Disposition: FLAGGED; proposed default is delegation to one canonical JSON so the round-trip property is shared, not duplicated.
