...
| Field | Value |
|---|---|
| **Status** | **Implemented (Rust-first), pending ratification** (2026-06-21 — code landed as `mycelium-std-sys` (M-541, #done, Batch P5-B; 18/18 tests pass, `cargo clippy -D warnings` clean). Maintainer ratification is the orchestrator's append-only decision; this spec documents the Rust-first surface as the ratification basis. Was *Draft (needs-design)* implicitly prior to code landing 2026-06-19.) |
