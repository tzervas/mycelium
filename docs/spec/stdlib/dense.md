...
| `sum` (float DT) | `Proven` (ε ∝ DIM) **[FLAG Q1]** | total | `none` | yes (ε-bound artifact) |
...
| `dot` (float DT) | `Proven` (ε) **[FLAG Q1]** | total | `none` | yes (ε-bound artifact) |
...
- **Float elementwise + `add`/`sub`/`hadamard`/`scale`/`dot`/`sum`.** These route their per-element / accumulation round-off through the **ADR-010 `ErrorBound` (ε) kernel** (affine arithmetic; `Bound = ErrorBound{eps, norm}`, basis `ProvenThm{citation: …}`, RFC-0001 §4.3). They are tagged **`Proven` only where the rounding-error theorem's side-conditions are *checked*** by the numerics checker (Tier-i certificate re-validation, ADR-010 "Trusted base"). The classic statement is the Higham backward-error bound for floating-point summation / inner products (the standard `nγ_n`-style bound). I have **not** reproduced its exact constant or its precise side-conditions here, so the *content* of the citation is FLAGGED (**Q1**); where the checker cannot discharge the instantiation, the op **downgrades to `Empirical`** (an `EmpiricalFit` basis from trials, RFC-0001 M-I3), never staying `Proven` on faith.
...
- **(Q1) The exact Higham float-summation / inner-product bound + its checked side-conditions.** The float `sum`/`dot`/`add`-family rows are tagged `Proven` *on the premise* that the ADR-010 numerics checker discharges the standard backward-error bound's arithmetic instantiation. The **exact constant, norm, and side-conditions** (e.g. the `nγ_n` form, ordering / no-overflow / rounding-mode pre-conditions) are owned by ADR-010 / `std.numerics` (M-512) and are **not reproduced here** — this spec must *cite* them, not restate them. Until M-512 fixes the checked instantiation, treat the `Proven` tag as **provisional**; any op the checker cannot discharge **downgrades to `Empirical`** (M-I3). Disposition: resolve jointly with M-512; do not ratify the `Proven` rows without the cited, checked bound. (Ties to RFC-0016 §8-Q1, the v0 surface.)
