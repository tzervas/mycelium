...
| Field | Value |
|---|---|
| **Status** | **Implemented (Rust-first half) — pending ratification** (2026-06-18; was Draft/needs-design 2026-06-17) — RFC-0016 is **Accepted**, so the **Rust-first library half** of M-520 landed as `mycelium-std-recover` (M-520, #156, Batch P5 Tier-A completion): the `Outcome`/`Resolution` sums (**no `Dropped` variant** — I1), the closed v0 action set, the content-addressed `RecoveryPolicy`/`PolicyRef`, budgeted effects via `mycelium_interp::budget`, and the never-silent `handle_classified` driver. The recovered tag is honest (`Ok`→floor, `fallback`→`Declared`, never laundered up — I2/VR-5; fixes the P5-B exact-tag bug), guarantee matrix asserted in tests. The **self-hosting migration half** (recover authored in Mycelium-lang) is **Batch P5-C, M-502-gated** and is *not* in this wave. The maintainer's append-only **ratification** remains. |
