...
- **(Q4) The platform-entropy `wild`/FFI floor.** The entropy source bottoms out in a platform facility (getrandom/`wild`, ADR-014); whether `rand` inherits a `wild` block to inventory (LR-9) or that floor lives in a `std-sys` phylom (the §8-Q6 split, shared with `fs`/`time`) is open. — *Disposition: FLAGGED; ties to RFC-0016 §8-Q6; keep pure `std.rand`'s seeded half leak-free regardless.*
