...
- **2026-06-23 — §7.2 / §10.2 deferred body-comment preservation now enacted (M-689 + M-690; append-only, VR-5).** The §7.2 known gap (interior comments were refused — exit 4) and the §10.2 open question (trivia-attaching parse) are now **enacted**: M-689 extended the lexer with `lex_with_comments` (a `pub` side-table of comment text + line + trailing flag, keeping the existing `lex` byte-identical — no `Tok` change), and M-690 replaced `mycfmt`'s interior-comment refusals with **placement** (full-line comments become leading doc-blocks before the following item; trailing same-line comments are interleaved at their line; the C1/C2/C3 round-trip guards are preserved — comments are AST-invisible). Agreement across the comment-preservation corpus is **`Empirical`** (trial tests, conformance fixture green) — **never `Proven`** (VR-5). **Remaining honest deferrals (not silently removed):** **nested-match trailing-arm comments** that cannot be anchored to a specific item without AST line numbers remain an explicit refusal (exit 4 — G2; future work requires a thin `Item`/`Arm` line-number extension); a structured **`///` doc-comment convention** (myc-doc-extractable) is a planned fast-follow RFC (M-691 note), not yet ratified. The §10.2 question is considered **enacted** for the `//` faithful-preservation case; the `///` convention extension stays open.
