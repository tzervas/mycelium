...
|---|---|
| **Note** | DN-40 |
| **Status** | **Accepted** (2026-06-26; **ratified by maintainer**) — the **six secure-input principles**, the **gap-ledger prioritization** (the three `Proven` items A1/A2/A3 first), the **recognizer-per-boundary architecture**, and the **generalizable principle** are ratified as the design direction, and the maintainer **authorized executing the named fixes via swarms** (A1/A2 then A3 first, then the mediums). **Accepted ratifies the direction + authorizes the work** — the note itself still **enacts no code** and upgrades no guarantee (each gap keeps its honest severity/tag basis — exhibited gaps `Proven`, architecture `Declared`, prior-art `Empirical`/`Proven`-at-source; VR-5); each fix lands as its own reviewed change. Prior: **Draft (advisory)** (2026-06-26; review capture). Append-only; house rule #3. Records the maintainer-commissioned **input-validation architecture review**: *devise a highly concurrent, high-throughput system for handling input validation across the stack so only intended inputs can occur*, and *consider where we do need input validation and other secure best practices*. Answers both halves: **(a) WHERE** validation is needed — a ranked **gap ledger** over the 5 audited boundaries / 10 finding clusters (§3); and **(b) the ARCHITECTURE** — one closed-grammar recognizer per boundary minting an immutable, canonical, bounded typed value, which is *simultaneously* the security mint and the lock-free concurrency fan-out (§4–§5). Motivated by the spore `content_address` injectivity finding (DN-39 §5 / PR #617, now **FIXED**) as the **reference pattern**. **Three actionable security items** lead the ledger: a **CRITICAL `Proven`** type-subgrammar stack-overflow DoS and a **HIGH `Proven`** pattern-subgrammar DoS in the L1 parser (un-guarded recursion, no `MAX_EXPR_DEPTH` charge → uncatchable `SIGABRT`), and a **HIGH `Proven`** parse-don't-validate gap on the identity-bearing dependency hash (free-text `String`, the existing `ContentHash::parse` smart constructor unused). Per-finding tags held at the finding's own honest basis (exhibited code gaps `Proven`; architecture + Mycelium-specific design `Declared`; prior-art mechanisms `Empirical`/`Proven`-at-source) — no severity inflated (VR-5). Append-only; house rule #3. Enacts no code, moves no decision status — it **records gaps and recommends fixes**; each fix is a separate forward decision the maintainer ratifies/directs. **CLOSURE (2026-06-28, `hrd`): the three `Proven` items A1/A2/A3 LANDED** — A1/A2 the shared parser recursion-depth guard (`MAX_EXPR_DEPTH = 256`, never-silent `ParseError`; `4456bd3`, 2026-06-26 hardening release) and A3 the typed dep-hash (`Dependency.hash: Option<ContentHash>`, parsed at the manifest boundary; `e7e705f`/`3f55eaa`). Crash-refused L1 depth tests + `mycelium-proj` manifest tests re-run green this session; recorded in `CHANGELOG.md` §Security (2026-06-26: DN-40 input-validation hardening). The bound is `Proven`-by-construction; the `256` limit value stays `Declared` (VR-5). The note itself still **enacts nothing** — this records only that its named fixes shipped. |
...
- **2026-06-28 — A1/A2/A3 fixes LANDED (closure note; `hrd`).** The three `Proven` gaps the note's ledger
  prioritized are now closed in code: A1/A2 the shared L1 parser recursion-depth guard (`MAX_EXPR_DEPTH = 256`,
  never-silent `ParseError`; `4456bd3`, the 2026-06-26 hardening release) and A3 the typed dep-hash
  (`Dependency.hash: Option<ContentHash>`, parsed at the manifest boundary; `e7e705f`/`3f55eaa`). Crash-refused
  L1 depth tests + `mycelium-proj` manifest tests re-run green this session; recorded in `CHANGELOG.md` §Security
  (2026-06-26). The bound is `Proven`-by-construction; the `256` limit value stays `Declared` (VR-5). The note
  still **enacts nothing** — this records only that its named fixes shipped (the mediums/lows landed in the same
  gap-ledger wave). Status row updated with the same closure. (Append-only; house rule #3; VR-5; G2.)
- **2026-06-26 — Created (Draft, advisory) — authored.** Records the maintainer-commissioned **input-validation
  ...
  **generalizable principle**; (§8) the **open-question ledger** (sequencing A1/A2/A3 before E14-1; a shared
  ...
  (Append-only; VR-5; G2.)
- **Ratified Draft → Accepted (2026-06-26).** The maintainer ratified the six principles, the gap-ledger
  prioritization (A1/A2/A3 first), the recognizer-per-boundary architecture, and the principle — and
  **authorized executing the named fixes via swarms**. The status move adopts the *direction* and greenlights
  the *work*; it enacts no code and upgrades no guarantee (each gap keeps its `Proven`/`Declared`/`Empirical`
  basis — VR-5). Each fix (A1/A2 parser depth-guard; A3 dep-hash parse-don't-validate; then the mediums)
  lands as its own reviewed, change-scoped PR.
