...
|---|---|
| **Note** | DN-126 |
| **Status** | **Draft** (2026-07-12). A design-reasoner recommendation for M-1077, worked forward to a ranked, ratification-ready proposal. **Not Accepted, not Enacted** — the maintainer ratifies (house rule #3: this note does not self-ratify and ships no code). Every mechanism below is `Declared` until built and differential-witnessed; `Empirical` claims are read against the tree at `dev@fa53dc46` and cite `file:line`. |
...
**This note ratifies nothing (house rule #3): Draft only; supersede, never rewrite.**
