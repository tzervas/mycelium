...
| Date | Status | Note |
|---|---|---|
| 2026-06-27 | **Accepted** *(§4 mixed-width refusal superseded for arithmetic by DN-51; append-only)* | The §4 / Q? rule that a **mixed-width** binary call is a never-silent *refusal* was the *conservative* placeholder ("don't guess"). **DN-51** (Accepted 2026-06-27) supersedes it **for arithmetic**: cross-width operands now **auto-widen to the wider** (DN-41 zero-extend, `Exact`/lossless), with hybrid overflow (promotion default keeps the `Overflow` refusal; `widening_*` growth-to-fit is opt-in) and explicit narrowing (checked-narrow + `truncate`). A cross-**paradigm** mix (binary vs ternary) still refuses (a swap question — RFC-0034/ADR-032), and an *undetermined* width is still `Residual`. DN-51 also resolves the width-generic **guarantee model** to **per-instance** (the Decision-1 fork). Implementation is a follow-on; this is a forward pointer, no status change here. |
