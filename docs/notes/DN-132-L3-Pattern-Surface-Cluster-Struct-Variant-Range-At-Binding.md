...
|---|---|
| **Note** | DN-132 |
| **Status** | **Accepted — P1 only** (2026-07-12, ratified under explicit maintainer delegation — mirrors the DN-115/117/118/122/123/124/125/126/127/128/129/130/131 precedent). Was **Draft** (2026-07-12, same day). **Per FLAG-6, the ratification takes the split the note itself flags as available:** P1 (struct-variant patterns, §5.1–§5.3) is **Accepted** — it has no cross-DN dependency and is independently verified sound and immediately buildable. **P2/P3 (range and `@`-binding patterns, the `when`-guard idiom, §5.4/§6 B1/C1) are explicitly PREREQUISITE-GATED, NOT ratified as "already served":** they remain a **documented conditional** — the recommendation to adopt the `when`-guard idiom is accepted **in direction**, but is **not actionable and not Accepted as a build precondition** until **M-833/DN-79** (guard-clause semantics, Accepted 2026-07-02 but `status: todo`, unbuilt) lands. Until then P2/P3 stay honest catch-all transpiler gaps, exactly as this note's own §2/§5.4 describe. **The decline of dedicated L1 range-token/`Pattern::Range`/`Pattern::Bind` grammar (B2/C2) is Accepted regardless of the M-833/DN-79 timeline** (a permanent KISS/YAGNI decline, not itself gated). **Accepted (P1), not Enacted** (house rule #3) — P1 **builds nothing** yet; every mechanism stays `Declared`/unbuilt until the FLAGGED build issue (M-1089, minted at this close-out) lands and is differential-witnessed. Does not edit `crates/mycelium-l1/**`/`crates/mycelium-transpile/**`; `Doc-Index.md`/`CHANGELOG.md`/`issues.yaml` are applied by this ratification's integration close-out (recorded here, append-only). |
| **Ratification basis (recorded verbatim, 2026-07-12)** | **P1 (struct-variant patterns) is ratified as a Native-Equivalent desugar** to positional `Pattern::Ctor` via a **variant-aware `StructLayout`** (keyed by emitted constructor name, populated from `Item::Enum` struct-variants too — a population change to the existing `HashMap<String, Vec<Option<String>>>`, not a new type) and a `Pat::Struct` arm in `map_pattern_inner`, reusing the Maranget usefulness pass **unchanged** — **zero kernel growth** (KC-3), the identical shape the landed or-pattern (M-823) and tuple-pattern (M-826) sugar already take. Name resolution, `..`-rest arity, and field-order canonicalization are all never-silent (OQ-4/OQ-5); pattern-position matching on a DN-104-sealed constructor **stays allowed** under DN-132 (matching is not construction, OQ-3(a)). **P2/P3 are explicitly NOT ratified as already-served** — the note's own load-bearing self-correction (a reviewer-caught error in an earlier draft) is upheld: `Arm` has no guard field and the transpiler refuses every guard today (`emit.rs:1060–1064`), so the `when`-guard idiom is a **real, named dependency on M-833/DN-79**, not existing machinery. The decline of B2/C2 (dedicated range/`@`/`Bind` L1 grammar) is grounded in DN-119 §8's *measured* ~0 `checked_fraction` payoff for pattern-surface grammar, not convenience — retained as a **named-trigger** residual (OQ-1: if a port driver needs provable range-exhaustiveness, B2 becomes warranted). Gate PASS after the load-bearing P2/P3 correction patch — ratified on the merits under maintainer delegation, taking the FLAG-6 split exactly as offered; this note's own reasoning (§1–§10) is not re-litigated, only executed and recorded (VR-5). |
...
of the wrong arity (a check failure at best, a mismatch at worst). The resolvability gate (§5.1) makes
...
**Applied at the 2026-07-12 ratification close-out (append-only note, taking the FLAG-6 split as
offered):** items 7–11 are ratified for **P1 only**; the P2/P3 half of item 7 (adopt B1/C1 once
M-833/DN-79 lands) is accepted **in direction** but recorded as a **conditional, not an executed
ratification** — it re-activates automatically once M-833/DN-79 lands, with no further DN-132 edit
needed (the design in §5.4 is already complete and ready). Status is **Accepted — P1 only** (not plain
Draft, not a blanket Accepted covering P2/P3).
...
**Applied at the 2026-07-12 ratification close-out (append-only note):** `Doc-Index.md` DN-132 row added
at status **Accepted — P1 only**; `CHANGELOG.md` carries the ratification entry, noting the P1/P2-P3
split explicitly (never "DN-132 Accepted" unqualified); **M-1089** minted for exactly the P1 transpiler
build named in FLAG-5 (`depends_on: []`, immediately actionable) — the self-hosted P1 half (DN-123-
dependent) and the P2/P3 `when`-guard-idiom emission (`depends_on: [M-833]`, not yet actionable) are
**not** minted in this close-out; left as FLAGged follow-ups per FLAG-5's own recommendation, to avoid
filing a blocked issue that implies near-term actionability it does not have.
...
- **2026-07-12** — Ratified **Accepted — P1 only** (delegated ratification, gap-close-2 batch, taking
  the FLAG-6 split as offered). Status moved Draft → Accepted-for-P1 under explicit maintainer
  delegation (mirrors DN-115/117/118/122/123/124/125/126/127/128/129/130/131). P1 (struct-variant
  patterns) is Accepted and its build M-id (**M-1089**) minted. **P2/P3 are NOT ratified as
  already-served** — they remain a documented conditional, prerequisite-gated on M-833/DN-79 landing;
  the B2/C2 grammar decline is Accepted regardless of that timeline. Append-only; VR-5/G2.
