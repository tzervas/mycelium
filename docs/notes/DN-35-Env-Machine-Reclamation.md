...
|---|---|
| **Note** | DN-35 |
| **Status** | **Accepted** (2026-06-26; **ratified by maintainer**) — the recommended design is ratified as the **design direction**: (1) static-decisions / dynamic-verification with a trusted core interpreting only `incref`/`decref`/`is-unique`/`reuse-or-alloc`; (2) drop-guided (Frame-Limited) reuse with the runtime `is-unique` gate as the mandatory safety valve; (3) the §5 content-address side-condition (reuse at rc==1 + weak intern + evict-or-copy); (4) the §6 RC ⊕ region exactly-once invariant; (5) the `fast`/`certified` split (ADR-032). **Accepted ratifies the direction, not an implementation** — it **enacts no code**; the build is the forward epic (E12 Increment 3 / task #6), beginning at §9 Increment 1 behind the §2.3 oracle's differential, the runtime `RcCell` probe remaining the sound fallback. The two Mycelium-specific obligations (§5 reuse-vs-identity, §6 exactly-once) stay `Empirical`/`Declared` until prototyped + property-tested; no guarantee upgraded (VR-5). Prior: **Draft** (2026-06-25; direction capture). Append-only; house rule #3. |
...
- **Ratified Draft → Accepted (2026-06-26).** The maintainer ratified the §10 design direction
  (static-decisions/dynamic-verification; drop-guided Frame-Limited reuse with the `is-unique` safety
  valve; the §5 content-address side-condition; the §6 RC ⊕ region exactly-once invariant; the
  `fast`/`certified` split). The status move accepts the *direction only* — it enacts no code and
  upgrades no guarantee: prior-art single-mechanism results stay `Proven`-at-source, the two
  Mycelium-specific obligations stay `Empirical`/`Declared` until prototyped + property-tested, the §9
  audit trail stays `Exact` count / `Declared` correspondence (VR-5 — no upgrade past basis). The build
  is the forward epic (E12 Increment 3 / task #6), beginning at §9 Increment 1 behind the §2.3 oracle.
