...
|---|---|
| **Note** | DN-97 |
| **Status** | **Draft** (2026-07-09) — works up the **one unified** branching/merge/propagation decision for the maintainer to ratify. **Enacts nothing** and **moves no other doc's status** (house rule #3, append-only). It **consumes** DN-95 (down-propagation) and DN-96 (forward/up-flow) as inputs and **reconciles** them under the maintainer's ratified decisions + three steers (§0), differing from each where those differ (§10). On ratification it **supersedes** the standalone recommendations of DN-95/DN-96 where they conflict with §0 — append-only: DN-95/DN-96 are **not** edited, only referenced. All tags `Declared` unless a cited source or a verified working-tree fact holds them higher (VR-5). |
