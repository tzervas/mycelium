...
|---|---|
| **Note** | DN-58 |
| **Status** | **Draft** (2026-06-27) — proposes, for maintainer ratification, the L1 *surface forms* (grammar + AST + typing + elaboration plan) for the three RFC-0008 R1 runtime constructs `fuse`, `reclaim`, `tier`, whose surface RFC-0027 §10.5 / ADR-020 deliberately deferred to a follow-on. **Enacts no code.** This is the "follow-on RFC/note" RFC-0027 §10.5 calls for, scoped to the L1 surface; the runtime *machinery* it dispatches to is already landed (M-709/711/713, `mycelium-std-runtime`). |
...
```
// request native (AOT) execution for this definition when it is AOT-eligible
@tier(native)
...
`tier` requests a mode; it never *forces* one in violation of the §4 gate. If a `@tier(native)`
...
tier_mode   ::= 'native' | 'interpreted' ;   // the RFC-0004 ExecutionMode surface spelling
```
...
- `@tier(mode)` is checked as an attribute on a definition; `mode ∈ {native, interpreted}` maps to
...
- Elaboration records the requested `ExecutionMode` on the definition's metadata; the §4
  ...
  (KC-3) — `tier` annotates, it does not lower to a runtime op. An ineligible `native` request →
