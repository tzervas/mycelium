...
|---|---|
| **Note** | DN-59 |
| **Status** | **Accepted** (2026-06-28) — **ratified by maintainer 2026-06-28 (in-session)**. G3 ratification-consolidation accepted: seven axes resolved (Option A cross-hypha sharing for R1; ownership-mode separate RC-annotated IR; Layer-2 analysis frame-limited; drop-latency SLO `Declared` per-epoch model; `rc==1` reuse EXPLAIN-record-only; sweep-coupling parent-child TOTAL/siblings CONCURRENT; env-machine reclamation R1 direction). Fuel-model research note (DN-62) COMMISSIONED as a separate document (being drafted in parallel). → Resolved once DN-62 completes and the FLAG-1 drop-latency question is answered. Prior status chain (append-only): **Draft** (2026-06-28) — a **single ratification-consolidation vehicle** for the G3 "memory / reclamation strategy" group of the Blocked-Decisions Ratification Map. **Proposes** a consolidated resolution of the seven G3 axes for one maintainer sign-off. **Enacts no code** and **moves no other doc's status**: it is a *record-and-confirm* over the already-Accepted RFC-0027 / DN-32 / DN-33 / DN-35 cluster plus a small set of **proposals** for the residue those docs left genuinely open. |
...
- **2026-06-28 — Accepted (ratified by maintainer, in-session).** All seven G3 axes ratified. Fuel-model research note (DN-62) COMMISSIONED as a separate document (being drafted in parallel) to address the FLAG-1 drop-latency / fuel-bound open question. → Resolved once DN-62 completes. (Append-only; VR-5; G2.)
