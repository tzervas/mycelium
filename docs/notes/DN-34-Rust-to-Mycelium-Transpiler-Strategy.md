...
|---|---|---|
| `fn` / closures | nodule function / L1 `Lam` (RFC-0007) | higher-order via RFC-0024 static defunctionalization |
...
| ownership / `&` / `&mut` borrows | the **three-layer memory model** (DN-32): affine move · RC · regions | the transpiler can **reuse the MEM-4 ownership/borrow analysis** (`mycelium-mir-passes`) — Rust's borrow structure maps onto Mycelium's affine/RC layers |
| `Result` / `Option` / `?` | never-silent `Option`/`Result` (`std.error`, G2) | a natural fit — Rust is already explicit here |
...
The interesting synergy: **Mycelium's own memory model is a transpilation asset.** Rust's ownership
annotations are precisely the information the MEM-4 affine/borrow analysis wants — so a Rust→Mycelium
transpiler is unusually well-positioned (the source language already encodes the ownership facts the
target needs).
...
1. **Transpiler home & language.** A Rust tool over `syn` (recommended — native AST, runs in the
   workspace) vs extending the Python py2rust. The `syn` route keeps it in-ecosystem and lets it
   **reuse `mycelium-mir-passes`/`mycelium-core`** directly.
...
- **Corpus:** DN-26 (self-hosting bootstrap — the mechanism this note supplies), DN-27 (post-1.0
  ...
  DN-33 / `mycelium-mir-passes` (the memory model + ownership analysis the transpiler reuses),
