...
- **No runs yet.** The eval harness (`tero-eval`), its committed question set, and the first verdict
  land with **M-1018** (the VSA Layer-2), which is additive on top of the served Layer-1 product here
  (M-1015/M-1016/M-1017). Until that lands, `layer2_enabled` is `false` by construction, the system
  serves Layer-1 only, and the improved-on-RAG claim stays aspiration (G2/VR-5) — no gate has run, so
  none is recorded.
...
- 2026-07-07 — **M-1016/M-1017 landed** (PR #1241, PR #1254; promoted to `main` as a scoped PR). The
  query engine + mandatory provenance (M-1016); and the API fronts (MCP + HTTP, token-scoped) + the 4
  cross-platform skills (M-1017; ADR-044 — the workspace's first async runtime). The served product
  is Layer-1 only; `layer2_enabled` is `false`. **M-1018** (the VSA Layer-2 + the Empirical eval gate,
  §6.1) lands additively next. Minted **M-1020** (native HTTPS/TLS for the HTTP front, maintainer
  follow-on). The §2 fork confirmations remain the last gate before this note → Resolved.
