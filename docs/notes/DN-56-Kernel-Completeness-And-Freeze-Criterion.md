...
| 2026-07-10 | **Enacted** (amended by ADR-045, append-only; no status change) | **Amended by ADR-045 (Accepted 2026-07-10).** ADR-045 **suspends, for a bounded re-freezable whole-project (L0→L3 + lexicon) gap-closure window,** the **§9** kernel-freeze declaration (M-969) and the **§6** DN-39-only post-freeze diff policy — generalizing this note's own 2026-07-06 per-change discipline-gate authorization into a standing window. Re-freeze on ADR-045 §2.4: the DN-99 residual worklist (the 4 open + 12 partial rows, tracked as M-1024…M-1034 under E28-1) exhausted + this §5 gate re-scored green by a DN-56/DN-76-successor scorecard (re-score owner: the maintainer) + a follow-up maintainer-ratified ADR reinstating the §6 DN-39-only policy. DN-56's decision body is **not modified** here — only this dated pointer is appended. Status stays **Enacted**; append-only (VR-5/house rule #3). |
