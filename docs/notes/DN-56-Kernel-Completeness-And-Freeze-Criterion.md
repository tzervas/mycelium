...
|---|---|
| **Note** | DN-56 |
| **Status** | **Accepted** (2026-06-27) — *Proposed → Accepted as the freeze-criterion **framework**, **ratified by the maintainer 2026-06-27** (R5 gate). **Ratifying the framework does NOT declare the kernel frozen (VR-5):** freeze is a future event gated on the five conditions below. Condition #1 (census / never-silent floor) is **satisfied** (W5); condition #3 (primitive set closed) is **contingent on ADR-033 FLAG-1** resolution; #2 (reject-ledger), #4 (lowering surface closed — needs RFC-0037 epic + DN-54 checker), #5 (KC-3 completeness review) remain open.* Prior: **Proposed** (2026-06-27; lang-design wave capstone synthesizing DN-52/ADR-033/DN-54/DN-55). |
