...
| 2026-07-10 | **Enacted** (forward-reference, append-only; no status change) | → see **ADR-045** (Draft): proposes lifting the **§9** kernel-freeze declaration (M-969) and the **§6** DN-39-only post-freeze diff policy for a **bounded, re-freezable** early gap-closure/optimization window — generalizing this note's own 2026-07-06 per-change discipline-gate authorization into a standing window. Re-freeze on ADR-045 §2.4: named worklist exhausted + this §5 gate re-scored green by a DN-76-successor scorecard + a follow-up maintainer-ratified ADR reinstating the §6 DN-39-only policy. DN-56 is **not modified** here; the amendment lands only if the maintainer ratifies ADR-045. Status stays **Enacted**; append-only (VR-5/house rule #3). |
