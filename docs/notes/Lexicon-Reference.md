...
| `derive` *(was `grow`)* | Derive | Automatically generate or extend behavior on a type. **Reconciled `grow → derive`** (DN-38 §8.1, Accepted 2026-06-26 — conventional over the themed `grow`; companions `via` delegation, `reveal` inspector). | Conventional (Rust `#[derive]` / Haskell `deriving`); `grow` superseded for this construct (DN-03 2026-06-27 changelog). | Derive-like / generative features. | `derive Debug for Point;` |
