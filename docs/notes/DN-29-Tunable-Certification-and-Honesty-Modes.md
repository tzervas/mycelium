...
|---|---|
| **Note** | DN-29 |
| **Status** | **Superseded** (2026-06-24) by **RFC-0034** + **ADR-032** (both ratified & Enacted-design 2026-06-24; the corpus amendments applied). This note's deliberation is complete; it is retained as the **rationale record** (append-only). *Draft history (deliberation anchor — advisory): rev. 1 — phase-decomposed knobs, `fast`-default, north-star reframe · rev. 2 — `fast`+`certified` first-class modes, safe-by-default + explicit per-use unsafe escape, diagnostic-verbosity knob, "honesty"→"transparency & auditability" reframe · rev. 3 — provenance tag as adjustable unit (`fast` omits `Empirical`/`Proven`), signal **generation** split from **consumption**, **§11 ripple map** · rev. 4 — **deliberation settled**: §9 Q4–Q7 + Q11–Q13 resolved · rev. 5 — binding act **drafted** (RFC-0034 + ADR-032) · rev. 6 2026-06-24 — binding act **ratified & Enacted (design)**; corpus amendments applied; this note → **Superseded**.* |
...
> *2026-06-24 (rev. 5)* — the binding act is now **drafted**: **RFC-0034** (Tunable Certification &
> Transparency Modes, *Proposed*) carries the mechanism, **ADR-032** (*Proposed*) carries the
> always-on→per-mode supersession + the charter/north-star reframe + the honesty→transparency vocabulary
> reframe, and the staged **`tools/dn29_apply.py`** + `docs/notes/dn29-amendment-manifest.json` (the
> anchor-keyed, never-silent batched amender; dry-run-clean on the SC-3/FR-M3 spine anchors) implement
> §11.4 — **staged, not run** until both are Accepted. DN-29 stays **Draft** until then, at which point it
> moves to **Superseded** (append-only). *2026-06-24 (rev. 6 — terminal)* — the maintainer **ratified**
> RFC-0034 + ADR-032 (Proposed → Accepted → **Enacted (design-driven)**); the amendment manifest was
> **applied** (21 amendments / 13 files: charter SC-3/FR-M3 conditionalize, living-doc transparency reframe,
> append-only footnotes on RFC-0001/0002/0005 + ADR-010/011/013/016/017; process/colloquial "honest"
> excluded). The per-mode rules now govern; the runtime mode mechanism is the paired TDD cycle (pending,
> never claimed done — VR-5/G2). **DN-29 → Superseded**, retained as the rationale record.
