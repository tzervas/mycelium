...
|---|---|
| **Note** | DN-53 |
| **Status** | **Accepted** (2026-06-27, ratified by maintainer, R5 gate) — **implemented (Rust-first), pending ratification → Enacted** (2026-06-28; `mycelium-l1` as M-811). `object` is now an active top-level keyword: parses, desugars to `type`+`impl`+`via` in the checker (Phase 0 structural expansion + Phase 0b `via` forwarding impl generation), `via` keyword added to the token set, conformance corpus fixtures added (`accept/23-object-composition`, `reject/28-object-empty-body` — renumbered from 22/26 at integration to avoid a number collision with DN-54's `accept/22-lower-derive` + `reject/26`/`27`), three-way differential test in `tests/object_desugar.rs` (Empirical agreement: DN-53 §A.3.3). Item-level `pub fn` inside object body is deferred (conservative; see §B.2). Full ratification (`Accepted → Enacted`) pending maintainer review of the landed implementation. Prior: **Proposed** (2026-06-27). |
| **Task** | M-811 |
...
| 2026-06-28 | **Implemented (Rust-first), pending ratification** | M-811 implementation landed in `mycelium-l1`. Changes: (1) `Tok::Via` added to `token.rs` (active keyword, lexed as `"via"`); (2) `ViaDecl` + `ObjectDecl` AST nodes + `Item::Object` variant in `ast.rs`; (3) `parse_object_decl` in `parse.rs` — `object Name[params] { Ctor(T1, T2); via N : Trait; impl …; fn … }` now ACTIVE at item position (was reserved-not-active); mandatory constructor `;`, `pub` inside body refused (G2); (4) `checkty.rs` Phase 0 structural expansion (`desugar_object_structural`) + Phase 0b `via` forwarding impl generation (`expand_object_via_decls`) — never-silent: unknown trait / out-of-range field index → `CheckError`; (5) `ambient.rs` — full ambient resolution of `object` bodies (`r.object_decl(…)`) + surface-rendering (`print_object_decl`) for `expand_to_source`; (6) conformance fixtures `accept/23-object-composition.myc` and `reject/28-object-empty-body.myc` (renumbered from `accept/22`/`reject/26` at integration to deconflict with DN-54's `accept/22-lower-derive.myc` + `reject/26-lower-missing-eq`/`27-derive-missing-for`); (7) `tests/object_desugar.rs` — five tests, including the three-way Empirical differential (`observe(object) == observe(lower(object))`), never-silent `via` refusals, and `pub object` check. Item-level `pub fn` inside object body deferred (§B.2). Full ratification (`Accepted → Enacted`) pending maintainer review. Append-only; VR-5; G2. |
