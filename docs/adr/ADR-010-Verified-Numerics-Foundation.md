...
2. **`ProbBound` kernel (δ).** Start with the **union bound** (lightest; natural for "decode succeeds w.p. ≥ 1−δ" and "P(any of N retrievals fails) ≤ Σ"). Reserve **apRHL**-style approximate couplings (judgments carry `⟨ε,δ⟩`; the `[SEQ]` rule composes multiplicatively in ε, additively in δ) for *relational* reference-vs-implementation certificates, and the **weakest-pre-expectation** calculus (Kaminski/Katoen) for quantitative/expected-value invariants.
