...
| 040 | Scalar-float value form (route ii): a first-class `Repr::Float{width}`, **binary64/F64-only** at introduction on a frozen-tag width registry; IEEE-754 **round-to-nearest-even only** (rounding as operations, ADR-028 parallel); canonical quiet-NaN at construction, bit-distinct signed zeros; in-band propagating specials (±inf/NaN, FLAG-2) with never-silent conversion boundaries (float↔int/parse → explicit `Option`/error); content-address impact documented-not-spent (single E20-1 rehash deferred to first value-persistence, RFC-0033 §7). Fixes the float design inside ADR-038 §2.6's decided route (ii); kernel entry via the §2.6 double gate (this ADR + DN-69). Kickoff `enb` Gap A; M-895 authored, M-896…M-900 implemented | **Enacted** (2026-07-02; stepped through Accepted same day — maintainer-ratified — then Enacted once M-896…M-900 landed) | `ADR-040-Scalar-Float-Value-Form-Route-II.md` |
