...
|---|---|
| **ADR** | 033 |
| **Status** | **Accepted** (2026-06-27) → **propose `Enacted` (2026-06-28, pending maintainer final nod)** — *Proposed → Accepted, **ratified by the maintainer 2026-06-27** (R2 gate): the `FieldSpec::Fn` trusted-core extension for dynamic dispatch is approved, with KC-3 growth (one enum variant) accepted.* **FLAG-1 RESOLVED AND IMPLEMENTED (2026-06-28, r4v + ADR-033 integration wave, M-810):** Path A landed — `FieldSpec::Fn { sig: FnSig }` with `FieldTyRef` per param + return type in `mycelium-core`. `encode_decl` encodes full signature (`FIELD_FN` / `FN_SIG_*` / `FTR_*` tags — injective over typed structure). `cargo test -p mycelium-core` 233+11 green; distinct-hash property test + no-match differential pass. FLAG-1 → **resolved (implemented)**, not merely resolved-pending. Soundness tag: **`Empirical`** (trial-tested via distinct-hash property + differential; **not `Proven`** — unmechanized, VR-5). → **Propose `Enacted`** pending maintainer's final nod: the full-sig encoding landed + verified; KC-3 growth is deliberate and bounded. Prior proposal record: **Path A (type-carrying hash)** recommended (§10); **Path B was NOT recommended** (§10.4 — single-producer side-condition structurally false at L0 trusted-input boundary). Prior: **Proposed** (2026-06-27). |
