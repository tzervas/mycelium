...
| Date | Status | Note |
|---|---|---|
| 2026-06-25 | **Decision (D2/D3; Status unchanged — Accepted)** | **Post corpus-alignment audit.** **D2 — RATIFY + IMPLEMENT E19-1** (not defer): the E19-1 value-model additions — `Repr::Seq` / `Repr::Bytes` + length-in-type + the adapters (ADR-025/026/027, M-749/M-750) — are **to be implemented**, via a **prototype branch first**. **Honest current state:** E19-1 is **currently unbuilt** in code (`Repr::Seq`/`Repr::Bytes` absent; the value model is `Binary`/`Ternary`/`Dense`/`Vsa` only). The work is flagged as a **tracked implementation epic** (forthcoming; *no code is implemented by this note*). **D3 — strict-sequence gate:** the **core 1.0.0 (T1) tag act (M-703)** is committed **only after E19-1 is implemented + verified** (the M-703 `depends_on E19-1` edge above is therefore a hard prerequisite, not advisory — the three-way differential + M-752 conformance gate must be green before the tag). Append-only; the §2/§5 normative criteria are unchanged (VR-5/G2). |
