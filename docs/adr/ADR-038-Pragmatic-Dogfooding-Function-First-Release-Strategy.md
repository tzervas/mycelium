...
|---|---|
| **ADR** | 038 |
| **Status** | **Accepted** (2026-07-01 — **ratified by the maintainer** ("ratify 38"); the strategy binds. Authored + refined the same session, held `Proposed` pending this ratification. **FLAG-V1/V2 (§2.8) resolved by the maintainer (2026-07-01):** the public release is a **sub-`1.0.0` (`0.x`) semver** at the fully-functional, pre-full-dogfood state (**FLAG-V1** — the public release is *not* labeled `1.0.0`, so the label collision dissolves); **semver `1.0.0` = fully dogfooded / self-hosted / rewritten into Mycelium *where appropriate*** — the terminal (**FLAG-V2**)). Records the project's **North Star** — *"Rust where appropriate, Mycelium everywhere else"* (pragmatic dogfooding, **not** zero-Rust dogmatism) — and the **function-first** release strategy that follows: **the public release is gated on functional usability and is *version-independent*** — it happens at whatever semantic version fits the work completed (a **`0.x`**, well before `1.0.0`), **not** at any version milestone (§2.8). The **public semantic version tracks the Mycelium-rewrite progress**, climbing `0.x → 1.0.0` **in the open**, where **`1.0.0` ≡ "fully rewritten into Mycelium (where appropriate) and 100% operational."** So: **Phase I** → reach functional usability, then **go public at a `0.x`**; **Phase II** → the progressive Mycelium rewrite continues **publicly**, advancing the version toward `1.0.0`, with **compiler self-hosting a deferred, conditional aspiration**. **For now the version stays `0.0.0`; the concrete semver scheme is deferred until the project is actually ready to publish** (§2.8). Execution doctrine (§2.7): **Fable-class models are reserved solely for planning + complex design; implementation and lighter work run on Opus/Sonnet/Haiku scoped to task complexity.** In doing so it (a) **refines ADR-036 §2.4** (append-only — the public-release gate becomes functional usability, decoupled from any version number, not completed Rust-replacement) and (b) records that **ADR-036 superseded RFC-0031 §5 D1's "compiler stays Rust forever" boundary** (append-only note there). Two ambiguities are **FLAGged for the maintainer, not guessed** (§2.8): the `lang 1.0.0` label collision with ADR-022's functional-completeness milestone (FLAG-V1) and whether `1.0.0` requires compiler self-hosting (FLAG-V2). **Now Accepted (2026-07-01)** — the ADR-036 §2.4 refinement and the RFC-0031 §5 D1 supersession are **in force**. |
...
**FLAG-V1 — the `lang 1.0.0` label collision — RESOLVED (maintainer 2026-07-01).**
...
> **RESOLVED (maintainer, 2026-07-01):** the public release is a **sub-`1.0.0` (`0.x`) semver** at the
> fully-functional, pre-full-dogfood state — so the public-release milestone is **not** labeled
> `1.0.0`, and **the collision dissolves**. ADR-022's `lang 1.0.0` functional-completeness *event*
> triggers the sub-`1.0.0` public flip; the numeric semver `1.0.0` is reserved for rewrite-completion.
> **No ADR-022 relabel is required** — the distinction holds because the public release is explicitly
> sub-`1.0.0`.

**FLAG-V2 — does `1.0.0` require compiler self-hosting? — RESOLVED (maintainer 2026-07-01).** §2.3 makes compiler self-hosting a
...
> **RESOLVED (maintainer, 2026-07-01):** `1.0.0` is reached when the project is **fully dogfooded /
> self-hosted / rewritten into Mycelium *where appropriate*** and 100% operational (the confirmed §2.8
> terminal). Compiler self-hosting rides §2.3's condition (only if stability/perf-proven) — it is part
> of *"where appropriate,"* not an independent hard gate: if a Rust compiler remains the appropriate
> choice, `1.0.0` is reachable with it. The **public-release gate is the pre-full-dogfood, fully-
> functional state at a sub-`1.0.0` (`0.x`) semver** and requires none of the rewrite.
...
- [x] Maintainer ratification: status `Proposed → Accepted` — **maintainer-ratified 2026-07-01
  ("ratify 38")**; the strategy binds; §2.4's refinement of ADR-036 takes effect.
...
- [x] FLAG-V1 / FLAG-V2 (§2.8) **resolved by the maintainer 2026-07-01**: the public release is a
  **sub-`1.0.0` (`0.x`) semver** at the fully-functional, pre-full-dogfood state (FLAG-V1 — the public
  release is *not* `1.0.0`, so the collision dissolves); **semver `1.0.0` = fully dogfooded / self-hosted
  / rewritten into Mycelium where appropriate** (FLAG-V2).
...
| Date | Status | Note |
|---|---|---|
| 2026-07-01 | **Accepted** (maintainer-ratified) | Maintainer ratified via **"ratify 38"** — status `Proposed → Accepted`; the strategy §2.1–§2.8 binds. The **ADR-036 §2.4 refinement** and the **RFC-0031 §5 D1 supersession** are now **in force**. **FLAG-V1/V2 resolved** (maintainer, same day): the public release is a **sub-`1.0.0` (`0.x`) semver** at the fully-functional pre-full-dogfood state (public ≠ `1.0.0` → no collision, FLAG-V1); **`1.0.0` = fully dogfooded/self-hosted/rewritten-where-appropriate** (FLAG-V2). Does not amend ADR-022's tag gate; `Enacted` still awaits both phase gates (§5). |
