...
|---|---|
| **ADR** | 037 |
| **Status** | **Enacted** (2026-07-01 — implemented + demonstrated; stepped through **Accepted** first, same day). The remote backend landed in `mycelium-spore` (`remote` module, M-871), and its Definition of Done (§3) is met: publish→resolve→fetch-and-verify round-trips green against **both** a local `registry:2` OCI registry (`scripts/dist/spore-oci-selftest.sh`, 57 unit tests) **and** the **live GitHub Packages registry** — the example phyla `hello` + `std` were published to `ghcr.io/tzervas/{hello,std}:0.0.0-dogfood` and resolved back with identical, hash-verified `spore_id`s (`hello=blake3:6ce6d13…`, `std=blake3:52b6982…`, byte-identical local↔GHCR — deterministic content-addressing confirmed across backends). One disclosed v0 scope limit (never-silent): remote publish does **not** yet enforce `name@version` immutability the way the local store does (OCI tags are mutable; a best-effort client-side pre-check is tracked as **M-872**). Fixes the **remote/networked backend** for the `spore` registry (M-732's local file store gains a remote sibling): a published spore is distributed as an **OCI artifact** in the **GitHub Packages container registry (GHCR)**, decomposed into the **DN-28 dense-map** — each content-object (source file, by BLAKE3 address) is its own OCI blob (so identical objects **dedup** across versions and phyla by construction), and the dense-map DAG (spore_id + object references + dependency edges + germination surface) is the OCI **config** blob; the `name@version` index is the OCI **tag**. `oras` is the **v0 wire-transport driver** (a pluggable transport; a pure-Rust OCI client is named future work, not faked). This is the binding backend decision **DN-28 anticipated** ("the binding decision is a future RFC"); it does not amend any ADR-022 Definition-of-Done criterion. |
...
> do not prove — never `Proven`. The ADR reached **Enacted** on exactly the basis its Definition of Done
> (§3) names: the `mycelium-spore` remote backend implemented + tested + a **live GHCR** publish/resolve
> round-trip demonstrated (M-871, epic E26-1). What is **not** claimed: remote `name@version` immutability
> enforcement (a disclosed v0 gap — the local store enforces it, the remote does not yet; tracked as
> **M-872**), SemVer range resolution (stays `Unsupported`), and any `Proven`-tier guarantee on the
> `oras`/GHCR wire.
