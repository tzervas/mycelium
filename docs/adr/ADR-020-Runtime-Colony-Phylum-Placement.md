...
|---|---|---|---|
| `Colony<T, E>` | Type alias (`= Scope<T, E>`) | DN-06; RFC-0008 §4.7; `runtime.rs` line 116 <!-- erratum 2026-06-25: was "line 106"; the alias is at runtime.rs:116 --> | — (structural alias; inherits `Scope` tags) |
...
- **2026-06-25 — Record-completion (post corpus-alignment audit; append-only; no status change).** The header has read **Enacted (2026-06-20)** since landing, but the only changelog entry below recorded "Proposed … awaiting maintainer ratification" — the intermediate `Accepted` and `Enacted` append-only transitions (house rule #3) were never written. This entry **completes the record retroactively** (it records the transitions that already occurred; it moves no status backward and ratifies nothing new):
  - **2026-06-20 — Accepted.** Maintainer-ratified the Option-C hybrid decision (dedicated `runtime` phylum + thin `std.runtime` facade) and the v0 API surface / guarantee matrix above. Basis: **M-521** (design-first acceptance criterion — "the binding set + the std-vs-separate-phylum decision presented" — met).
  - **2026-06-20 — Enacted.** `landed_basis:` `crates/mycelium-std-runtime` — the M-521 v0 R1 implementation (the `runtime` phylum crate + the `std.runtime` facade nodule, wrapping `mycelium-mlir::runtime`/`channel`) landed on `main` with the per-op guarantee matrix as tested data. The reserved RFC-0008 §4.5 vocabulary (`hypha`/`fuse`/…/`reclaim`) stays Glossary ⟂, activating construct-by-construct at the Phase-7 gate.
