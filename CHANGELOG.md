...
- **DN-25 — Full-Language 1.0.0 Program Map** (new design note, 2026-06-23). Records the nine-track decomposition (T1–T9), the per-track Definition of Done, the critical-path analysis (long poles: T4 stdlib-in-Mycelium `lib10` and T9 self-hosting capstone `boot10`, both gated on T2 surface completeness), inter-track dependencies, and the governing ADR-022 gate scope. Advisory, append-only; planning artifact only — no spec moves to Accepted/Enacted here.
...
### Fixed (2026-06-23: post-l1-capstone follow-ups — Copilot review · idmap union-merge · issue-status)
- **All five Copilot #397 review comments resolved + replied.** Four (fn-typed-LHS parenthesization in the `Ty::Fn` / `BaseType::Fn` / `--expect-main` printers; CRLF `\r` strip in comment capture) landed in the wave squash; the fifth — `lex()` delegated to `lex_with_comments()` and allocated a `String` per `//` comment it then discarded — fixed by gating capture behind a `capture_comments` flag (a shared `run_lexer` driver), so the compiler front-end fast path is **allocation-free on comments** (which matters now that docstrings are ubiquitous); token stream identical on both paths (new `lex_fast_path_skips_comments_capture_path_keeps_them` test). (Copilot #397)
- **`.gitattributes`: dropped `merge=union` on `tools/github/idmap.tsv` + `tools/github/issues.yaml` (swarm mitigation #2).** Union-merge silently appended both sides of a divergent merge, producing duplicate task-id rows / YAML keys (a local `gh-issues-sync.py --all` reported 26 "task-id mapped twice"). These structured, orchestrator-owned files are never edited by parallel agents, so they now use the default 3-way merge — a real collision surfaces as an explicit conflict, never a silent duplicate (G2). `CHANGELOG.md` + `docs/{notes,rfcs,adr}/*.md` prose keep union-merge. (`origin` idmap verified dup-free; this prevents recurrence.)
- **issues.yaml status reconciled to landed:** the 11 l1-capstone issues (E7-3 + M-684…688, E7-4 + M-689…691, M-649) → `done`; **E7-5 / M-692** (operator syntax) honestly remain `needs-design` (only DN-23 landed; RFC-0025 + the symbol sugar are future — VR-5, no overclaim).

### Added (2026-06-23: E7-3/E7-4/M-649 — l1-capstone wave: HOF via static defunctionalization + comment-preserving mycfmt + std.result self-hosts)
- **HOF via static defunctionalization — `A -> B` function type + named-fn-as-value (RFC-0024; E7-3 / M-685…688).** The L1 frontend now supports a minimal higher-order surface: the function type `A -> B` in parameter/field positions (`BaseType::Fn` in `ast.rs`; `Ty::Fn` in `checkty.rs`), and a bare top-level function name in value position (typed by `check_path` as `Ty::Fn`). Higher-order calls are lowered by **static defunctionalization** entirely in `mono.rs` — each call site is specialized by its statically-known fn argument, rewriting `f(x)` to a direct call and dropping the fn-valued parameter — so the **L0 kernel stays first-order** and **`mycelium-core` is untouched (KC-3)**. The dispatch choice is reified in the EXPLAIN selection record (`MonoSelections`; house rule #2 — no black boxes). **Honesty (VR-5):** out-of-scope cases (closures, dynamic fn-flow through `match` arms or data fields, partial application, still-generic fn-as-value) remain **explicit never-silent `Residual`s** — full Reynolds defunctionalization is the named future path. Contract is **`Declared`**; three-way differential agreement (L1-eval ≡ L0-interp ≡ AOT) is **`Empirical`** (trial tests). **DN-23** word-operators-as-first-class values are unblocked as a side benefit. **RFC-0024 → "implemented (Rust-first), pending ratification"** (never silently `Accepted` — VR-5). (E7-3; M-685/686/687/688; RFC-0024)
- **Comment-preserving `mycfmt` — `//` comments survive the formatter (E7-4 / M-689 + M-690); §7.2/§10.2 enacted.** The lexer gains `lex_with_comments` (a `pub` side-table of comment text + line + col + trailing flag, byte-identical to the existing `lex`; `Tok` unchanged — KC-3). `mycfmt` replaces its interior-comment refusals (exit 4) with **placement**: full-line comments become leading doc-blocks before the following item, trailing same-line comments are interleaved at their line, idempotence preserved (one-arm-per-line match rendering gated on comment presence). The C1/C2/C3 round-trip guards are unaffected (comments are AST-invisible). A genuinely unplaceable comment still refuses explicitly (G2 — never-silent). Agreement is **`Empirical`** (conformance fixture corpus, never `Proven` — VR-5). **Honest residual deferrals:** nested-match trailing-arm comments (need AST line numbers) and a structured `///` doc-comment convention remain future work. (E7-4; M-689/690; Mycfmt-Formatter-Contract §7.2/§10.2)
- **`std.result` self-hosts `map`/`and_then`/`fold` with inline docstrings (M-649).** `lib/std/result.myc` now contains all six functions as fully executable self-hosted Mycelium-lang code (the three original + three HOF combinators via RFC-0024). The companion pseudocode file `result.pending-hof.md` is removed (the honest deferral placeholder, per the 2026-06-23 DN-14 entry — now superseded by executable code). Inline `//` docstrings survive `mycfmt --check` (E7-4). Three-way differential agreement `Empirical`; type-level contract `Declared`. (M-649; DN-14)
- **DN-23 — Operator Syntax design note.** Advisory (DN-17 posture) recording the design space for operator syntax: **words** (status quo) vs **symbols** (buys precedence/associativity). Recommendation: keep words canonical in the kernel; if symbols are wanted, add them as pure surface sugar that desugars to word functions (frontend-only, no kernel change — KC-3), its own RFC-0025. Word operators (`add`, `xor`, …) become first-class values once RFC-0024 lands. (DN-23)

### Changed (2026-06-23: doc-status reconciliation — RFC-0024 + DN-14 HOF gap + Mycfmt-Formatter-Contract enacted)
- **RFC-0024** status: `Proposed → implemented (Rust-first), pending ratification` (E7-3 / M-685…688 + M-649 — landed on l1-capstone head; append-only; never silently `Accepted` — VR-5). Doc-Index + `rfcs/README` rows updated accordingly.
- **DN-14** HOF gap closed (append-only changelog entry): HOF combinators now executable + self-hosted; `result.pending-hof.md` companion removed; remaining honest deferrals (closures, dynamic fn-flow, partial application, multi-arg arrows, `///` doc-comment convention) recorded unchanged.
- **Mycfmt-Formatter-Contract** §7.2/§10.2 deferred body-comment preservation enacted (M-689 + M-690; `Empirical` agreement; residual deferrals — nested-match trailing-arm comments + `///` convention — recorded; append-only).
- **issues.yaml** status: M-685/686/687/688/689/690/M-649/E7-3/E7-4 → `in-progress` (implemented on l1-capstone head; NOT yet `done` — pending the integration→main PR).

### Added (2026-06-23: M-673 — monomorphization makes generics + traits run to closed L0; E7-1)
- **`mycelium-l1` now elaborates generics and traits to closed L0** (RFC-0007 §11.3/§12.3 — the M-657/M-659 checkers' staged half). A new frontend **monomorphization pre-pass** (`crates/mycelium-l1/src/mono.rs`) specializes every reachable concrete generic data/fn instantiation at its type arguments and **statically resolves** each trait-method call to a direct call to the one coherent instance's (mangled, monomorphic) method body, yielding a closed monomorphic `Env` the existing `elaborate`/`build_registry` lower **unchanged** — **no `mycelium-core` change** (KC-3). Trait dispatch is reified in a queryable **EXPLAIN** selection record (`MonoSelections`; no black boxes — house rule #2); an additive `Env.impls` retains the instances' resolved method bodies. **Mangling is injective + surface-disjoint** (`$`-joined, with a `#` tag on a nullary data type, so a data type named like a repr — e.g. `Binary8` — can never alias the repr `Binary{8}`; two instantiations never silently collide to one body — G2). **Honesty (VR-5/S1):** totality is **recomputed** over the specialized set (never fabricated), no `Swap` is ever inserted, and an undetermined type parameter + every out-of-fragment construct (multi-parameter traits, higher-order `A->B` generics, `wild`/FFI, `spore`, VSA, `Substrate`) stay **explicit** `Residual`s — the generic/trait refusal sites in `elab.rs` are kept as defensive invariants. A generic's content-addressed identity **fragments across specializations** — recorded via the mangled names, not hidden (RFC-0019 §4.4); the **literal RFC-0019 §4.5 runtime dictionary-record** form remains deferred (it needs an abstract-parameter `FieldSpec` in the trusted core — a separate ADR). **284 `mycelium-l1` tests green** incl. the **M-210 three-way differential** (L1-eval ≡ L0-interp ≡ AOT) on the monomorphized env for generics + traits, a **mutant-witness** that the differential discriminates, determinism, the mangling-injectivity boundary (the adversarial repr-named data type), and the defensive-`Residual` invariants; `cargo clippy -D warnings` + `cargo fmt` clean. RFC-0007 §11.3/§12.3 + RFC-0019 §4.4/§4.5 carry append-only "landed" notes; **DN-14 §3 rows 6 (generics) + 7 (traits) → `present`**; **M-657/M-659 → done**; unblocks M-649. The trait-lowering shape (static resolution + EXPLAIN record) was maintainer-confirmed. (M-673; E7-1)

### Fixed (2026-06-23: doc-status sync — Doc-Index RFC-0022/0023 → Accepted; dfr orientation refreshed)
- **Synced the canonical status map to the landed RP-10/RP-9 ratification.** The #344 discharge moved RFC-0022 (Web-Tooling) and RFC-0023 (ADK) **Draft → Accepted** in their headers, the RFC index (`docs/rfcs/README.md`), `research-prompts.md`, and the M-670/M-671 issue bodies — but left the **top-level `docs/Doc-Index.md`** rows at **Draft**. Brought those two rows to **Accepted** (append-only, newest-first; mirroring `rfcs/README.md`: design agreed, **Enacted** still gated on the `mycelium-web`/`mycelium-adk` builds + E7-1/E7-2). No decision changed — this only propagates the already-recorded ratification into the index #344 missed (the `dfr` research gate is closed; see the 2026-06-21 entry below). Also refreshed the agent-session orientation docs (`.claude/agent-context.md`, `.claude/kickoffs/README.md`) to mark the **`dfr`** kickoff **landed (#344)** and archived. (dfr housekeeping)

### Added (2026-06-22: DN-22 — Compact Diagnostic Code Representation (design capture))
- **`docs/notes/DN-22-Compact-Diagnostic-Code-Representation.md`** — a design capture (advisory, DN-17 posture) for a **compact diagnostic code**: a packed status **byte** (`(component<<3)|reason` — `echo $?` decodes uniquely: `component = code>>3`, `reason = code & 7`) plus a variable-length **base-36 string** (`F<component><reason>`, e.g. `Fb2` = format/rust-unformatted, `Fn2` = doc-index/stale), encoding *which component failed and why* at high granularity in very few tokens. Framed as a **lossless, `EXPLAIN`-able projection** (G11) of an RFC-0013 reified diagnostic for the AI co-author loop (NFR-2/SC-5b/M-330) — governed by DN-04's rule that **diagnostics are additive over the explicit error, never substitutive** (G2): the byte is a coarse glance over an unbounded b36 handle; a **registry is the single source of truth**, codes are *derived* (never hand-assigned), `EXPLAIN`-decode back to the full diagnostic (house rule #2 — no black boxes), and are an **append-only versioned interface** (ADR-018, like the guarantee tags) with severity encoded. The check tooling (`scripts/checks/all.sh`: component-id + reason packed exit byte + `F<c><r>` base-36 code + lossless failure digest) is the **already-landed reference implementation** (#377). Six open questions for ratification (byte field layout, b36 grammar, registry home, RFC-0013 derivation, payload tier, KC-3 boundary). No kernel change. (DN-22)

### Changed (2026-06-22: M-679…M-683 — DN-21 unsafe-code hardening landed (M-678 epic; #373))
- **Enacted the DN-21 behaviour-preserving, zero-new-dependency unsafe-code hardening epic** across `crates/mycelium-mlir` + the check tooling (ADR-014; DN-21 §6). **M-679:** strengthened the three thin JIT-FFI `// SAFETY:` comments (`jit.rs` — the `dlclose` dangling-pointer obligation §4, the `dlsym` handle-liveness chain, the `dlopen` `RTLD_NOW = 2` platform assumption + no-global-ctors fact) and added `debug_assert!(!ptr.is_null())` at the FFI/transmute sites (machine-checks the "checked non-null" preconditions in dev/test; zero release cost). **M-680:** re-pinned `#![forbid(unsafe_code)]` on the ADR-014 trusted base (`mycelium-core`/`-cert`/`-numerics`/`-vsa`/`-interp`/`-select`) and the 11 zero-unsafe `mycelium-mlir` submodules — "no unsafe by convention" is now compiler-enforced. **M-681:** added `just safety-check` (`scripts/checks/safety.sh`, wired into `scripts/checks/all.sh`) — a `// SAFETY:`-adjacency gate (pure `git grep`, no toolchain, never skips) that fails loudly (G2) on any Rust `unsafe` under `crates/` lacking an adjacent `// SAFETY:` (or a `# Safety` rustdoc section for an `unsafe fn`); a deliberately un-commented `unsafe` trips it (mutant-witness). **M-682:** added the in-house **`Sym<'lib, T>`** lifetime-binding newtype + **bind-once** `BoundBitnetDot`/`BoundSpecializedDot` handles — closing the §4 `*mut c_void` co-location dangling-pointer risk **structurally** (a compiler-checked lifetime; no raw fn-ptr field survives; no per-call `dlsym` in the E1 hot loop). The ABI `transmute` is one private `unsafe fn get` exposed only through audited **safe, fixed-type accessors** (`jit_kernel`/`bitnet_dot`/`spec_dot`), so no safe code makes an ABI claim (a soundness fix from the PR review) and `bitnet`/`specialize` are now themselves `#![forbid(unsafe_code)]` — **all workspace `unsafe` is now confined to `jit.rs`**. **M-683:** documented the `audit_wild` (`.myc`) ⟂ Rust `unsafe` (`.rs`) two-population scope split (`Security-Checks-Contract.md` §4.1). The DN-21 §7 irreducible-unsafe floor (calling JIT'd fn-ptrs; the ABI claim) is unchanged — now expressed once, lifetime-bound, never pretended away (VR-5/G2/KC-3). Verified: `mycelium-mlir` 95 unit + all differential suites green (behaviour-preserving), `safety-check` green, full-workspace build clean. (M-678 → M-679…M-683; #373)
### Added (2026-06-22: M-663 — RFC-0018 stage-1a static guarantee grading (E7-1); RFC-0018 → Enacted)
- **`mycelium-l1` now statically enforces the guarantee index `@ g`** (RFC-0018 §4.3, **stage 1a**, **Design A** — data-lineage/data-provenance integrity). The grade `g ∈ {Exact, Proven, Empirical, Declared}` was a *runtime* tag checked dynamically (RFC-0007 §4.3, stage 0); it is now a **statically-checked constraint** over the integrity lattice `Exact ⊐ Proven ⊐ Empirical ⊐ Declared`. A self-contained checker pass — **`grade.rs` (Pass 3d)**, run after type-checking so the trusted type checker is untouched (KC-3) — enforces: **G-App** (a call argument's grade must satisfy, `⊒`, its parameter's declared demand — passing a `@ Empirical` value to a `@ Exact` parameter is an explicit `CheckError`); **G-Weaken** (an `@ g` on a `let` ascription, a value ascription, or a function **return** may only *weaken* — VR-5 as a typing rule); the **meet** composition rule **G-Let/G-Con/G-Op** (a value built from parts carries the weakest part's grade); **`G-Match/A`** (Design A — the scrutinee's *control* grade does **not** degrade the result, but a destructured field binder inherits the scrutinee's *data* grade, so genuine data flow is tracked); and **G-Swap** (the endorsement point — the certificate **reference** is trusted at the type level, its validity discharged by the RFC-0002 checker at elaboration/runtime per **R18-Q4**, keeping the proof checker out of the type checker — KC-3). `Strength` gains the lattice ops `rank`/`meet`/`satisfies` (the single encoding of the chain order). **`elab.rs` no longer returns `Residual` for an `@ g` index** — a grade, like a type, is statically checked and **erased** (no L0 node — KC-3); the six former `@ g` Residual sites (prelude return, `let`/swap/ascribe, inline-call + Fix-fn param/return) now lower transparently. **Unannotated default (R18-Q5, the checker's call): modular/bottom** — an unannotated parameter demands `Declared` and an unannotated return advertises `Declared`, so grading **only ever *bites* where an `@ g` is written** (never breaks existing un-annotated code) and a function's advertised grade is exactly what its signature writes (S2/LR-6); cross-function return-grade *inference* (an SCC fixpoint) is **stage 1b**, deliberately not built here. **Honesty (VR-5):** the pass is tagged **`Declared`** (it enforces the *design*); the noninterference *theorem* stays **Declared-with-argument** — **not** upgraded; the pass can only ever reject or over-degrade, never silently upgrade a grade (G2). **RFC-0018 → Enacted** (stage 1a — the RFC's deliverable; stages 1b/2 remain future RFCs) per the M-648 audit criterion; **DN-14 §3 row 11 → `present (stage 1a)`**. `cargo fmt`/`clippy -D warnings`/`cargo test -p mycelium-l1` + the full `just check` green; new tests: the lattice ops, the headline `@ Exact → @ Exact` accept, the `@ Empirical → @ Exact` call refusal, body-vs-return refusal, `let`-ascription-may-only-weaken, swap-endorses-a-Proven-return, and an un-annotated-code-unaffected regression; the prior elab "`@ g` is a Residual" test is rewritten to assert it now elaborates. The two architecturally-significant choices (the modular/bottom unannotated default; swap = trust-the-cert-reference-at-the-type-level) were maintainer-confirmed. **Review hardening (Copilot, #377):** the bare-pattern **ctor-vs-binder** resolution is now done **once in the type checker** — `Cx::resolve_pattern` rewrites a nullary-constructor `Pattern::Ident` *of the scrutinee's own type* into an explicit `Ctor(name, [])` in the canonical checked AST — instead of by a *global* constructor scan inside the grade pass. The global scan was a latent **unsound grade-upgrade**: a binder whose name merely collided with a nullary ctor of an *unrelated* type was mis-classified as a ctor, dropping its binding so a later reference graded `Exact` instead of the (weaker) scrutinee grade. Pass 3d now walks the **resolved (canonical) own-fn *and* impl-method bodies** (the latter newly threaded out of Pass 3b) and is fully **type-free** (a binder is always a binder, a `Ctor` always a constructor); a cross-type-collision regression test pins the fix, and `eval`/`totality` benefit from the same canonical patterns. (M-663; E7-1; RFC-0018)
