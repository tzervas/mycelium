...
| **`rsm`** | `rsm.md` | **cross-cutting (coordination, not a single tree)** — W1 owns `crates/mycelium-l1/**` · `lib/std/**` · tests; W2 owns `docs/**` · `.claude/**` · `tools/github/**` · `docs/api-index/` | Sonnet · W1 serial-on-L1 ∥ W2 docs (largely disjoint) | Status: **ready** (created 2026-06-26 post-security-posture); branches off `dev`. **W1** M-753 width-generics → M-718 generic math + generic-key lookup → M-719 conformance (DN-41/42 ratified). **W2** documentation-currency sweep (current-state + `PROJECTED` annotations; `just docs-index`; issues/idmap/memory; promote agent-durability procedures to CLAUDE.md). **W3** capture-only: future DN/RFC/research stubs — F1 OSV-of-`.myc` (post-1.0), F2 honest-insecurity-disclosure gate (DN-44 §1.1), F3 projection (RFC-0021 relationship), F4 L3→L4/`reveal` lowering, F5 kernel/primitives consolidation + multi-kernel question, F6 post-critical testing-refactor/DRY/perf passes. deps: DN-42 ✅ (M-753); coordinates with `s10`/`kpr`/`lib10` |
