...
| **`trx2`** (M-991 + E32/E33/E34) | `crates/mycelium-transpile/**` · `crates/mycelium-doc/**` · `tools/docgen/**` · `gen/myc-drafts/**` · `docs/lib-index/**` | ▶ **FIRED 2026-07-06 — wave 1 landed on `dev`** (stacked PRs #1205→#1207 + the drafts PR; per-PR agent review loop, 3 HIGHs found+fixed in-loop). **E32-1 done** (M-1000 vet loop + M-1001 gap closure; **M-991 verdict: NO-GO bulk porter / GO gap-profiling instrument**, DN-34 §8.7–8.8, `Empirical`); **E34-1 done** (M-1004/M-1005 `docs/lib-index/`, drift-gated); **E33-1 wave-1 done** (M-1002/M-1003 `gen/myc-drafts/` — union checked_fraction **3.7%** over the 17-target port surface, DN-34 §8.9 812-gap worklist). **M-1006 phase-1 LANDED (2026-07-06, PR #1226 → `dev`, promoted to `main`):** two disjoint-file leaves hardened the transpiler against §8.9 — generic type-applications now map to `type_args` (`GenericBound` 59→46), string/float/array literal arms, sharpened diagnostics; expressible 6.06%→6.19%, checked flat 3.69% (DN-34 §8.10). The §8.10 residual enumeration declares the dominant classes (type-coverage scalars, named-field structs/variants, imports, bounded generics, derives) **language-surface design (E18-1), not transpiler defects** — current-corpus transpiler-fixable surface near-exhausted. **Remaining: M-1006 later phases** — expand the target set beyond the port surface in controlled phases (per-phase minting) |
