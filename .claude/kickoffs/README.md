...
| **`trx2`** (M-991 + E32/E33/E34) | `crates/mycelium-transpile/**` · `crates/mycelium-doc/**` · `tools/docgen/**` · `gen/myc-drafts/**` · `docs/lib-index/**` | ▶ **FIRED 2026-07-06 — wave 1 landed on `dev`** (stacked PRs #1205→#1207 + the drafts PR; per-PR agent review loop, 3 HIGHs found+fixed in-loop). **E32-1 done** (M-1000 vet loop + M-1001 gap closure; **M-991 verdict: NO-GO bulk porter / GO gap-profiling instrument**, DN-34 §8.7–8.8, `Empirical`); **E34-1 done** (M-1004/M-1005 `docs/lib-index/`, drift-gated); **E33-1 wave-1 done** (M-1002/M-1003 `gen/myc-drafts/` — union checked_fraction **3.7%** over the 17-target port surface, DN-34 §8.9 812-gap worklist). **M-1006 ladder §8.10–§8.16 landed** (2026-07-07): String→`Bytes` (§8.14) is the largest lever (union `checked_fraction`→**5.79%**), landed **to `main`** (PR #1267). Then — **kernel UNFROZEN** (maintainer determination) — a **prim-gap closure wave** landed **CU-1/CU-2/CU-4/CU-6** (Π **59→66**: `bit.mul` unsigned multiply, `flt.is_nan/is_finite/is_infinite`, `bit.popcount/clz/ctz`, signed comparators + the `std.math` surface). **Remaining** (durable record: DN-34 §8.16 + `trx2.md` §Session-C UPDATE): **CU-3** float↔int, **CU-5** executable `wrapping`, **CU-7** arbitrary-width ternary, the **transpiler operator/comparator emission** (D3 operand-type inference), spec-doc sync + CHANGELOG; **deferred to design**: CU-6 rotate/reverse, CU-8 atomics, CU-9 Dense dtype |
| **`mem`** (DN-87 · E39-1 + M-1015…M-1019) | `crates/mycelium-tero/**` · `tools/tero/**` · `.claude/skills/tero-*/**` | ▶ **FIRED 2026-07-07 — named `mycelium-tero`** (maintainer; a quiet homage to Atsushi Tero). The transparent memory substrate & agent knowledge API: hybrid deterministic-index + VSA-semantic-layer substrate (the improved-on-RAG claim `Empirical`-gated, M-1018), MCP + HTTP fronts + skills, Rust core + Python ingestion v0, the Mycelium-lang package phase-gated (M-1019 ⟂ M-993). Scaffold landed with the naming PR; wave 1 = M-1015, then M-1016/M-1017/M-1018 parallel. See [`mem.md`](mem.md) |
| **`spw`** (E33-1 · ADR-042/043) | `lib/std/**` (`.myc` ports) · `crates/mycelium-std-conformance/tests/std_*.rs` (per-module differentials) | ▶ **READY (Wave-0 pilot) — the desktop/parallel stdlib port wave.** Hand-vet-port the unported stdlib crates to `lib/std/<mod>.myc` (transpiler as SCAFFOLD only — M-991, `checked%` ~0–8%; graduate the clean fraction from `gen/myc-drafts/stdlib/<mod>/`), each with its three-way conformance differential. **Disjoint from `boot10`'s semcore SERIAL lane** (owns `lib/compiler/**` + `mycelium-l1/**`), so it runs concurrently — designed to run on the maintainer's desktop while the cloud drives semcore. Pilot targets: `std-numerics`/`std-time`/`std-content`. Heavy durability via `scripts/wave-desktop-checks.sh`. See [`spw.md`](spw.md) |
