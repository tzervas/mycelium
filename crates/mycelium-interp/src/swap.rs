...
/// — which owns a `Box<dyn SwapEngine>` — across rayon's worker threads, so every engine must be
/// safely readable from multiple threads at once. Every shipped/known implementation (the identity
/// engine here, the M-120 certified engines in `mycelium-cert`) is a plain, interior-mutability-free
/// struct and satisfies this automatically; a future engine that needs interior mutability would need
/// its own synchronization (e.g. a `Mutex`), same as any other `Sync` type.
