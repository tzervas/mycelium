...
fn corpus() -> Vec<&'static str> {
    vec![
        ...
        "nodule d;\n\nfn widen(x: Binary{8}) => Ternary{6} =\n  swap(not(x), to: Ternary{6}, policy: rt);\n\nfn main() => Ternary{6} =\n  widen(0b1011_0010);",
...
fn data_corpus() -> Vec<&'static str> {
    vec![
        ...
        "nodule d;\n\ntype Box = Mk(Binary{8});\n\nfn flip(x: Box) => Binary{8} =\n  match x { Mk(b) => not(b) };\n\nfn main() => Binary{8} =\n  flip(Mk(0b1010_1010));",
        // `if` desugaring to a Bool match
        "nodule d;\n\nfn pick(b: Bool) => Binary{8} =\n  if b then 0b1111_1111 else 0b0000_0000;\n\nfn main() => Binary{8} =\n  pick(True);",
        ...
        "nodule d;\n\ntype Pair = Mk(Bool, Bool);\n\nfn both(p: Pair) => Bool =\n  match p { Mk(True, b) => b, Mk(False, _) => False };\n\nfn main() => Bool =\n  both(Mk(True, False));",
        ...
        "nodule d;\n\ntype Nat = Z | S(Nat);\n\nfn hi(n: Nat) => Binary{8} =\n  match n { Z => 0b1111_1111, S(m) => lo(m) };\n\nfn lo(n: Nat) => Binary{8} =\n  match n { Z => 0b0000_0000, S(m) => hi(m) };\n\nfn main() => Binary{8} =\n  hi(S(S(Z)));",
        ...
        "nodule d;\n\ntype Nat = Z | S(Nat);\n\ntype Two = Mk(Nat, Nat);\n\nfn shrink(t: Two) => Nat =\n  match t { Mk(Z, b) => b, Mk(S(a), b) => expand(Mk(a, b)) };\n\nfn expand(t: Two) => Nat =\n  match t { Mk(a, Z) => a, Mk(a, S(b)) => shrink(Mk(a, b)) };\n\nfn main() => Nat =\n  shrink(Mk(S(Z), S(Z)));",
    ]
}
...
fn recovery_match_over_a_result_sum_agrees_three_ways() {
    ...
    let src = "nodule d;\n\ntype Result = Ok(Binary{8}) | Err(Binary{8});\n\nfn recover(r: Result) => Binary{8} =\n  match r { Ok(v) => v, Err(e) => 0b0000_0000 };\n\nfn main() => Binary{8} =\n  recover(Err(0b1111_1111));";
    let env = check_nodule(&parse(src).unwrap()).unwrap();
    let registry = build_registry(&env).unwrap();
...
fn cross_nodule_program_runs_three_way() {
    ...
    let src = "phylum app.cross\n\nnodule a;\n\npub fn helper(x: Binary{8}) => Binary{8} =\n  not(x);\n\nnodule b;\n\nuse a.*;\n\nfn main() => Binary{8} =\n  helper(0b1011_0010);";
...
fn reclaim_sequential_reference_runs_three_way() {
    let src = "nodule d;\n\nfn main() => Binary{8} =\n  reclaim(0b0000_0001) { not(0b1010_1010) };";
...
fn reclaim_real_supervision_driver_dispatches_with_explain() {
    ...
    let bad_src =
        "nodule d;\n\nfn main() => Binary{8} =\n  reclaim(0b0000_0001) { add_bin(0b1111_1111, 0b0000_0001) };";
