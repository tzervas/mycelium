...
fn bool_bridge_from_comparison_bit() {
    ...
    let src = "nodule d;\n\nfn main() => Bool =\n  match eq(0b1010_0000, 0b1010_0000) { 0b1 => True, _ => False };";
...
fn and_or_surfaced() {
    assert_three_way(
        "and",
        "nodule d;\n\nfn main() => Binary{8} =\n  and(0b1100_1010, 0b1010_1010);",
        &Repr::Binary { width: 8 },
        &Payload::Bits("10001010".chars().map(|c| c == '1').collect()),
    );
    assert_three_way(
        "or",
        "nodule d;\n\nfn main() => Binary{8} =\n  or(0b1100_1010, 0b1010_1010);",
        &Repr::Binary { width: 8 },
...
fn add_bin_in_range() {
    ...
    assert_three_way(
        "add_bin",
        "nodule d;\n\nfn main() => Binary{8} =\n  add_bin(0b0000_0001, 0b0000_0010);",
        &Repr::Binary { width: 8 },
...
fn sub_bin_in_range() {
    ...
    assert_three_way(
        "sub_bin",
        "nodule d;\n\nfn main() => Binary{8} =\n  sub_bin(0b0000_0101, 0b0000_0010);",
        &Repr::Binary { width: 8 },
        &Payload::Bits("00000011".chars().map(|c| c == '1').collect()),
    );
}
...
fn add_bin_overflow_refuses_on_every_path() {
    let src = "nodule d;\n\nfn main() => Binary{8} =\n  add_bin(0b1111_1111, 0b0000_0001);";
    let env = check_nodule(&parse(src).expect("parses")).expect("checks");
...
fn sub_bin_underflow_refuses_on_every_path() {
    let src = "nodule d;\n\nfn main() => Binary{8} =\n  sub_bin(0b0000_0000, 0b0000_0001);";
    let env = check_nodule(&parse(src).expect("parses")).expect("checks");
...
fn bytes_get_surface_three_way() {
    ...
    assert_three_way(
        "bytes_get index 1",
        "nodule d;\n\nfn main() => Binary{8} =\n  bytes_get(0x01_02_03, 0b0000_0001);",
        &Repr::Binary { width: 8 },
        &Payload::Bits(want),
    );
}
