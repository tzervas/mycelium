...
What is built (MEM-4 B0 + Increment 1): naive fully-owned RC emission (`emit::emit`), borrow elision for non-escaping let-bindings (`emit::emit_elided`), a structural balance invariant checker (`balance`), and a reference RC-evaluator differential (`eval::differential`) that checks owned vs elided emissions reclaim the same multiset with no use-after-free while strictly reducing `Dup` count. Recursion (`Fix`/`FixGroup`) is refused explicitly and is a later increment.

## Key items

- `rc_ir::RcNode` — the RC-annotated IR mirror of the Core IR first-order fragment plus `Dup`/`Drop`/`Borrow`/`DropAfter` wrappers.
- `rc_ir::Mode` — per-binding ownership mode: `Owned` or `Borrowed`.
- `emit::emit` — naive fully-owned RC emission (`Node → RcNode`): `k` uses → `k-1` `Dup`s, unused binding → `Drop`.
...
- `eval::differential` — the reference RC-evaluator differential (the Q3 soundness check from DN-33 §8.1).
